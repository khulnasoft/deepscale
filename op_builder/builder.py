...
try:
    ...
    print(f"{WARNING} unable to import torch, please install it if you want to pre-compile any deepscale ops.")
else:
...
# to build deepscale and system-wide installed cuda 11.2
...
def assert_no_cuda_mismatch(name=""):
    ...
    if sys_cuda_version != torch_cuda_version:
        if (cuda_major in cuda_minor_mismatch_ok and sys_cuda_version in cuda_minor_mismatch_ok[cuda_major]
            ...
            print(
                f"{WARNING} DeepScale Op Builder: Installed CUDA version {sys_cuda_version} does not match the "
                f"version torch was compiled with {torch.version.cuda}."
        ...
        raise CUDAMismatchException(
            f">- DeepScale Op Builder: Installed CUDA version {sys_cuda_version} does not match the "
...
class OpBuilder(ABC):
    ...
    def absolute_name(self):
        '''
        Returns absolute build path for cases where the op is pre-installed, e.g., deepscale.ops.adam.cpu_adam
        will be installed as something like: deepscale/ops/adam/cpu_adam.so
        '''
        pass
    ...
    def sources(self):
        '''
        Returns list of source files for your op, relative to root of deepscale package (i.e., DeepScale/deepscale)
        '''
        pass
    ...
    def validate_torch_version(torch_info):
        ...
        if install_torch_version != current_torch_version:
            raise RuntimeError("PyTorch version mismatch! DeepScale ops were compiled and installed "
                               "with a different version than what is being used at runtime. "
                               f"Please re-install DeepScale or switch torch versions. "
    ...
    def validate_torch_op_version(torch_info):
        if not OpBuilder.is_rocm_pytorch():
            ...
            if install_cuda_version != current_cuda_version:
                raise RuntimeError("CUDA version mismatch! DeepScale ops were compiled and installed "
                                   "with a different version than what is being used at runtime. "
                                   f"Please re-install DeepScale or switch torch versions. "
            ...
            if install_hip_version != current_hip_version:
                raise RuntimeError("HIP version mismatch! DeepScale ops were compiled and installed "
                                   "with a different version than what is being used at runtime. "
                                   f"Please re-install DeepScale or switch torch versions. "
    ...
    def include_paths(self):
        '''
        Returns list of include paths, relative to root of deepscale package (i.e., DeepScale/deepscale)
        '''
        return []
    ...
    def deepscale_src_path(self, code_path):
    ...
    def load(self, verbose=True):
        ...
        from deepscale.git_version_info import installed_ops, torch_info, accelerator_name
    ...
    def jit_load(self, verbose=True):
        ...
        sources = [os.path.abspath(self.deepscale_src_path(path)) for path in self.sources()]
        extra_include_paths = [os.path.abspath(self.deepscale_src_path(path)) for path in self.include_paths()]
