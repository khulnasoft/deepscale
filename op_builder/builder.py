...
try:
    ...
    print(
        f"{WARNING} unable to import torch, please install it if you want to pre-compile any deepscale ops."
    )
...
def assert_torch_info(torch_info):
    ...
    if (install_cuda_version != current_cuda_version
        ...
        raise RuntimeError(
            "PyTorch and CUDA version mismatch! DeepScale ops were compiled and installed "
            "with a different version than what is being used at runtime. Please re-install "
            f"DeepScale or switch torch versions. DeepScale install versions: "
...
class OpBuilder(ABC):
    ...
    def jit_load(self, verbose=True):
        ...
        sources = [self.deepscale_src_path(path) for path in self.sources()]
        extra_include_paths = [
            self.deepscale_src_path(path) for path in self.include_paths()
        ]
        op_module = load(
            name=self.name,
