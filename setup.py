...
if sys.platform == "win32":
    shutil.rmtree('.\\deepscale\\ops\\csrc', ignore_errors=True)
    pathlib.Path('.\\deepscale\\ops\\csrc').unlink(missing_ok=True)
    shutil.copytree('.\\csrc', '.\\deepscale\\ops\\csrc', dirs_exist_ok=True)
    shutil.rmtree('.\\deepscale\\ops\\op_builder', ignore_errors=True)
    pathlib.Path('.\\deepscale\\ops\\op_builder').unlink(missing_ok=True)
    shutil.copytree('.\\op_builder', '.\\deepscale\\ops\\op_builder', dirs_exist_ok=True)
    shutil.rmtree('.\\deepscale\\accelerator', ignore_errors=True)
    pathlib.Path('.\\deepscale\\accelerator').unlink(missing_ok=True)
    shutil.copytree('.\\accelerator', '.\\deepscale\\accelerator', dirs_exist_ok=True)
    egg_info.manifest_maker.template = 'MANIFEST_win.in'

# Parse the DeepScale version string from version.txt.
...
with open('deepscale/git_version_info_installed.py', 'w') as fd:
...
if sys.platform == "win32":
    scripts = ['bin/deepscale.bat', 'bin/ds', 'bin/ds_report.bat', 'bin/ds_report']
else:
    scripts = [
        'bin/deepscale', 'bin/deepscale.pt', 'bin/ds', 'bin/ds_ssh', 'bin/ds_report', 'bin/ds_bench', 'bin/dsr',
        'bin/ds_elastic', 'bin/ds_nvme_tune', 'bin/ds_io'
    ]

start_time = time.time()

setup(name='deepscale',
      version=version_str,
      description='DeepScale library',
      long_description=readme_text,
      long_description_content_type='text/markdown',
      author='DeepScale Team',
      author_email='deepscale-info@microsoft.com',
      url='http://deepscale.ai',
      project_urls={
          'Documentation': 'https://deepscale.readthedocs.io',
          'Source': 'https://github.com/khulnasoft/DeepScale',
      },
      install_requires=install_requires,
      extras_require=extras_require,
      packages=find_packages(include=['deepscale', 'deepscale.*']),
...
end_time = time.time()
print(f'deepscale build time = {end_time - start_time} secs')
