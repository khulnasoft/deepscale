...
class HashChecker(ContentChecker):
    pattern = re.compile(r'(?P<hash_name>sha1|sha224|sha384|sha256|sha512|md5)='
                         r'(?P<expected>[a-f0-9]+)')
...
class PackageIndex(Environment):
    ...
    def scan_egg_links(self, search_path):
        ...
        egg_links = ((path,
                      entry) for path in dirs for entry in os.listdir(path)
    ...
    def process_index(self, url, page):
        ...
        return PYPI_MD5.sub(lambda m: '<a href="%s#md5=%s">%s</a>' % m.group(1,
                                                                             3,
                                                                             2),
                            page)
    ...
    def check_hash(self, checker, filename, tfp):
        ...
        if not checker.is_valid():
            tfp.close()
            os.unlink(filename)
            raise DistutilsError("%s validation failed for %s; "
    ...
    def gen_setup(self, filename, fragment, tmpdir):
        match = EGG_FRAGMENT.match(fragment)
        dists = match and [
            d for d in interpret_distro_name(filename,
                                             match.group(1),
                                             None) if d.version
        ] or []
...
class PyPIConfig(configparser.RawConfigParser):
    ...
    def creds_by_repository(self):
        sections_with_repositories = [
            section for section in self.sections() if self.get(section,
                                                               'repository').strip()
        ]
