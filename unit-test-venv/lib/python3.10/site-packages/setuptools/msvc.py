...
def _msvc14_get_vc_env(plat_spec):
    ...
    try:
        out = subprocess.check_output(
            'cmd /u /c "{}" {} && set'.format(vcvarsall,
                                              plat_spec),
            stderr=subprocess.STDOUT,
        ).decode('utf-16le',
                 errors='replace')
    except subprocess.CalledProcessError as exc:
        raise distutils.errors.DistutilsPlatformError("Error executing {}".format(
            exc.cmd)) from exc
    ...
    env = {
        ...
        for key,
        _,
        value in (line.partition('=') for line in out.splitlines()) if key and value
    }
...
class PlatformInfo:
    ...
    def current_dir(self, hidex86=False, x64=False):
        ...
        """
        return ('' if (self.current_cpu == 'x86' and hidex86) else r'\x64' if
                (self.current_cpu == 'amd64' and x64) else r'\%s' % self.current_cpu)
    ...
    def target_dir(self, hidex86=False, x64=False):
        ...
        """
        return ('' if (self.target_cpu == 'x86' and hidex86) else r'\x64' if
                (self.target_cpu == 'amd64' and x64) else r'\%s' % self.target_cpu)
    ...
    def cross_dir(self, forcex86=False):
        ...
        return ('' if self.target_cpu == current else self.target_dir().replace(
            '\\',
            '\\%s_' % current))
...
class SystemInfo:
    ...
    def _use_last_dir_name(path, prefix=''):
        ...
        str
            name
        """
        matching_dirs = (dir_name for dir_name in reversed(listdir(path))
                         if isdir(join(path,
                                       dir_name)) and dir_name.startswith(prefix))
    ...
    def VCTools(self):
        ...
        if self.vs_ver == 14.0:
            ...
            host_dir = (r'bin\HostX86%s'
                        if self.pi.current_is_x86() else r'bin\HostX64%s')
