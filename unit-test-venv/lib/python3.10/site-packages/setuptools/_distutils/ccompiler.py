...
class CCompiler:
    ...
    def _check_macro_definitions(self, definitions):
        ...
        for defn in definitions:
            if not (isinstance(defn,
                               tuple) and
                    (len(defn) in (1,
                                   2) and
                     (isinstance(defn[1],
                                 str) or defn[1] is None)) and isinstance(defn[0],
                                                                          str)):
    ...
    def link_shared_lib(
        ...
        self.link(
            ...
            objects,
            self.library_filename(output_libname,
                                  lib_type='shared'),
            output_dir,
            libraries,
            library_dirs,
...
""" % funcname)
    ...
    def object_filenames(self, source_filenames, strip_dir=0, output_dir=''):
        if output_dir is None:
            output_dir = ''
        return list(
            self._make_out_path(output_dir,
                                strip_dir,
                                src_name) for src_name in source_filenames)
    ...
    def _make_out_path(self, output_dir, strip_dir, src_name):
        ...
        try:
            ...
            raise UnknownFileError("unknown file type '{}' (from '{}')".format(
                ext,
                src_name))
        if strip_dir:
...
def get_default_compiler(osname=None, platform=None):
    ...
    for pattern, compiler in _default_compilers:
        if (re.match(pattern,
                     platform) is not None or re.match(pattern,
                                                       osname) is not None):
            return compiler
