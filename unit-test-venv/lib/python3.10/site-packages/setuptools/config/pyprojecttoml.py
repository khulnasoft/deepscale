...
def _skip_bad_config(project_cfg: dict,
                     setuptools_cfg: dict,
                     dist: Optional["Distribution"]) -> bool:
    """Be temporarily forgiving with invalid ``pyproject.toml``"""
    # See pypa/setuptools#3199 and pypa/cibuildwheel#1064

    if dist is None or (dist.metadata.name is None and dist.metadata.version is None
...
class _ConfigExpander:
    ...
    def _expand_all_dynamic(self, dist: "Distribution", package_dir: Mapping[str, str]):
        ...
        obtained_dynamic = {
            field: self._obtain(dist,
                                field,
                                package_dir)
            for field in self.dynamic if field not in special
        }
        obtained_dynamic.update(
            self._obtain_entry_points(dist,
                                      package_dir) or {},
            version=self._obtain_version(dist,
                                         package_dir),
    ...
    def _expand_directive(self,
                          specifier: str,
                          directive,
                          package_dir: Mapping[str,
                                               str]):
        with _ignore_errors(self.ignore_option_errors):
            root_dir = self.root_dir
    ...
    def _obtain_optional_dependencies(self, dist: "Distribution"):
        ...
        if "optional-dependencies" in self.dynamic_cfg:
            ...
            return {
                ...
                for group,
                directive in optional_dependencies_map.items()
            }
