...
def get_flag(var, fallback, expected=True, warn=True):
    ...
    if val is None:
        if warn:
            warnings.warn(
                f"Config variable '{var}' is unset, Python ABI tag may "
                "be incorrect",
                RuntimeWarning,
                stacklevel=2,
            )
...
def get_abi_tag():
    ...
    if not soabi and impl in ("cp", "pp") and hasattr(sys, "maxunicode"):
        ...
        if get_flag(
                "WITH_PYMALLOC",
                impl == "cp",
                warn=(impl == "cp" and sys.version_info < (3,
                                                           8)),
...
class bdist_wheel(Command):
    ...
    def finalize_options(self):
        ...
        if self.py_limited_api and not re.match(PY_LIMITED_API_PATTERN,
                                                self.py_limited_api):
        ...
        if "universal" in wheel:
            ...
            log.warning("The [wheel] section is deprecated. Use [bdist_wheel] instead.",
                        )
    ...
    def get_tag(self):
        ...
        plat_name = (plat_name.lower().replace("-",
                                               "_").replace(".",
                                                            "_").replace(" ",
                                                                         "_"))
