...
try:
    ...
    from deepscale.utils import logger as accel_logger
except ImportError as e:
    accel_logger = None

try:
    from accelerator.abstract_accelerator import DeepScaleAccelerator as dsa1
except ImportError as e:
    dsa1 = None
try:
    from deepscale.accelerator.abstract_accelerator import DeepScaleAccelerator as dsa2
except ImportError as e:
    dsa2 = None
...
def _validate_accelerator(accel_obj):
    ...
    # and run time (deepscale.accelerator.abstract_accelerator)
    # and extension would import the
    # run time abstract_accelerator/DeepScaleAccelerator as its base
    # class, so we need to compare accel_obj with both base class.
    # if accel_obj is instance of DeepScaleAccelerator in one of
    # accelerator.abstractor_accelerator
    # or deepscale.accelerator.abstract_accelerator, consider accel_obj
    ...
    if not ((dsa1 is not None and isinstance(accel_obj, dsa1)) or (dsa2 is not None and isinstance(accel_obj, dsa2))):
        raise AssertionError(f"{accel_obj.__class__.__name__} accelerator is not subclass of DeepScaleAccelerator")
...
def get_accelerator():
    ...
    ds_set_method = None
    # 1. Detect whether there is override of DeepScale accelerators from environment variable.
    if "DS_ACCELERATOR" in os.environ.keys():
        ...
        if accelerator_name == "xpu":
            ...
            try:
                import intel_extension_for_deepscale  # noqa: F401 # type: ignore
            except ImportError as e:
                raise ValueError(
                    f"XPU_Accelerator external requires intel_extension_for_deepscale, which is not installed on this system."
                )
    ...
    if accelerator_name is None:
        ...
        try:
            from intel_extension_for_deepscale import XPU_Accelerator  # noqa: F401,F811 # type: ignore
...
**************************************************************************
-----------[code] test_set.py -----------
from deepscale.accelerator.cuda_accelerator import CUDA_Accelerator
cu_accel = CUDA_Accelerator()
logger.info(f'{id(cu_accel)=}')
from deepscale.accelerator import set_accelerator, get_accelerator
...
my_accelerator=<deepscale.accelerator.cuda_accelerator.CUDA_Accelerator object at 0x7f025f4bffa0>
my_accelerator.name='cuda'
my_accelerator.communication_backend='nccl'
my_accelerator.HalfTensor().device=device(type='cuda', index=0)
my_accelerator.total_memory()=34089730048
