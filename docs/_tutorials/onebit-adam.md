...
- **DeepScale-enabled:** Start training with DeepScale by providing the following 4 arguments to this script:

```shell
bash run_squad_deepscale.sh <NUM_GPUS> <PATH_TO_CHECKPOINT> <PATH_TO_DATA_DIR> <PATH_TO_OUTPUT_DIR>`
```

The first argument is the number of GPUs to train with, second argument is the path to the pre-training checkpoint, third is the path to training and validation sets (e.g., train-v1.1.json), and fourth is path to an output folder where the results will be saved. This script will invoke `nvidia_run_squad_deepscale.py`.

- **DeepScale with 1-bit Adam enabled:** In order to run with 1-bit Adam feature enabled, the same script (`nvidia_run_squad_deepscale.py`) can be used but there are two options for launching this properly: 1) Launch using deepscale launcher and 2) Launch with mpirun.

To enable the 1-bit compressed training, 1-bit Adam uses an MPI library (E.g. MVAPICH2-GDR, OpenMPI, etc.) as the communication backend, which means that we can use `mpirun` to launch the training job. However, our user-friendly launcher called `deepscale` has been enhanced to launch MPI jobs as well.

### Launch with deepscale

The following helper script in the DeepScaleExamples/BingBertSQuAD will launch the training without the need for setting any `mpirun` parameters. The number of nodes and GPUs will be automatically detected and the job will be launched on all the available resources.

```shell
bash run_squad_deepscale_onebitadam.sh <PATH_TO_OUTPUT_DIR>
```
...
- **DeepScale-enabled:** Start training with DeepScale by running the command below:

```shell
bash ds_train_bert_bsz64k_seq128.sh
```

- **DeepScale with 1-bit Adam enabled:** In order to run with 1-bit Adam feature enabled, the same script (`deepscale_train.py`) can be used but there are two options for launching this properly:

### Launch with deepscale

As discussed for BingBertSQuAD fine-tuning, we can simply use the `deepscale` launcher to launch our BERT pre-training jobs as follows.
