...
The first step to apply DeepScale is adding DeepScale arguments to CIFAR-10 model, using `deepscale.add_config_arguments()` function as below.
...
We create `model_engine`, `optimizer` and `trainloader` with the help of `deepscale.initialize`, which is defined as following:
...
The `model` returned by `deepscale.initialize` is the _DeepScale Model Engine_ that we will use to train the model using the forward, backward and step API.
...
Zeroing the gradients is handled automatically by DeepScale after the weights have been updated using a mini-batch.



### Configuration

The next step to use DeepScale is to create a configuration JSON file (ds_config.json). This file provides DeepScale specific parameters defined by the user, e.g., batch size, optimizer, scheduler and other parameters.
...
cmd=['pdsh', '-w', 'worker-0', 'export NCCL_VERSION=2.4.2; ', 'cd /data/users/deepscale/test/ds_v2/examples/cifar;', '/usr/bin/python', '-u', '-m', 'deepscale.pt.deepscale_launch', '--world_info=eyJ3b3JrZXItMCI6IFswXX0=', '--node_rank=%n', '--master_addr=192.168.0.22', '--master_port=29500', 'cifar10_deepscale.py', '--deepscale', '--deepscale_config', 'ds_config.json']
...
worker-0: DeepScale info: version=2.1, git-hash=fa937e7, git-branch=master
worker-0: [INFO 2020-02-06 19:53:49] Set device to local rank 0 within node.
worker-0: 1 1
worker-0: [INFO 2020-02-06 19:53:56] Using DeepScale Optimizer param name adam as basic optimizer
worker-0: [INFO 2020-02-06 19:53:56] DeepScale Basic Optimizer = FusedAdam (
...
worker-0: )
worker-0: [INFO 2020-02-06 19:53:56] DeepScale using configured LR scheduler = WarmupLR
worker-0: [INFO 2020-02-06 19:53:56] DeepScale LR Scheduler = <deepscale.pt.deepscale_lr_schedules.WarmupLR object at 0x7f64c4c09c18>
worker-0: [INFO 2020-02-06 19:53:56] rank:0 step=0, skipped=0, lr=[0.001], mom=[[0.8, 0.999]]
worker-0: DeepScaleLight configuration:
...
worker-0:   tensorboard_job_name ......... DeepScaleJobName
