...
DeepScale v0.3 includes new support for pipeline parallelism! Pipeline
...
DeepScale's training engine provides hybrid data and pipeline parallelism and
...
![3D parallelism in DeepScale](/assets/images/3d-parallelism.png)

DeepScale uses *gradient accumulation* to extract pipeline parallelism (shown
...
Below is an illustration of how DeepScale will train a batch with eight
...
DeepScale strives to accelerate *and* simplify the process of pipeline
...
parallelized by DeepScale with no modification:
```python
net = nn.Sequential(
...
from deepscale.pipe import PipelineModule
...
are present, DeepScale will also use hybrid data parallelism.
...
`backward()` and `step()`.
Instead, DeepScale's pipeline engine provides a `train_batch()` method that
...
traditional data parallel DeepScale:
```python
train_iter = iter(train_loader)
...
For convenience, the DeepScale pipeline engine can construct a distributed
data loader when a dataset is provided to `deepscale.initialize()`. DeepScale
handles the rest of the complexity of data loading, and so the pipeline
training loop becomes:
```python
engine, _, _, _ = deepscale.initialize(
    args=args,
    model=net,
...
Of course, DeepScale will work with any data loader that you wish to use.
...
DeepScale provides a convenience class `deepscale.utils.RepeatingLoader` that
simply wraps an iterable such as a data loader and restarts it whenever the
end is reached:
```python
train_loader = deepscale.utils.RepeatingLoader(train_loader)
train_iter = iter(train_loader)
for step in range(args.steps):
    loss = engine.train_batch(data_iter=train_iter)
```
...
balance. DeepScale provides several mechanisms for partitioning the model
...
DeepScale provides a `LayerSpec` class that delays the construction of
...
```python
from deepscale.pipe import PipelineModule, LayerSpec
...
DeepScale provides a `TiedLayerSpec` that is an extension of
