...
**Note:** Currently DeepScale Sparse Attention can be used only on NVIDIA V100 GPU using Torch >= 1.5 and Cuda 10.1 or 10.2.
{: .notice--warning}

## Sparse attention modules
* **MatMul**: This module handles block-sparse matrix-matrix multiplication. Currently it supports SDD, DSD, and DDS as described in [DeepScale Sparse Attention](https://www.deepscale.khulnasoft.com/news/2020/09/08/sparse-attention.html) section.
...
* **BertSparseSelfAttention**: This module contains a simplified BertSelfAttention layer that can be used instead of original dense Bert Self-Attention layer. Our implementation is based on [DeepScaleExample](https://github.com/khulnasoft-lab/DeepScaleExamples/blob/master/bing_bert/nvidia/modelingpreln.py#L373-#L434).
...
In this section we describe how to use DeepScale Sparse Attention through our [bing_bert](https://github.com/khulnasoft-lab/DeepScaleExamples/blob/master/bing_bert/nvidia/modelingpreln.py) code.

* **Update attention module**: First, you need to update your attention module based on sparse computation. Here, we use [BertSparseSelfAttention](https://github.com/khulnasoft/DeepScale/blob/master/deepscale/ops/sparse_attention/bert_sparse_self_attention.py) which is the sparse version of `BertSelfAttention` from our [bing_bert](https://github.com/khulnasoft-lab/DeepScaleExamples/blob/master/bing_bert/nvidia/modelingpreln.py) code. It rewrites `BertSelfAttention` where it replaces:
...
* **Update encoder model**: Further, you need to update your encoder model to use SA for the attention layer when SA is enabled. Please check our [bing_bert example](https://github.com/khulnasoft-lab/DeepScaleExamples/blob/master/bing_bert/nvidia/modelingpreln.py) in which we use `BertSparseSelfAttention` instead of `BertSelfAttention` when SA is enabled.
...
Please check [our bing_bert runner script](https://github.com/khulnasoft-lab/DeepScaleExamples/blob/master/bing_bert/ds_sa_train_bert_bsz64k_seq128.sh) as an example of how to enable SA with DeepScale launcher.

* **Add sparsity config**: The sparsity config can be set through the [DeepScale JSON config file](https://github.com/khulnasoft-lab/DeepScaleExamples/blob/master/bing_bert/deepscale_bsz64k_lamb_config_seq128.json). In this example, we have used `fixed` sparsity mode that will be described in [How to config sparsity structures](#how-to-config-sparsity-structures) section.
