...
In this tutorial we will be adding DeepScale to the BingBert model for the SQuAD fine-tuning task, called "BingBertSquad" henceforth. We will also demonstrate performance gains.

## Overview

If you don't already have a copy of the DeepScale repository, please clone in
now and checkout the DeepScaleExamples submodule the contains the BingBertSquad
example (DeepScaleExamples/BingBertSquad) we will be going over in the rest of
this tutorial.
...
cd DeepScaleExamples/BingBertSquad
```
...
You also need a pre-trained BERT model checkpoint from either DeepScale, [HuggingFace](https://github.com/huggingface/transformers), or [TensorFlow](https://github.com/google-research/bert#pre-trained-models) to run the fine-tuning. Regarding the DeepScale model, we will use checkpoint 160 from the BERT pre-training [tutorial](/tutorials/bert-pretraining/).

### Running BingBertSquad

- **DeepScale-enabled:** We provide a shell script that you can invoke to start training with DeepScale, it takes 4 arguments: `bash run_squad_deepscale.sh <NUM_GPUS> <PATH_TO_CHECKPOINT> <PATH_TO_DATA_DIR> <PATH_TO_OUTPUT_DIR>`. The first argument is the number of GPUs to train with, second argument is the path to the pre-training checkpoint, third is the path to training and validation sets (e.g., train-v1.1.json), and fourth is path to an output folder where the results will be saved. This script will invoke `nvidia_run_squad_deepscale.py`.
- **Unmodified baseline** If you would like to run a non-DeepScale enabled version of fine-tuning we provide a shell script that takes the same arguments as the DeepScale one named `run_squad_baseline.sh`. This script will invoke `nvidia_run_squad_baseline.py`.

## DeepScale Integration
...
made to the baseline in order to enable DeepScale, you don't have to make these
changes yourself since we have already done them for you.

### Configuration

The `deepscale_bsz24_config.json` file gives the user the ability to specify DeepScale
...
file must be specified using `--deepscale_config
deepscale_bsz24_config.json`. Table 1 shows the fine-tuning configuration
...
The first step to apply DeepScale is adding arguments to BingBertSquad, using `deepscale.add_config_arguments()` in the beginning of the main entry point as in the `main()` function in `nvidia_run_squad_deepscale.py`. The argument passed to `add_config_arguments()` is obtained from the `get_argument_parser()` function in utils.py.
...
DeepScale has an initialization function to wrap the model, optimizer, LR
...
This is identical in both Baseline and DeepScale, and is performed by `loss = model(input_ids, segment_ids, input_mask, start_positions, end_positions)`.

#### Backward pass

In the Baseline script you need to handle the all-reduce operation at the gradient accumulation boundary explicitly by using `enable_need_reduction()` followed by `optimizer.backward(loss)` in FP16 and `loss.backward()` in FP32. In DeepScale, you may simply do `model.backward(loss)`.
...
`optimizer.zero_grad()`. DeepScale handles this internally (by setting the
...
Congratulations! Porting to DeepScale is complete.
...
## Enabling DeepScale's Transformer Kernel for better Throughput

DeepScale's optimized transformer kernel can be enabled during fine-tuning to
increase the training throughput. In addition to supporting the models
pre-trained with DeepScale, the kernel can be used with TensorFlow and
HuggingFace checkpoints.

### Enabling Transformer Kernel

An argument `--deepscale_transformer_kernel` is already created in `utils.py`, we enable the transformer kernel by adding it in the shell script.
...
In the `BertEncoder` class of the modeling source file, DeepScale transformer kernel is created as below when it is enabled by using `--deepscale_transformer_kernel` argument.

```python
if args.deepscale_transformer_kernel:
    from deepscale import DeepScaleTransformerLayer, \
        DeepScaleTransformerConfig, DeepScaleConfig
...
will throw an exception. In the DeepScale configuration file micro batch size
...
dive](https://www.deepscale.ai/2020/05/27/fastest-bert-training.html) on
...
2. `--ckpt_type`, indicates the checkpoint type, `TF` for Tensorflow, `HF` for HuggingFace, default value is `DS` for DeepScale.
...
`run_squad_deepscale.sh` to run the above HuggingFace and TensorFlow examples.
...
1. `--deepscale_transformer_kernel` flag is required for using HuggingFace or TensorFlow pretrained models.

2. `--preln` flag cannot be used with HuggingFace or TensorFlow pretrained models, since they use a post-layer-norm.

3. BingBertSquad will check the pretrained models to have the same vocabulary size and won't be able to run if there is any mismatch. We advise that you use a model checkpoint of the style described above or a DeepScale bing\_bert checkpoint.
...
Table 2. Samples/second for running SQuAD fine-tuning on NVIDIA V100 (16GB) using PyTorch and DeepScale transformer kernels.
...
Table 3. Samples/second for running SQuAD fine-tuning on NVIDIA V100 (32GB) using PyTorch and DeepScale transformer kernels.
...
transformer kernel as described in [DeepScale Transformer
...
### FineTuning model pre-trained with DeepScale Transformer Kernels

Fine-tuning the model pre-trained using DeepScale Transformer and the recipe in [DeepScale Fast-Bert Training](https://www.deepscale.ai/2020/05/27/fastest-bert-training.html) should yield F1 score of 90.5 and is expected to increase if you let the pre-training longer than suggested in the tutorial.
