...
For this tutorial, we will configure a 18 billion parameter GPT-2 model using the DeepScale [Megatron-DeepSpeed](https://github.com/microsoft/Megatron-DeepSpeed/tree/master/) GPT-2 code. We will use 4 nodes of 16x [NVIDIA Tesla V100-SXM3 Tensor Core GPU](https://www.nvidia.com/en-us/data-center/v100/) with 32GB RAM per node for this exercise.
...
An example snippet of deepscale configurations with all three ZeRO++ optimization enable is shown below:
```json
{
    "zero_optimization": {
        "stage": 3,
...
```python
       deepscale pretrain_zeropp_gpt.py \
       ...
       --batch-size 1 \
       --zero-stage 3 \
       --deepscale_config ds_zeropp_config.json \
       --deepscale-activation-checkpointing \
       --fp16 \
