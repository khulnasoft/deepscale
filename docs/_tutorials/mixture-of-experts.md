...
our [cifar10 example](https://github.com/khulnasoft-lab/DeepScaleExamples/tree/master/cifar) going forward.
...
To support different forms of parallelism, we create a notion of DeepScale process groups that resides in ```deepscale.utils.groups.py```
...
```python
    ...
    self.fc3 = deepscale.moe.layer.MoE(hidden_size=84, expert=self.fc3, num_experts=args.num_experts, ...)
...
After that, the model code needs to use the deepscale.moe.layer.MoE API as follows.

```python
self.experts = deepscale.moe.layer.MoE(hidden_size=input_dim, expert=ExpertModule(), num_experts=EXPERTS)
```
...
For a runnable end-to-end example, please look at [cifar10 example](https://github.com/khulnasoft-lab/DeepScaleExamples/tree/master/cifar)

### Combining ZeRO-Offload and DeepScale MoE for very large models

To use MoE Layers in DeepScale, we rely on two parameter groups that are passed to an optimizer. A concrete example to create such groups is available from the [cifar10 example](https://github.com/khulnasoft-lab/DeepScaleExamples/tree/master/cifar).

The relevant function that creates these param groups is as follows.

```python
def create_moe_param_groups(model):
    from deepscale.moe.utils import is_moe_param
...
To run the [cifar10 example](https://github.com/khulnasoft-lab/DeepScaleExamples/tree/master/cifar) with ZeRO-Offload (stage 2) and MoE, please set the ds_config flags
...
Watch this space! We plan to add more interesting and detailed examples of using DeepScale MoE in the coming weeks.
