...
class BaseTestCase(unittest.TestCase):
    def __init__(self, methodName="DeepScale performance test"):
    ...
    def gen_output_name(self, test_config, prefix, baseline_config=False):
        ...
        if test_config["deepscale"] and not baseline_config:
    ...
    def run_gpt2_test(self, test_config, output):
        ds_flag = "-d " + test_config["json"] if test_config["deepscale"] else ""
        ckpt_num = (test_config["ckpt_num_layers"]
                    if "ckpt_num_layers" in test_config else 1)
        other_args = ("-o " +
                      test_config["other_args"] if "other_args" in test_config else "")
