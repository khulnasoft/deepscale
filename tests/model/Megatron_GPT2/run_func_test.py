...
class GPT2FuncTestCase(BaseTestCase):
    ...
    def run_partition_activations_test(self, test_config, r_tol):
        ...
        deepscale_config = test_config["json"]
        baseline_deepscale_config = False
        cpu_optimizer_flag = self.gen_cpu_optimizer_flag(test_config, True)

        # baseline run...
        # turnoff deepscale if baseline deepscale config
        # is not provided
    ...
    def run_test(self, test_config, r_tol):
        ...
        # baseline run...
        # turn off deepscale if a baseline deepscale config
        # is not provided
        if not "baseline" in test_config:
            test_config["deepscale"] = False
        else:
