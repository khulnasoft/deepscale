...
helpFunction()
{
    ...
    echo -e "\t-o other args"
    echo -e "\t-d DeepScale config json file"
    echo -e "\t-z Enable Zero optimization"
    echo -e "\t-p DeepScale master port"
    exit 1
}
...
do
    case "$opt" in
        ...
        o ) other_args="$OPTARG" ;;
        d ) ds_opt="--deepscale --deepscale_config $script_dir/$OPTARG" ;;
...
work_dir="../../../DeepScaleExamples/Megatron-LM/"
run_cmd="(cd ${work_dir} && deepscale --master_port ${master_port} --num_nodes $nodes --num_gpus $gpus pretrain_gpt2.py ${gpt_options})"
echo ${run_cmd}
eval ${run_cmd}
