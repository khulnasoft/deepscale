...
if [ -z "${BingBertSquad_DIR+x}" ]; then
  export BingBertSquad_DIR=../../../DeepScaleExamples/BingBertSquad
...
fp16_config_json=deepscale_bsz24_fp16_config.json
validate_file ${fp16_config_json} "fp16_config_json"
fp32_config_json=deepscale_bsz24_fp32_config.json
...
do
        #run_cmd="nohup bash run_BingBertSquad.sh -g ${num_gpus} -d --deepscale_config ${fp16_config_json} --fp16 > logs/deepscale_fp16_${num_gpus}_`date +"%Y%m%d%H%M%S"`.out 2> logs/deepscale_fp16_${num_gpus}_`date +"%Y%m%d%H%M%S"`.err &"
        run_cmd="bash run_BingBertSquad.sh -g ${num_gpus} -d --deepscale_config ${fp16_config_json} --fp16"
        start_time=`date +"%D %T"`
        echo "---------------begin @ ${start_time}--------------"
        echo ${run_cmd}
        eval ${run_cmd}
        ...
        #run_cmd="nohup bash run_BingBertSquad.sh -g ${num_gpus} -d --deepscale_config ${fp32_config_json} > logs/deepscale_fp32_${num_gpus}_`date +"%Y%m%d%H%M%S"`.out 2> logs/deepscale_fp32_${num_gpus}_`date +"%Y%m%d%H%M%S"`.err &"
        run_cmd="bash run_BingBertSquad.sh -g ${num_gpus} -d --deepscale_config ${fp32_config_json}"
        start_time=`date +"%D %T"`
        echo "---------------begin @ ${start_time}--------------"
        echo ${run_cmd}
        eval ${run_cmd}
