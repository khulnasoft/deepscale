...
echo "deepscale: ${enable_deepscale}"
...
if [[ ${enable_deepscale} == true ]]; then
  BingBertSquad_script=${BingBertSquad_DIR}/nvidia_run_squad_deepscale.py
else
  BingBertSquad_script=${BingBertSquad_DIR}/nvidia_run_squad_baseline.py
fi

JOB_NAME="BingBertSquad_ds-${enable_deepscale}_${num_gpus}-gpu"
