...
from deepscale import DeepScaleTransformerLayer, DeepScaleTransformerConfig
import deepscale

import sys

# if not deepscale.ops.__installed_ops__['transformer']:
...
class DSEncoder(nn.Module):
    def __init__(self, config, weights, biases):
        ...
        self.layer = nn.ModuleList([
            copy.deepcopy(DeepScaleTransformerLayer(config,
                                                    weights,
                                                    biases))
...
@pytest.mark.parametrize(
    "batch_size, hidden_size, seq_len, heads, num_layers, is_preln, use_fp16",
    [
...
def test_forward(batch_size,
    ...
    ds_config = DeepScaleTransformerConfig()
...
@pytest.mark.parametrize(
    "batch_size, small_bsz, hidden_size, seq_len, heads, num_layers, is_preln, use_fp16",
    [
