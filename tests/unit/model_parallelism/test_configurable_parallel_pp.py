...
def get_deepscale_model(model):
    ...
    model, _, _, _ = deepscale.initialize(model=model, model_parameters=model.parameters(), config=ds_config_dict)
...
class TestConfigurablePP(ConfigurablePP):
    ...
    def test_pp_basic(self, inputs, tmpdir):
        ...
        gpt2_pipe_model = GPT2ModelPipe(num_layers=8,
                                        ...
                                        topo=topo)
        model = get_deepscale_model(gpt2_pipe_model)
