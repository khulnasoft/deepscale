...
DEEPSCALE_TEST_TIMEOUT = int(os.environ.get('DS_UNITTEST_TIMEOUT', '600'))
...
class DistributedExec(ABC):
    ...
    _pool_cache = {}
    exec_timeout = DEEPSCALE_TEST_TIMEOUT
    ...
    def _dist_run(self, local_rank, num_procs, master_port, init_method, skip_msg=""):
        if not dist.is_initialized():
            """ Initialize deepscale.comm and execute the user function. """
            if self.set_dist_env:
                ...
                # DeepScale single node launcher would also set LOCAL_SIZE accordingly
            ...
            if self.init_distributed:
                deepscale.init_distributed(dist_backend=self.backend,
