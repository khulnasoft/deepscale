...
import deepscale
from deepscale.accelerator import get_accelerator, is_current_accelerator_supported
...
class no_child_process_in_deepscale_io:

    def __enter__(self):
        # deepscale_io defaults to creating a dataloader that uses a
        ...
        self.old_method = deepscale.runtime.engine.DeepScaleEngine.deepscale_io

        def new_method(*args, **kwargs):
            kwargs["num_local_io_workers"] = 0
            return self.old_method(*args, **kwargs)

        deepscale.runtime.engine.DeepScaleEngine.deepscale_io = new_method

    def __exit__(self, *_):
        deepscale.runtime.engine.DeepScaleEngine.deepscale_io = self.old_method
