...
import torch
import deepscale
from deepscale.runtime.zero.tiling import TiledLinear, TiledLinearReturnBias
...
def test_tiled_init(in_splits, out_splits):
    ...
    for out_id in range(out_splits):
        for in_id in range(in_splits):
            ...
            assert (
                local_l.weight.size()[1] == local_in
            ), f"local[{out_id}][{in_id}].size {local_l.weight.size()}"
