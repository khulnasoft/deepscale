...
from deepscale import get_accelerator
from deepscale.moe.sharded_moe import top1gating, topkgating
from deepscale.moe.utils import split_params_into_different_moe_groups_for_optimizer, is_moe_param
...
class TestSimpleMoE(DistributedTest):
    ...
    def test(self, zero_stage):
        if not required_torch_version(min_version=1.8):
            pytest.skip("DeepScale MoE tests need torch 1.8 or higher to run correctly")
        ...
        # should automatically create moe param groups in deepscale backend
        hidden_dim = 16
        model = SimpleMoEModel(hidden_dim=hidden_dim, ep_size=1)
        model, optimizer, _, _ = deepscale.initialize(config=config_dict, model=model)
...
class TestExpertWeightGradWithZero(DistributedTest):
    ...
    def test(self, zero_stage):
        ...
        def get_state_dict_ep2(state_dict):
            ...
            """
            rank = int(deepscale.comm.get_rank())
            ep_state_dict = dict()
            dst_sub_key = f"deepscale_moe.experts.deepscale_experts.0"
            src_sub_key = f"deepscale_moe.experts.deepscale_experts.{rank}"
            ...
            for key in state_dict.keys():
                if "deepscale_moe.experts.deepscale_experts" not in key:
        ...
        def get_models(hidden_dim):
            ...
            model_ep1, _, _, _ = deepscale.initialize(config=config_dict, model=model_ep1)
            model_ep2, _, _, _ = deepscale.initialize(config=config_dict, model=model_ep2)
        ...
        def extract_expert_grad(model, expert_id):
            ...
            def _get_weight_bias(experts):
                return ([deepscale.utils.safe_get_full_grad(expert[0].weight)
                         for expert in experts][expert_id].detach().clone(),
                        [deepscale.utils.safe_get_full_grad(expert[0].bias)
                         for expert in experts][expert_id].detach().clone(),
                        [deepscale.utils.safe_get_full_grad(expert[1].weight)
                         for expert in experts][expert_id].detach().clone(),
                        [deepscale.utils.safe_get_full_grad(expert[1].bias)
                         for expert in experts][expert_id].detach().clone())

            return (*_get_weight_bias(model.moe_1.deepscale_moe.experts.deepscale_experts),
                    *_get_weight_bias(model.moe_2.deepscale_moe.experts.deepscale_experts))
