...
class MPU():
    ...
    def __init__(self, tp_world_size):
        self.rank = deepscale.comm.get_rank()
        self.world_size = deepscale.comm.get_world_size()
        self.tp_world_size = tp_world_size

        for i in range(0, self.world_size, tp_world_size):
            ranks = range(i, i + tp_world_size)
            group = deepscale.comm.new_group(ranks)
...
class TestMOETensorParallel(DistributedTest):
    ...
    def test(self, ep_size, tp_size, enable_expert_tp, use_residual):
        ...
        world_size = deepscale.comm.get_world_size()
        model = MoE(
        ...
        if enable_expert_tp:
            assert deepscale.utils.groups._get_expert_model_parallel_world_size() == tp_size
        else:
            assert deepscale.utils.groups._get_expert_model_parallel_world_size() == 1
