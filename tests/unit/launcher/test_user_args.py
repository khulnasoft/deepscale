...
def cmd(user_script_fp, prompt, multi_node):
    if multi_node:
        cmd = ("deepscale", "--force_multi", "--num_nodes", "1", "--num_gpus", "1", user_script_fp, "--prompt", prompt)
    else:
        cmd = ("deepscale", "--num_nodes", "1", "--num_gpus", "1", user_script_fp, "--prompt", prompt)
    return cmd


@pytest.mark.parametrize("prompt", [
    '''"I am 6' tall"''', """'I am 72" tall'""", """'"translate English to Romanian: "'""",
    '''I'm going to tell them "DeepScale is the best"'''
])
...
def test_bash_string_args(tmpdir, user_script_fp):
    bash_script = f"""
    ARGS="--prompt 'DeepScale is the best'"
    echo ${{ARGS}}|xargs deepscale --num_nodes 1 --num_gpus 1 {user_script_fp}
    """
