...
from deepscale.runtime.lr_schedules import (
    LR_RANGE_TEST,
...
def test_get_lr_before_train(tmpdir, scheduler_type, params):
    ...
    def _test_get_lr_before_train(args, model, hidden_dim):
        model, _, _, lr_scheduler = deepscale.initialize(
...
def test_lr_warmup_decay_schedule(tmpdir, warmup_num_steps):
    ...
    _test_lr_warmup_decay_schedule(
        args=args,
        model=model,
        hidden_dim=hidden_dim,
...
@pytest.mark.parametrize(
    "min_lr, step_rate, step_size, staircase",
    [
        (1e-4, 1e-5, 1, True),
        (1e-5, 1e-5, 1, False),
        (1e-4, 1e-3, 10, True),
...
@pytest.mark.parametrize(
    "min_lr, max_lr, decay_rate, step_size",
    [
        (1e-5, 1e-2, 1e-3, 10),
        (1e-3, 1e-1, 0, 21),
        (1e-5, 1e-2, 1e-3, 10),
...
@pytest.mark.parametrize(
    "min_mom, max_mom, decay_rate, step_size",
    [
        (0.08, 0.09, 1e-3, 10),
        (0.08, 0.09, 0, 21),
        (0.08, 0.09, 1e-3, 10),
