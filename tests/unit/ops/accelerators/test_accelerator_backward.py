...
from deepscale.ops.op_builder import TransformerBuilder
...
class DSEncoder(nn.Module):
    ...
    def __init__(self, config, weights, biases):
        ...
        self.layer = nn.ModuleList([
            copy.deepcopy(DeepScaleTransformerLayer(config, weights, biases)) for _ in range(config.num_hidden_layers)
        ])
        self.grads = []
...
class TestCUDABackward(DistributedTest):
    ...
    @pytest.mark.skipif(not deepscale.ops.__compatible_ops__[TransformerBuilder.NAME],
