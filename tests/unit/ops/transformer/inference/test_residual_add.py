...
from deepscale.ops.transformer.inference.op_binding import ResidualAddOp
...
def test_residual_add(batch, sequence, hidden_dim, dtype, mlp_after_attn, add_bias, mp_size, pre_attn_norm,
    ...
    if use_triton_ops:
        from deepscale.ops.transformer.inference.triton import residual_add_bias
        ds_out = residual_add_bias(*res_add_args)
    else:
        config = DeepScaleInferenceConfig(dtype=dtype)
