...
def test_attention(BATCH, H, N_CTX, D_HEAD, causal, use_flash, dtype=torch.float16):
    if not deepscale.HAS_TRITON:
        pytest.skip("triton has to be installed for the test")

    minus_inf = -65504.0
    dev = deepscale.accelerator.get_accelerator().device_name()
    ...
    from deepscale.ops.transformer.inference.triton.attention import _triton_attention, _triton_packed_flash
