...
from deepscale.ops.transformer import DeepScaleInferenceConfig
from deepscale.ops.transformer.inference.op_binding.bias_gelu import BiasGeluOp
...
def run_gelu_ds(activations, use_triton_ops=False):
    if use_triton_ops:
        from deepscale.ops.transformer.inference.triton import gelu
        return gelu(activations)

    device = deepscale.accelerator.get_accelerator().device_name()
    channels = activations.shape[-1]
    bias = torch.zeros((channels), dtype=activations.dtype, device=device)
    config = DeepScaleInferenceConfig(dtype=activations.dtype)
...
def test_gelu(batch, sequence, channels, dtype, use_triton_ops):
    ...
    if not deepscale.HAS_TRITON and use_triton_ops:
