# DeepScale note, some parts of code taken & adapted from commit c368a9fd1b2c9dee4cc94de9a6bb0be3d447be41
...
import torch
import deepscale
from deepscale.ops.op_builder import SparseAttnBuilder

if not deepscale.ops.__compatible_ops__[SparseAttnBuilder.NAME]:
...
def test_sparse_attention_module_availability():
    try:
        from deepscale.ops import sparse_attention
    except ImportError:
...
def test_matmul_module_availability():
    try:
        from deepscale.ops.sparse_attention import MatMul
    except ImportError:
...
def test_softmax_module_availability():
    try:
        from deepscale.ops.sparse_attention import Softmax
    except ImportError:
...
def test_sparsityconfig_module_availability():
    try:
        from deepscale.ops.sparse_attention import SparsityConfig
    except ImportError:
...
def test_densesparsityconfig_module_availability():
    try:
        from deepscale.ops.sparse_attention import DenseSparsityConfig
    except ImportError:
...
def test_fixedsparsityconfig_module_availability():
    try:
        from deepscale.ops.sparse_attention import FixedSparsityConfig
    except ImportError:
...
def test_variablesparsityconfig_module_availability():
    try:
        from deepscale.ops.sparse_attention import VariableSparsityConfig
    except ImportError:
...
def test_bigbirdsparsityconfig_module_availability():
    try:
        from deepscale.ops.sparse_attention import BigBirdSparsityConfig
    except ImportError:
...
def test_bslongformersparsityconfig_module_availability():
    try:
        from deepscale.ops.sparse_attention import BSLongformerSparsityConfig
    except ImportError:
...
def test_cpp_utils_availability():
    try:
        from deepscale.ops.sparse_attention import cpp_utils
    except ImportError:
...
def dense_to_sparse(w, mask, block):
    ...
    for zz in range(Z):
        for idx, (hh, ii, jj) in enumerate(zip(h, i, j)):
            ret[zz, idx, :, :] = w[
                zz, hh, ii * block : (ii + 1) * block, jj * block : (jj + 1) * block
            ]
    return ret
...
def _skip_on_cuda_compatability():
    ...
    if (cuda_version != 101 and cuda_version != 102) and (cuda_version != 111
                                                          and cuda_version != 110):
...
def test_softmax(block, width, dtype):
    ...
    rho = 0.4
    M = N = width
    layout, x, dx, bool_attn_mask, fp_attn_mask, kp_mask = init_softmax_inputs(
        Z, H, M, N, scale, rho, block, dtype, layout=None
    )
    ref_y, ref_dx = run_softmax_reference(
        x, scale, dx, kp_mask, bool_attn_mask, layout, block
    )
...
testdata = [(16,
             ...
             mode,
             trans_a,
             trans_b) for dtype in [torch.float16,
                                    torch.float32] for mode in ["sdd",
                                                                "dsd",
                                                                "dds"]
...
def test_matmul(block, dtype, mode, trans_a, trans_b):
    ...
    K = 192
    rho = 0.5
    x, w, dy, shape, layout = init_matmul_inputs(
        Z, H, M, N, K, rho, mode, trans_a, trans_b, block, dtype, layout=None
    )
    ref_y, ref_dx, ref_dw = run_matmul_reference(
        x.clone(), w.clone(), mode, trans_a, trans_b, layout, block, dy
    )
    st_y, st_dx, st_dw = run_matmul_sparse(
        x.clone(), w.clone(), mode, trans_a, trans_b, layout, block, dy
    )
    assert allclose(ref_y, st_y)
    assert allclose(ref_dx, st_dx)
