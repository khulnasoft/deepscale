...
from deepscale.runtime.pipe.module import PipelineModule, LayerSpec
...
def test_onebitadam_checkpointing(tmpdir):
    ...
    def _test_onebitadam_checkpointing(mask1, mask2, args, model, hidden_dim):
        model_1, optimizer_1, _, _ = deepscale.initialize(
            args=args, model=model, model_parameters=optimizer_grouped_parameters_1
        )
        data_loader = random_dataloader(
            model=model_1,
            total_samples=10,
        ...
        model_2, optimizer_2, _, _ = deepscale.initialize(
            args=args, model=model, model_parameters=optimizer_grouped_parameters_2
        )
        ...
        model_3, optimizer_3, _, _ = deepscale.initialize(
            args=args, model=model, model_parameters=optimizer_grouped_parameters_3
        )
