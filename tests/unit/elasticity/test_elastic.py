...
from deepscale.git_version_info import version as ds_version
import os
from unit.simple_model import SimpleModel
from deepscale.ops.op_builder import FusedAdamBuilder, FusedLambBuilder

if not deepscale.ops.__compatible_ops__[FusedAdamBuilder.NAME]:
...
def test_basic_10k(ds_config):
    final_batch_size, valid_gpus = deepscale.elasticity.compute_elastic_config(ds_config=ds_config,
...
def test_old_version(ds_config):
    with pytest.raises(deepscale.elasticity.config.ElasticityError):
        final_batch_size, valid_gpus = deepscale.elasticity.compute_elastic_config(ds_config=ds_config,
                                                                                   target_deepscale_version="0.2")
...
def test_valid_world_size(ds_config):
    final_batch_size, valid_gpus, mbsize = deepscale.elasticity.compute_elastic_config(
        ds_config=ds_config, target_deepscale_version=ds_version, world_size=64)
    assert mbsize == 17


def test_invalid_world_size(ds_config):
    with pytest.raises(deepscale.elasticity.config.ElasticityIncompatibleWorldSize):
        final_batch_size, valid_gpus, mbsize = deepscale.elasticity.compute_elastic_config(
            ds_config=ds_config, target_deepscale_version=ds_version, world_size=128)


def test_future_elastic_version(ds_config):
    ds_config['elasticity']['version'] = '0.3'
    with pytest.raises(deepscale.elasticity.config.ElasticityError):
        deepscale.elasticity.compute_elastic_config(ds_config=ds_config, target_deepscale_version=ds_version)
...
def test_model_parallel_v2_invalid(ds_config):
    ...
    with pytest.raises(deepscale.elasticity.config.ElasticityError):
        deepscale.elasticity.compute_elastic_config(ds_config=ds_config,
                                                    target_deepscale_version=ds_version,
                                                    world_size=16)
...
def test_proper_mbsz(ds_config):
    ...
    final_batch_size, valid_gpus, mbsize = deepscale.elasticity.compute_elastic_config(
        ds_config=ds_config, target_deepscale_version=ds_version, world_size=7)
    assert mbsize == 3
...
class TestNonElasticBatchParams(DistributedTest):
    ...
    @pytest.mark.skipif(not deepscale.ops.__compatible_ops__[FusedLambBuilder.NAME],
...
class TestElasticConfigChanged(DistributedTest):
    ...
    def test(self):
        ...
        os.environ['DEEPSCALE_ELASTICITY_CONFIG'] = json.dumps(scheduler_elastic_config)
        hidden_dim = 10
