...
from deepscale.inference.quantization.quantization import _init_group_wise_weight_quantization
from deepscale.inference.quantization.utils import Quantizer, DeQuantizer
from deepscale.inference.quantization.layers import QuantizedLinear
...
def zero3_post_init_quantization_test_helper(cpu_offload: bool, nvme_offload: bool, bits: int):
    ...
    with torch.no_grad():
        dschf = HfDeepScaleConfig(ds_config)
        ...
        ds_engine = deepscale.initialize(model=model, config_params=ds_config)[0]
