...
from deepscale.git_version_info import torch_info
from deepscale.model_implementations import DeepScaleTransformerInference
from deepscale.ops.op_builder import InferenceBuilder
from deepscale.ops.op_builder import OpBuilder
...
def query(model_w_task):
    ...
    if task == "fill-mask":
        ...
        return "DeepScale is the greatest"
...
# Used to verify DeepScale kernel injection worked with a model
def check_injection(model):

    def verify_injection(module):
        for child in module.children():
            if isinstance(child, nn.ModuleList):
                assert isinstance(child[0], DeepScaleTransformerInference),\
                    "DeepScale-Inference Transformer kernels has not been injected in the model"
                break
            else:
...
def validate_test(model_w_task, dtype, enable_cuda_graph, enable_triton):
    ...
    elif enable_triton and not deepscale.HAS_TRITON:
...
class TestModelTask(DistributedTest):
    ...
    def test(
        ...
        if pipe.tokenizer.model_max_length < deepscale.ops.transformer.inference.config.DeepScaleInferenceConfig(
        ).max_out_tokens:
            args.update({'max_out_tokens': pipe.tokenizer.model_max_length})
        pipe.model = deepscale.init_inference(pipe.model, **args)
...
class TestMPSize(DistributedTest):
    ...
    def test(
        ...
        print(local_rank, "deepscale", ds_output)
        assert assert_fn(bs_output, ds_output)
...
class TestAutoTensorParallelism(DistributedTest):
    ...
    def test(
        ...
        pipe.model = deepscale.init_inference(pipe.model, mp_size=world_size, dtype=dtype)
        ds_output = pipe(query, **inf_kwargs)
...
class TestLMCorrectness(DistributedTest):
    ...
    def test(self, model_family, model_name, task):
        ...
        ds_model = deepscale.init_inference(
