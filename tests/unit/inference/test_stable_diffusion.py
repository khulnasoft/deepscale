...
class TestStableDiffusion(DistributedTest):
    ...
    def test(self):
        ...
        generator.manual_seed(seed)
        deepscale_image = pipe(prompt, guidance_scale=7.5, generator=generator).images[0]

        rmse_value = rmse(org_img=numpy.asarray(baseline_image), pred_img=numpy.asarray(deepscale_image))
