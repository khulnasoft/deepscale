...
from deepscale.runtime.pipe.topology import *

PipeTopo = PipeDataParallelTopology

from deepscale.ops.op_builder import FusedLambBuilder, CPUAdamBuilder
...
def compare_deepscale_states(saved_model, loaded_model):
...
def compare_model_states(saved_model, loaded_model, compare_optimizer=True):
    compare_deepscale_states(saved_model, loaded_model)

    for p0, p1 in zip(saved_model.module.parameters(), loaded_model.module.parameters()):
        assert id(p0) != id(
            p1
        ), f"Comparing fp16 model state tensor against itself : {id(p0)} <====> {id(p1)}"
        assert torch.allclose(
            p0, p1, atol=1e-07
        ), f"FP16 model state {p0} is not equal to {p1}"

    if not compare_optimizer:
        return

    if FP16_DeepScaleZeroOptimizer_Stage3 is not None and isinstance(
            saved_model.optimizer,
            FP16_DeepScaleZeroOptimizer_Stage3):
        for p0, p1 in zip(
            saved_model.optimizer.fp32_partitioned_groups_flat,
            loaded_model.optimizer.fp32_partitioned_groups_flat,
        ):
            assert torch.allclose(
                p0, p1, atol=1e-07
            ), f"Fp32 model states {p0} is not equal to {p1}"

    elif isinstance(saved_model.optimizer, FP16_DeepScaleZeroOptimizer):
        for p0, p1 in zip(
            saved_model.optimizer.single_partition_of_fp32_groups,
            loaded_model.optimizer.single_partition_of_fp32_groups,
        ):
            assert id(p0) != id(
                p1
            ), f"Comparing fp32 model state tensor against itself: {id(p0)} <====> {id(p1)}"
            assert torch.allclose(
                p0, p1, atol=1e-07
            ), f"Fp32 model states {p0} is not equal to {p1}"

    elif isinstance(saved_model.optimizer, FP16_DeepScaleZeroOptimizer_Stage1):
        for partition0, partition1 in zip(
            saved_model.optimizer.local_sub_partitions_of_fp32_groups,
            loaded_model.optimizer.local_sub_partitions_of_fp32_groups,
        ):
        ...
        for p0, p1 in zip(
            saved_model.optimizer.fp32_groups_flat,
            loaded_model.optimizer.fp32_groups_flat,
        ):
            assert id(p0) != id(
                p1
            ), f"Comparing fp32 model state tensor against itself: {id(p0)} <====> {id(p1)}"
        ...
        for params0, params1 in zip(
            saved_model.optimizer.fp32_groups, loaded_model.optimizer.fp32_groups
        ):
...
def compare_optimizer_states(saved_model, loaded_model, hidden_dim, fp16=True):
    ...
    loaded_optimizer = (loaded_model.optimizer.optimizer
                        if fp16 else loaded_model.optimizer)

    for state0, state1 in zip(
        saved_optimizer.state.values(), loaded_optimizer.state.values()
    ):
        for s0, s1 in zip(state0.values(), state1.values()):
            if isinstance(s0, torch.Tensor) and isinstance(s1, torch.Tensor):
                assert id(s0) != id(
                    s1
                ), f"Comparing optimizer state tensor against itself: {id(s0)} <====> {id(s1)}"
...
def create_deepscale_model(args, model, base_optimizer):
    if base_optimizer is None:
        ds_model, _, _, _ = deepscale.initialize(
            args=args, model=model, model_parameters=model.parameters()
        )
    else:
        ds_model, _, _, _ = deepscale.initialize(
            args=args, model=model, optimizer=base_optimizer
        )
...
def checkpoint_correctness_verification(
    ...
    ds_model = create_deepscale_model(args=args,
                                      model=models[0],
    ...
    loaded_model = create_deepscale_model(args=args,
                                          model=models[1],
...
@pytest.mark.skipif(
    not deepscale.ops.__compatible_ops__[FusedLambBuilder.NAME],
...
@pytest.mark.parametrize(
    "zero_stage, use_cpu_offload, adam_optimizer",
    [
        (1,
         False,
         "Adam"),
...
def test_checkpoint_zero_optimizer(tmpdir, zero_stage, use_cpu_offload, adam_optimizer):
    if use_cpu_offload and not deepscale.ops.__compatible_ops__[CPUAdamBuilder.NAME]:
...
def test_checkpoint_zero_no_optimizer(tmpdir,
    ...
    def _test_checkpoint_zero_no_optimizer(args,
                                           ...
                                           load_optimizer_states):
        if zero_stage == 3:
            global FP16_DeepScaleZeroOptimizer_Stage3
            from deepscale.runtime.zero.stage3 import FP16_DeepScaleZeroOptimizer_Stage3
    ...
    def _test(base_topo, test_topo, save_folder):
        ...
        for idx, A_layer in enumerate(A.forward_funcs):
            ...
            for p0, p1 in zip(A_layer.parameters(), B_layer.parameters()):
                assert torch.allclose(
                    p0, p1, atol=1e-07
                ), f"Model state {p0} is not equal to {p1}"
...
def test_checkpoint_missing_latest(tmpdir):
    ...
    def _helper(args, model, hidden_dim):
        model, _, _, _ = deepscale.initialize(
            args=args, model=model, model_parameters=model.parameters()
        )
...
def test_checkpoint_unknown_tag_validation(tmpdir):
    ...
    @distributed_test(world_size=[1])
    def _helper(args, model, hidden_dim):
        with pytest.raises(deepscale.DeepScaleConfigError):
            model, _, _, _ = deepscale.initialize(
                args=args, model=model, model_parameters=model.parameters()
            )
