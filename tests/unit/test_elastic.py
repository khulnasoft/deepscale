...
def test_basic_10k():
    ...
    final_batch_size, valid_gpus = deepscale.elasticity.compute_elastic_config(
        ds_config=ds_config, target_deepscale_version=ds_version
    )

    for gpu_num in valid_gpus:
        assert (
            final_batch_size % gpu_num == 0
        ), f"Batch {final_batch_size} is not divisible by GPU count {gpu_num}"
...
def test_old_version():
    ...
    with pytest.raises(deepscale.elasticity.config.ElasticityError):
        final_batch_size, valid_gpus = deepscale.elasticity.compute_elastic_config(
            ds_config=ds_config, target_deepscale_version="0.2"
        )
...
def test_invalid_world_size():
    ...
    with pytest.raises(deepscale.elasticity.config.ElasticityIncompatibleWorldSize):
        final_batch_size, valid_gpus, mbsize = (
            deepscale.elasticity.compute_elastic_config(
