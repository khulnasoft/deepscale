...
import pytest
import deepscale
from deepscale.runtime.zero.partition_parameters import ZeroParamStatus, partitioned_param_data_shape
...
class TestZeroGatheredParametersFree(DistributedTest):
    ...
    def test(self):
        ...
        with deepscale.zero.GatheredParameters(list(model.parameters())):
...
class TestMiCSGatheredParametersFree(DistributedTest):
    ...
    def test(self):
        ...
        with deepscale.zero.MiCS_Init(config_dict_or_path=config_dict):
            model = MyModel(hidden_dim)
...
class TestSerialContext(DistributedTest):
    ...
    def test_subclass_param(self):
        setup_serial_env()
        with deepscale.zero.Init(config=config):
            model = ConvNet()
    ...
    def test_ext_param_getattr(self):
        ...
        args = SimpleNamespace(local_rank=0)
        engine, optim, _, _ = deepscale.initialize(args=args,
                                                   model=net,
                                                   model_parameters=net.parameters(),
                                                   config=config)

        with deepscale.zero.GatheredParameters(net.linear1.weight):
...
class TestScatterGather(DistributedTest):
    ...
    def test(self):
        ...
        with deepscale.zero.GatheredParameters(l.weight):
...
class TestGatherUpdate(DistributedTest):
    ...
    def test(self):
        ...
        with deepscale.zero.GatheredParameters(l.weight, modifier_rank=1):
            assert l.weight.ds_status == ZeroParamStatus.AVAILABLE
            if dist.get_rank() == 1:
                with torch.no_grad():
                    l.weight.zero_()
