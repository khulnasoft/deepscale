...
from transformers.integrations.deepscale import HfDeepScaleConfig
...
class TestNestingInit(DistributedTest):
    ...
    def test_nesting_init(self):
        ...
        with deepscale.zero.Init(config_dict_or_path=ds_config):
            with deepscale.zero.Init(config_dict_or_path=ds_config):
        ...
        deepscale_engine, *_ = deepscale.initialize(model=model, config_params=ds_config)
...
class TestShutdownInNestingInit(DistributedTest):
    ...
    def test_shutdown_in_nesting_init(self):
        ...
        with deepscale.zero.Init(config_dict_or_path=ds_config):
            ...
            deepscale_engine1, *_ = deepscale.initialize(model=model1, config_params=ds_config)
            with deepscale.zero.Init(config_dict_or_path=ds_config):
                model2 = torch.nn.Linear(4, 4)

        # ensure that zero3 processed the parameter
        assert hasattr(model2.weight, "ds_id")
        deepscale_engine2, *_ = deepscale.initialize(model=model2, config_params=ds_config)
...
class TestNestedParallelInit(DistributedTest):
    ...
    def test_nested_parallel_init(self):
        ds_config = dict(train_batch_size=1, zero_optimization=dict(stage=3))
        dschf = HfDeepScaleConfig(ds_config)  # keep this object alive
