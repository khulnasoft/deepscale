...
import deepscale
from deepscale.runtime.engine import DeepScaleEngine
...
from deepscale.runtime.zero.utils import ZeRORuntimeException
...
class TestZeroUnbalancedGradients(DistributedTest):
    ...
    def test(self, zero_stage):
        ...
        model, _, _, _ = deepscale.initialize(config=config_dict, model=model, model_parameters=model.parameters())
...
# testing the fix https://github.com/khulnasoft/DeepScale/pull/1227
...
# testing the fix https://github.com/khulnasoft/DeepScale/pull/1227
# also reproduces the https://github.com/khulnasoft/DeepScale/pull/1372
    ...
    def test_1_param_group(self, tmpdir, zero_stage, freeze_params):
        ...
        class MyModel(torch.nn.Module):
            ...
            def __init__(self, hidden_dim, n_layers, freeze_params):
                super().__init__()
                # to reproduce https://github.com/khulnasoft/DeepScale/pull/1372 it is important that
        ...
        for name, param in model.module.named_parameters():
            if zero_stage == 3:
                with deepscale.zero.GatheredParameters(param, modifier_rank=None):
                    orig_state_dict[name] = param.detach().cpu()
            else:
                orig_state_dict[name] = param.detach().cpu()

        if zero_stage == 3:
            with deepscale.zero.GatheredParameters(model.parameters(), modifier_rank=None):
...
def _ds_initialize_for_param_partitioning_testing(model: Module, cfg: dict) -> DeepScaleEngine:
    ds_engine, _, _, _ = deepscale.initialize(config=cfg, model=model, model_parameters=model.parameters())
...
class TestZero3ParamPartitioningLargeParam(DistributedTest):
    ...
    def test(self, init_context_manager: bool, reduce_scatter: bool, param_sz: int = 8100) -> None:
        ...
        with deepscale.zero.Init(mem_efficient_linear=False, enabled=init_context_manager):
...
class TestZero3ParamPartitioningManyParams(DistributedTest):
    ...
    def test(self, init_context_manager: bool, param_sz: int = 100, n_layers: int = 100) -> None:
        ...
        class ManyParamModel(Module):
            ...
            def __init__(self) -> None:
                ...
                for layer_num, module in enumerate(self.modulelist):
                    with deepscale.zero.GatheredParameters(module.weight, modifier_rank=0):
        ...
        with deepscale.zero.Init(config=ds_cfg, mem_efficient_linear=False, enabled=init_context_manager):
...
class TestZero3InitForParentWeightInitialization(DistributedTest):
    ...
    def test(self):
        ...
        with deepscale.zero.Init(config=ds_cfg, mem_efficient_linear=False, enabled=True):
...
class TestParamPartitioningSkipInit(DistributedTest):
    ...
    def test(self):
        ...
        with deepscale.zero.Init(config=config_dict):
        ...
        model, _, _, _ = deepscale.initialize(model=model, model_parameters=model.parameters(), config=config_dict)
...
class TestZeroAdamOptimizerStepCount(DistributedTest):
    ...
    def test(self, zero_stage):
        ...
        model, optimizer, _, _ = deepscale.initialize(config=config_dict,
                                                      model=model,
                                                      model_parameters=model.parameters())
        data_loader = random_dataloader(model=model, total_samples=16, hidden_dim=hidden_dim, device=model.device)
...
class TestZeroFrozenWeights(DistributedTest):
    ...
    def test(self, zero_stage):
        ...
        with deepscale.zero.Init(config_dict_or_path=config_dict, enabled=zero_stage == 3):
            model = MyModel(hidden_dim)
    ...
    def test(self, force_ds_optim):
        ...
        if force_ds_optim:
            with pytest.raises(ZeRORuntimeException):
                model, _, _, _ = deepscale.initialize(model=model, optimizer=optimizer, config=config_dict)
        else:
            model, _, _, _ = deepscale.initialize(model=model, optimizer=optimizer, config=config_dict)
...
class TestZeroPartitionCache(DistributedTest):
    ...
    def test_training_partition_cache(self, training):
        ...
        with deepscale.zero.Init(config_dict_or_path=config_dict):
            model = SimpleModel(hidden_dim, empty_grad=False)

        model, _, _, _ = deepscale.initialize(model=model, config=config_dict)
