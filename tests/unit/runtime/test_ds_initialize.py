...
from deepscale.runtime.lr_schedules import WARMUP_LR, WarmupLR
from deepscale.runtime.config import ADAM_OPTIMIZER
...
from deepscale.ops.op_builder import FusedAdamBuilder
...
class TestNoOptim(DistributedTest):
    ...
    def test(self, zero_stage):
        ...
        with deepscale.zero.Init(enabled=zero_stage == 3, config_dict_or_path=ds_config):
            model = SimpleModel(hidden_dim, nlayers=78)
        see_memory_usage('pre-init', force=True)
        model, _, _, _ = deepscale.initialize(model=model, config=ds_config)
...
class TestClientOptimizer(DistributedTest):
    ...
    def test(self, optimizer_type):
        ...
        if (optimizer_type is None) and (not deepscale.ops.__compatible_ops__[FusedAdamBuilder.NAME]):
        ...
        _, ds_optimizer, _, _ = deepscale.initialize(config=config_dict,
                                                     model=model,
...
class TestConfigOptimizer(DistributedTest):
    ...
    @pytest.mark.skipif(not deepscale.ops.__compatible_ops__[FusedAdamBuilder.NAME],
    ...
    def test(self, client_parameters):
        ...
        _, ds_optimizer, _, _ = deepscale.initialize(config=ds_config, model=model, model_parameters=model_parameters)
...
class TestOptimizerImplementation(DistributedTest):
    ...
    def test(self, optimizer_extension, model_dtype, grad_accum_dtype):
        ...
        if bf16 and not bf16_required_version_check():
            pytest.skip(
                "DeepScale BFloat16 tests need torch >= 1.10, NCCL >= 2.10.3, CUDA > =11.0 and HW support for BFloat16 to run correctly"
            )
        ...
        if key in is_supported:
            _, ds_optimizer, _, _ = deepscale.initialize(config=ds_config,
                                                         model=model,
    ...
    def test(self, scheduler_type, optimizer_type):
        ...
        if isinstance(client_scheduler, _LRScheduler) and not isinstance(client_optimizer, Optimizer):
            with pytest.raises(AssertionError):
                _, _, _, _ = deepscale.initialize(config=config_dict,
                                                  model=model,
                                                  model_parameters=list(model.parameters()),
                                                  optimizer=client_optimizer,
                                                  lr_scheduler=client_scheduler)
        else:
            _, _, _, ds_lr_scheduler = deepscale.initialize(config=config_dict,
                                                            model=model,
                                                            model_parameters=list(model.parameters()),
                                                            optimizer=client_optimizer,
                                                            lr_scheduler=client_scheduler)
...
class TestClientLrSchedulerInit(DistributedTest):
    ...
    def test_diff_lrscheler_and_callable_onecyclelr_steplr(self, scheduler_type):
        ...
        from deepscale.runtime.lr_schedules import OneCycle, ONE_CYCLE, CYCLE_MIN_LR, CYCLE_MAX_LR
