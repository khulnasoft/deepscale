...
def compare_loss(self, config, dtype):
    ...
    baseline_engine, baseline_optimizer, _, _ = deepscale.initialize(config=baseline_config,
                                                                     model=baseline_model,
                                                                     model_parameters=baseline_model.parameters())

    if config["zero_optimization"]["stage"] == 3:
        with deepscale.zero.Init(config_dict_or_path=config):
    ...
    target_engine, target_optimizer, _, _ = deepscale.initialize(config=config,
                                                                 model=target_model,
