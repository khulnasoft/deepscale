...
# A test on its own
import deepscale
from deepscale.runtime.config import DeepScaleConfig, get_bfloat16_enabled
...
class TestBasicConfig(DistributedTest):
    ...
    def test_check_version(self):
        assert hasattr(deepscale, "__git_hash__")
        assert hasattr(deepscale, "__git_branch__")
        assert hasattr(deepscale, "__version__")
        assert hasattr(deepscale, "__version_major__")
        assert hasattr(deepscale, "__version_minor__")
        assert hasattr(deepscale, "__version_patch__")
...
#Tests different batch config provided in deepscale json file
...
class TestBatchConfig(DistributedTest):
    ...
    def test(self, num_ranks, batch, micro_batch, gas, success):
        ...
        ds_config = DeepScaleConfig(ds_batch_config)
...
def test_gather_16bit_params_on_model_save(gather_weights_key):
    ...
    config = DeepScaleZeroConfig(**config_dict)
...
class TestConfigLoad(DistributedTest):
    ...
    def test_dict(self, base_config):
        ...
        model, _, _, _ = deepscale.initialize(config=base_config, model=model, model_parameters=model.parameters())
    ...
    def test_json(self, base_config, tmpdir):
        ...
        hidden_dim = 10
        model = SimpleModel(hidden_dim)
        model, _, _, _ = deepscale.initialize(config=config_path, model=model, model_parameters=model.parameters())
...
class TestDeprecatedDeepScaleConfig(DistributedTest):
    ...
    def test(self, base_config, tmpdir):
        ...
        model = SimpleModel(hidden_dim)
        model, _, _, _ = deepscale.initialize(args=args, model=model, model_parameters=model.parameters())
...
class TestDistInit(DistributedTest):
    ...
    def test(self, base_config):
        ...
        model = SimpleModel(hidden_dim)
        model, _, _, _ = deepscale.initialize(config=base_config,
                                              model=model,
...
class TestInitNoOptimizer(DistributedTest):
    ...
    def test(self, base_config):
        ...
        model, _, _, _ = deepscale.initialize(config=base_config, model=model)
        data_loader = random_dataloader(model=model, total_samples=5, hidden_dim=hidden_dim, device=model.device)
        for n, batch in enumerate(data_loader):
            loss = model(batch[0], batch[1])
...
class TestArgs(DistributedTest):
    ...
    def test_none_args(self, base_config):
        ...
        model, _, _, _ = deepscale.initialize(args=None, model=model, config=base_config)
    ...
    def test_no_args(self, base_config):
        if get_accelerator().is_fp16_supported():
            ...
            base_config["bf16"] = {"enabled": True}
        model = SimpleModel(hidden_dim=10)
        model, _, _, _ = deepscale.initialize(model=model, config=base_config)
        data_loader = random_dataloader(model=model, total_samples=5, hidden_dim=10, device=model.device)
        for n, batch in enumerate(data_loader):
            loss = model(batch[0], batch[1])
...
class TestNoModel(DistributedTest):
    ...
    def test(self, base_config):
        ...
        model = SimpleModel(hidden_dim=10)
        with pytest.raises(AssertionError):
            model, _, _, _ = deepscale.initialize(model=None, config=base_config)

        with pytest.raises(AssertionError):
            model, _, _, _ = deepscale.initialize(model, config=base_config)
