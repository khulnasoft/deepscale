...
from deepscale.ops.op_builder import CPUAdamBuilder, FusedLambBuilder
...
class TestAdamwFP16Basic(DistributedTest):
    ...
    def test(self):
        ...
        model, _, _, _ = deepscale.initialize(config=config_dict, model=model, optimizer=optimizer)
        data_loader = random_dataloader(model=model, total_samples=50, hidden_dim=hidden_dim, device=model.device)
        for n, batch in enumerate(data_loader):
            loss = model(batch[0], batch[1])
            model.backward(loss)
...
class TestFP16OptimizerForMoE(DistributedTest):
    ...
    def test_unfused_gradnorm(self, monkeypatch):
        ...
        optimizer = torch.optim.AdamW(params=model.parameters())
        engine, optimizer, _, _ = deepscale.initialize(config=config_dict,
                                                       model=model,
                                                       optimizer=optimizer,
...
class TestZeroStaticScale(DistributedTest):
    ...
    def test(self, zero_stage, use_cpu_offload, hidden_dim=4):
        ...
        model = SimpleModel(hidden_dim)
        model, optim, _, _ = deepscale.initialize(config=config_dict, model=model, model_parameters=model.parameters())
...
class TestZeroAllowUntestedOptimizer(DistributedTest):
    ...
    def test(self, zero_stage, use_cpu_offload):
        ...
        with pytest.raises(AssertionError):
            model, optim, _, _ = deepscale.initialize(config=config_dict,
                                                      model=model,
                                                      optimizer=optimizer,
                                                      model_parameters=model.parameters())
...
class TestZeroSupportedClientOptimizer(DistributedTest):
    ...
    def test(self, zero_stage, optimizer_constructor):
        ...
        model, _, _, _ = deepscale.initialize(config=config_dict, model=model, optimizer=client_optimizer)
