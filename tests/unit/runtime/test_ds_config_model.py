...
from deepscale.runtime import config as ds_config
from deepscale.runtime.config_utils import DeepScaleConfigModel


class SimpleConf(DeepScaleConfigModel):
    param_1: int = 0
...
def test_only_required_fields(tmpdir):
    ...
    run_cfg = ds_config.DeepScaleConfig(cfg_json)
...
def test_config_duplicate_key(tmpdir):
    ...
    with pytest.raises(ValueError):
        run_cfg = ds_config.DeepScaleConfig(config_path)
