...
from deepscale.checkpoint import UNIVERSAL_CHECKPOINT_INFO
from deepscale.checkpoint.ds_to_universal import main as convert_to_universal
...
def init_ds_engine(model, ds_config, use_torch_adam):
    ...
    if use_torch_adam:
        ...
        model, _, _, _ = deepscale.initialize(config=ds_config, model=model, optimizer=ds_optimizer)
    else:
