...
def compare_model_states(saved_model, loaded_model, compare_optimizer=True, load_module_only=False):
    ...
    with deepscale.zero.GatheredParameters(params_to_fetch, enabled=enable_gather):
        for p0, p1 in zip(saved_model.module.named_parameters(), loaded_model.module.named_parameters()):
            np0, p0 = p0
            np1, p1 = p1
            if 'deepscale_moe.gate.wg' in np0:
    ...
    if DeepScaleZeroOptimizer_Stage3 is not None and isinstance(saved_model.optimizer, DeepScaleZeroOptimizer_Stage3):
    ...
    elif isinstance(saved_model.optimizer, DeepScaleZeroOptimizer):
...
def create_deepscale_model(config_dict, model, base_optimizer):
    ds_model, _, _, _ = deepscale.initialize(config=config_dict,
                                             model=model,
...
def checkpoint_correctness_verification(config_dict,
    ...
    ds_model = create_deepscale_model(config_dict=config_dict, model=models[0], base_optimizer=base_optimizers[0])
    ...
    loaded_model = create_deepscale_model(config_dict=config_dict, model=models[1], base_optimizer=base_optimizers[1])
