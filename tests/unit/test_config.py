...
def test_init_no_optimizer(tmpdir):
    ...
    def _helper():
        ...
        model, _, _, _ = deepscale.initialize(args=args, model=model)
        data_loader = random_dataloader(model=model,
                                        total_samples=5,
                                        hidden_dim=hidden_dim,
                                        device=model.device)
...
def test_none_args(tmpdir):
    ...
    def _helper():
        ...
        model, _, _, _ = deepscale.initialize(args=None, model=model, config=config)
        data_loader = random_dataloader(model=model,
                                        total_samples=5,
                                        hidden_dim=10,
                                        device=model.device)
...
def test_no_args(tmpdir):
    ...
    def _helper():
        model = SimpleModel(hidden_dim=10)
        model, _, _, _ = deepscale.initialize(model=model, config=config)
        data_loader = random_dataloader(model=model,
                                        total_samples=5,
                                        hidden_dim=10,
                                        device=model.device)
...
def test_no_model(tmpdir):
    ...
    def _helper():
        model = SimpleModel(hidden_dim=10)
        with pytest.raises(AssertionError):
            model, _, _, _ = deepscale.initialize(model=None, config=config)

        with pytest.raises(AssertionError):
            model, _, _, _ = deepscale.initialize(model, config=config)
