...
def initialize(args=None,
    ...
    Arguments:
        args: an object containing local_rank and deepscale_config fields.
        ...
        config: Optional: Instead of requiring args.deepscale_config you can pass your deepscale config
    ...
    Returns:
        ...
        * ``engine``: DeepScale runtime engine which wraps the client model for distributed training.

        * ``optimizer``: Wrapped optimizer if a user defined ``optimizer`` is supplied, or if
          optimizer is specified in json config else ``None``.

        * ``training_dataloader``: DeepScale dataloader if ``training_data`` was supplied,
          otherwise ``None``.
...
def _add_core_arguments(parser):
    r"""Helper (internal) function to update an argument parser with an argument group of the core DeepScale arguments.
        The core set of DeepScale arguments include the following:
        1) --deepscale: boolean flag to enable DeepScale
        2) --deepscale_config <json file path>: path of a json configuration file to configure DeepScale runtime.
...
def add_config_arguments(parser):
    r"""Update the argument parser to enabling parsing of DeepScale command line arguments.
        The set of DeepScale arguments include the following:
        1) --deepscale: boolean flag to enable DeepScale
        2) --deepscale_config <json file path>: path of a json configuration file to configure DeepScale runtime.
