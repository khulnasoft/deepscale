...
class DeepScaleMoEConfig(DeepScaleConfigModel):
...
class BaseQuantConfig(DeepScaleConfigModel):
    enabled: bool = True
    num_bits: int = 8
...
class QKVQuantConfig(DeepScaleConfigModel):
    enabled: bool = True
...
class InferenceCheckpointConfig(DeepScaleConfigModel):
    checkpoint_dir: Optional[str] = None
    save_mp_checkpoint_path: Optional[str] = None
    base_dir: Optional[str] = None


class DeepScaleInferenceConfig(DeepScaleConfigModel):
    ...
    """
    Path to deepscale compatible checkpoint or path to JSON with load policy.
    """
    ...
    replace_method: str = Field(
        ...
        json_schema_extra={
            "deprecated": True,
            "deprecated_msg": "This parameter is no longer needed, please remove from your call to DeepScale-inference"
        })
    ...
    policy. e.g., `{BertLayer : deepscale.inference.HFBertLayerPolicy}`
    """
    ...
    def moe_backward_compat(cls, field_value, values):
        if isinstance(field_value, bool):
            return DeepScaleMoEConfig(moe=field_value)
        return field_value

    @field_validator("use_triton")
    def has_triton(cls, field_value, values):
        if field_value and not deepscale.HAS_TRITON:
            raise ValueError('Triton needs to be installed to use deepscale with triton kernels')
        return field_value
