...
from deepscale.runtime.compiler import is_compile_supported
...
from ..ops.transformer.inference.ds_attention import DeepScaleSelfAttention
...
class InferenceEngine(Module):
    ...
    def __init__(self, model, config):
        ...
        super().__init__()
        if DeepScaleTransformerInference.workspace is not None:
            self.destroy()
    ...
    def destroy(self):
        DeepScaleTransformerInference.layer_id = 0
        DeepScaleSelfAttention.num_layers = 0
        if DeepScaleTransformerInference.workspace.is_allocated():
            DeepScaleTransformerInference.workspace.release_workspace()
        DeepScaleTransformerInference.workspace = None
    ...
    def _apply_injection_policy(self, config, client_module=None):
        ...
        if isinstance(self.module, torch.nn.Module):
            # config is our DeepScaleInferenceConfig and self.config is the HF model config
    ...
    def _generate(self, *inputs, **kwargs):
        ...
        if num_beams > 1:
            raise NotImplementedError("DeepScale does not support `num_beams` > 1, if this is important to you please "
                                      "add your request to: https://github.com/khulnasoft/DeepScale/issues/2506")
        ...
        if ("input_ids" in kwargs) and (kwargs["input_ids"].dim() == 2):
            for input_tensor in kwargs["input_ids"]:
                ...
                if tensor_length > self._config.max_out_tokens:
                    raise RuntimeError(
                        f"Input with size {tensor_length} exceeds maximum length of {self._config.max_out_tokens}. Please increase `max_tokens` in the DeepScale Inference Config."
                    )
