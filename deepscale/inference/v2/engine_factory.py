    ...
    if os.path.exists(os.path.join(path, "ds_model_config.pkl")):
        ...
        if model_config.model_type == "opt":
            if not model_config.do_layer_norm_before:
                raise ValueError(
                    "Detected OPT-350m model. This model is not currently supported. If this is not the 350m model, please open an issue: https://github.com/khulnasoft/DeepScale-MII/issues"
                )
