...
def init_distributed(
    ...
    if auto_mpi_discovery and not all(map(lambda v: v in os.environ, required_env)):
        if verbose:
            logger.info(
                "Not using the DeepScale or torch.distributed launchers, attempting to detect MPI environment..."
            )
...
def mpi_discovery(distributed_port=TORCH_DISTRIBUTED_DEFAULT_PORT, verbose=True):
    ...
    if torch.distributed.is_initialized():
        ...
        ), "MPI rank {} does not match torch rank {}".format(
            rank, torch.distributed.get_rank()
        )
        assert (
            torch.distributed.get_world_size() == world_size
        ), "MPI world size {} does not match torch world size {}".format(
