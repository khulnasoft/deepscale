...
def bwc_tensor_model_parallel_rank(mpu=None):
    ...
    *Tensor* model parallelism means that tensors are physically split across
    ...
    This should "just work" with both Megatron-LM and DeepScale's pipeline
    parallelism.
    ...
    if hasattr(mpu, 'get_tensor_model_parallel_rank'):
        # New Megatron and DeepScale convention (post pipeline-parallelism release)
        return mpu.get_tensor_model_parallel_rank()
    elif hasattr(mpu, 'get_slice_parallel_rank'):
        # Some DeepScale + pipeline parallelism versions
        return mpu.get_slice_parallel_rank()
    else:
        # Deprecated Megatron and DeepScale convention
