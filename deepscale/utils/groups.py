...
"""
Support, expert, data, and model (only megatron-style) parallelism in DeepScale

Following are the possible scenarios:

Scenario 1 : There is no expert parallelism or model parallelism (D)
model = my_model(args)
engine = deepscale.init(model) ---> initialize groups without mpu

Scenario 2 : There is expert parallelism but no model parallelism (E+D)
deepscale.init_groups(args) --> groups will be initialized here
model = my_model(args)
engine = deepscale.init(model) --> don't initialize groups

Scenario 3 : There is model parallelism but no expert parallelism (M)
mpu.init()
model = my_model(args)
engine = deepscale.init(model, mpu = mpu)  --> initialize groups with mpu but expert_parallel_size = dp_world_size

Scenario 4 : There is model, data, and expert parallelism (E+D+M)
mpu.init()
deepscale.init_groups(mpu, args)  ---> initialize groups with mpu
model = my_model(args)

#Valid but assert inside deepscale to make sure mpu passed here is same as the one used to init the groups
engine = deepscale.init(model, mpu = mpu)

#Also Valid
engine = deepscale.init(model)

"""

import torch
from deepscale.utils import logger, log_dist
...
def initialize(ep_size=1, mpu=None):
    ...
    1. If called from DeepScale.initialize(), initialize groups with mp_size=1 and ep_size=1
    2. If called from an application, initialize groups with mp_size=1 and ep_size=ep_size provided by the application
    """
    if mpu is not None:
        log_dist(message="initializing deepscale groups using mpu", ranks=[0])
        initialize_model_and_expert_parallel(ep_size, mpu)
    else:
        log_dist(message="initializing deepscale groups", ranks=[0])
...
def initialize_model_parallel(model_parallel_size_):
    ...
    """
    log_dist(
        "initializing deepscale model parallel group with size {}".format(
...
def initialize_expert_parallel(expert_parallel_size_):
    ...
    log_dist(
        "initializing deepscale expert parallel group with size {}".format(
...
def initialize_model_and_expert_parallel(expert_parallel_size_, mpu):
    ...
    log_dist(
        f"Initializing deepscale groups with model parallel size {model_parallel_size_}, expert parallel size {expert_parallel_size_}, and data parallel size {world_size}",
        [0],
    )
...
def is_initialized():
    """Check if deepscale groups have been initialized."""
    if (_MODEL_PARALLEL_GROUP is None or _DATA_PARALLEL_GROUP is None
            or _EXPERT_PARALLEL_GROUP is None or _EXPERT_DATA_PARALLEL_GROUP is None):
        return False
    return True
