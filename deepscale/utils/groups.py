...
"""
 Support different forms of parallelism in DeepScale using multiple process groups.
...
from deepscale.utils.bwc import bwc_tensor_model_parallel_world_size, bwc_pipeline_parallel_world_size
from deepscale.utils.exceptions import DeprecatedException
...
def initialize(ep_size=1, mpu=None):
    ...
    raise DeprecatedException(
        "Please do not use the groups.initialize() API as it is deprecated. Instead, pass the desired ep_size to deepscale.moe.layer.MoE(..,ep_size,..)"
    )
...
def _create_expert_data_and_model_parallel(expert_parallel_size_, mpu, use_data_before_expert_parallel_=False):
    ...
    log_dist(
        f"Creating deepscale groups with model parallel size {tensor_parallel_size_}, "
...
def _clone_world_group():
    """Create a clone of the world group
        ...
        use dist.get_global_rank() utility function in DeepScale at many places.
