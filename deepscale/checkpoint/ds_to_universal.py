...
from deepscale.checkpoint import DeepScaleCheckpoint
from deepscale.checkpoint import (
...
def parse_arguments():
    ...
    parser.add_argument('--input_folder', type=str, required=True, help='Input DeepScale Checkpoint folder')
    parser.add_argument('--output_folder', type=str, required=True, help='Output DeepScale checkpoint folder')
...
def main(args):
    print(f'Convert DeepScale Checkpoint to Universal Checkpoint')

    print(f'Converting DeepScale checkpoint in {args.input_folder} to Universal checkpoint in {args.output_folder}')

    optim_files = _get_optim_files(args.input_folder)
    zero_stage = _get_zero_stage(optim_files)

    if zero_stage <= 2:
        ds_checkpoint = DeepScaleCheckpoint(args.input_folder)
