...
# DeepScale Team
"""
DeepScale runner is the main front-end to launching multi-worker
training jobs with DeepScale. By default this uses pdsh to parallel
...
DEEPSCALE_ENVIRONMENT_NAME = os.getenv("DS_ENV_FILE", ".deepscale_env")
DEEPSCALE_ENVIRONMENT_PATHS = [os.path.expanduser("~"), '.']
...
def parse_args(args=None):
    parser = argparse.ArgumentParser(description="DeepScale runner to help launch distributed "
    ...
    parser.add_argument("--save_pid",
                        ...
                        "where <main-pid> is the pid of the first process that invoked `deepscale`. "
                        "Useful when launching deepscale processes programmatically.")
    ...
    parser.add_argument("--autotuning",
                        ...
                        choices=["tune", "run"],
                        type=str,
                        help="Run DeepScale autotuner to discover optimal configuration parameters "
                        "before running job.")

    parser.add_argument("--elastic_training",
                        action="store_true",
                        help="Enable elastic training support in DeepScale.")
...
def main(args=None):
    ...
    if not resource_pool and len(visible_devices):
        ...
        if len(args.include) or len(args.exclude) or args.num_nodes > 1 or args.num_gpus > 0:
            print(
                f"{detected_str} but ignoring it because one or several of --include/--exclude/--num_gpus/--num_nodes cl args were used. If you want to use CUDA_VISIBLE_DEVICES don't pass any of these arguments to deepscale."
            )
        else:
    ...
    if not multi_node_exec:
        deepscale_launch = [
            sys.executable, "-u", "-m", "deepscale.launcher.launch", f"--world_info={world_info_base64}",
            f"--master_addr={args.master_addr}", f"--master_port={args.master_port}"
        ]
        if args.no_ssh:
            deepscale_launch.append(f"--node_rank={args.node_rank}")
        if args.no_python:
            deepscale_launch.append("--no_python")
        if args.module:
            deepscale_launch.append("--module")
        if args.no_local_rank:
            deepscale_launch.append("--no_local_rank")
        if args.save_pid:
            deepscale_launch += ["--save_pid", f"{os.getpid()}"]
        if args.enable_each_rank_log:
            deepscale_launch.append(f"--enable_each_rank_log={args.enable_each_rank_log}")
        if args.elastic_training:
            deepscale_launch.append("--enable_elastic_training")
            deepscale_launch.append(f"--max_elastic_nodes={args.max_elastic_nodes}")
            deepscale_launch.append(f"--min_elastic_nodes={args.min_elastic_nodes}")
        if args.bind_cores_to_rank:
            deepscale_launch.append("--bind_cores_to_rank")
        if args.bind_core_list is not None:
            deepscale_launch.append(f"--bind_core_list={args.bind_core_list}")
        cmd = deepscale_launch + [args.user_script] + args.user_args
    else:
        ...
        for environ_path in DEEPSCALE_ENVIRONMENT_PATHS:
            environ_file = os.path.join(environ_path, DEEPSCALE_ENVIRONMENT_NAME)
            if os.path.isfile(environ_file):
                logger.info(f"deepscale_env file = {environ_file}")
