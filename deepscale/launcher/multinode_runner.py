...
class PDSHRunner(MultiNodeRunner):
    ...
    def parse_user_args(self):
        return list(
            map(
                lambda x: x if x.startswith("-") else "'{}'".format(x),
                self.args.user_args,
            ))
    ...
    def get_cmd(self, environment, active_resources):
        ...
        return (pdsh_cmd_args + deepscale_launch + [self.user_script] +
                self.user_arguments)
...
class OpenMPIRunner(MultiNodeRunner):
    ...
    def get_cmd(self, environment, active_resources):
        ...
        assert (
            self.args.include == "" and self.args.exclude == ""
        ), "openmpi backend does not support worker include/exclusion"
        assert (
            self.args.num_nodes == -1 and self.args.num_gpus == -1
...
class MVAPICHRunner(MultiNodeRunner):
    ...
    def get_cmd(self, environment, active_resources):
        ...
        assert (
            self.args.num_nodes == -1 and self.args.num_gpus == -1
        ), "mvapich backend does not support limiting num nodes/gpus"
