...
class PDSHRunner(MultiNodeRunner):
    ...
    def get_cmd(self, environment, active_resources):
        ...
        deepscale_launch = [
            exports, f"cd {os.path.abspath('.')};", sys.executable, "-u", "-m", "deepscale.launcher.launch",
        ...
        if self.args.elastic_training:
            ...
            deepscale_launch.append(f"--max_elastic_nodes={self.args.max_elastic_nodes}")
            deepscale_launch.append(f"--min_elastic_nodes={self.args.min_elastic_nodes}")

        cmd_to_search = [i + "\\" for i in deepscale_launch[2:6]]

        kill_command = pdsh_cmd_args + ["pkill -f ", " ".join(cmd_to_search)[:-2]]
        return pdsh_cmd_args + deepscale_launch + [self.user_script] + self.user_arguments, kill_command, environment
...
class IMPIRunner(MultiNodeRunner):
    ...
    def get_cmd(self, environment, active_resources):
        ...
        # turn off IMPI core binding, use deepscale's own core binding
