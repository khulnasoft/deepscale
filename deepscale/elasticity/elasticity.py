...
from .constants import ELASTICITY, ENABLED, ENABLED_DEFAULT, LATEST_ELASTICITY_VERSION, \
    MINIMUM_DEEPSCALE_VERSION, DEEPSCALE_ELASTICITY_CONFIG
...
def _compatible_ds_version_check(target_deepscale_version: str):
    ...
    err_str = f"Target deepscale version of {target_deepscale_version} is not compatible " \
...
def ensure_immutable_elastic_config(runtime_elastic_config_dict: dict):
    ...
    if DEEPSCALE_ELASTICITY_CONFIG in os.environ:
        scheduler_elastic_config_dict = json.loads(os.environ[DEEPSCALE_ELASTICITY_CONFIG])
        ...
        logger.warning("Unable to find DEEPSCALE_ELASTICITY_CONFIG environment variable, cannot " \
            "guarantee resource scheduler will scale this job using compatible GPU counts.")


def compute_elastic_config(ds_config: dict, target_deepscale_version: str, world_size=0, return_microbatch=False):
