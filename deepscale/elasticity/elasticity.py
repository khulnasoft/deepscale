...
from .constants import (
    ...
    MINIMUM_DEEPSCALE_VERSION,
    IGNORE_NON_ELASTIC_BATCH_INFO,
    IGNORE_NON_ELASTIC_BATCH_INFO_DEFAULT,
    DEEPSCALE_ELASTICITY_CONFIG,
)
...
def _get_compatible_gpus_v01(
    ...
    assert all(
        mb <= max_acceptable_batch_size for mb in micro_batches
...
def _compatible_ds_version_check(target_deepscale_version: str):
    min_version = pkg_version.parse(MINIMUM_DEEPSCALE_VERSION)
    target_version = pkg_version.parse(target_deepscale_version)

    err_str = (
        f"Target deepscale version of {target_deepscale_version} is not compatible "
        f"with minimum version {MINIMUM_DEEPSCALE_VERSION} supporting elasticity.")
...
def ensure_immutable_elastic_config(runtime_elastic_config_dict: dict):
    ...
    """
    if DEEPSCALE_ELASTICITY_CONFIG in os.environ:
        scheduler_elastic_config_dict = json.loads(
            os.environ[DEEPSCALE_ELASTICITY_CONFIG])
        ...
        if (runtime_elastic_config.micro_batches !=
        ...
        logger.warning(
            "Unable to find DEEPSCALE_ELASTICITY_CONFIG environment variable, cannot "
            "guarantee resource scheduler will scale this job using compatible GPU counts."
        )


def compute_elastic_config(ds_config: dict, target_deepscale_version: str, world_size=0):
    """Core deepscale elasticity API. Given an elastic config (similar to the example below)
    DeepScale will compute a total train batch size corresponding valid GPU count list that
    ...
    training convergence. This is achievable primarily due to DeepScale's gradient accumulation
    ...
    Intended to be called both by scheduling infrastructure and deepscale runtime.
    For the same `ds_config` we should return deterministic results.

    Args:
        ds_config (dict): DeepScale config dictionary/json
        target_deepscale_version (str): When called from scheduling
            infrastructure we want to ensure that the target deepscale version is
    ...
    Raises:
        ...
        ElasticityError: If target deepscale version is not compatible with current version
    ...
    if not elastic_config_dict.get(ENABLED, ENABLED_DEFAULT):
        raise ElasticityConfigError(
            "Elasticity is disabled, please enable it "
    ...
    # Ensure target deepscale version works with intended elasticity version
    if not _compatible_ds_version_check(target_deepscale_version):
        raise ElasticityError("Unable to run elasticity on target deepscale version of"
                              f" {target_deepscale_version}, currently {__version__}")
    ...
    if world_size > 0:
        if world_size not in valid_gpus:
            raise ElasticityIncompatibleWorldSize(
                f"World size ({world_size}) is not valid "
