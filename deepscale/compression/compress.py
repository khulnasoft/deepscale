...
def check_deepscale_config(config):
...
def init_compression(model, deepscale_config, teacher_model=None, mpu=None):
    """
    Compress a model: replace linear/conv2d layer with deepscale compression-aware modules
    Args:
        model (`torch.nn.Module`)
            The model to compress.
        deepscale_config (`DeepScaleConfig`)
            The path of ds_config
        mpu
            The mpu module for Row/Column parallelism
    """
    compress_methods = get_compression_config(check_deepscale_config(deepscale_config))
    ...
    if compress_methods[LAYER_REDUCTION][LAYER_REDUCTION_ENABLED]:
        ...
        student_initialization(c_model, teacher_model, deepscale_config)
...
def redundancy_clean(model, deepscale_config, mpu=None):
    """
...
def student_initialization(student_model, teacher_model, deepscale_config):
    '''
    ...
    Args:
        ...
        deepscale_config (`DeepScaleConfig`)
            The path of ds_config
    '''
    config = get_compression_config(check_deepscale_config(deepscale_config))
