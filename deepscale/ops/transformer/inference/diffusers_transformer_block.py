...
from deepscale import module_inject
from .diffusers_attention import DeepScaleDiffusersAttention
...
class DeepScaleDiffusersTransformerBlock(nn.Module):

    def __init__(self, equivalent_module: nn.Module, config: Diffusers2DTransformerConfig):
        super(DeepScaleDiffusersTransformerBlock, self).__init__()
        ...
        if isinstance(self.attn_1, DeepScaleDiffusersAttention):
        ...
        # Pull the bias in if we can
        if isinstance(self.attn_2, DeepScaleDiffusersAttention):
