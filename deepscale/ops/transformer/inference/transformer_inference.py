...
class DeepScaleInferenceConfig(TransformerConfig):
    """Initialize the DeepScale Transformer Config.
    ...
    Arguments:
        ...
        encoder_decoder: DeepScale-Inference currently support the encoder-only architecture! We will add
    ...
    def __init__(
        ...
        super(DeepScaleInferenceConfig,
              self).__init__(
                  hidden_size,
...
class DeepScaleSelfAttentionFunction(Function):
    @staticmethod
    def forward(
        ctx,
        input,
        ...
        def compute_attention(qkv_out, input_mask):
            score_context_func = (inference_cuda_module.softmax_context_fp32 if
                                  (not config.fp16 or not config.triangular_masking) else
            ...
            unfused_mode = (not config.specialized_mode or mixed_query.shape[1] >= 32
                            or head_size > 128)
        ...
        def selfAttention_fp():
            vector_matmul_func = (inference_cuda_module.vector_matmul_fp16 if config.fp16
                                  else inference_cuda_module.vector_matmul_fp32)
            if not config.pre_layer_norm:
                linear_func = (inference_cuda_module.linear_layer_fp16 if config.fp16
                               else inference_cuda_module.linear_layer_fp32)

                qkv_out = linear_func(input, attn_qkvw, attn_qkvb)
            else:
                qkv_func = (inference_cuda_module.qkv_gemm_fp16
                            if config.fp16 else inference_cuda_module.qkv_gemm_fp32)
                qkv_out = qkv_func(
                    input,
                    attn_qkvw,
        ...
        if (mp_group is not None
                and torch.distributed.get_world_size(group=mp_group) > 1):
    ...
    def backward(ctx, grad_output, grad_output1, grad_output2, grad_output3):
        raise RuntimeError("You are running with DeepScale Inference mode. \
                            Please switch to Training mode for running backward!")


class DeepScaleSelfAttention(nn.Module):
    def __init__(
        self,
        config,
        mp_group=None,
        ...
        super(DeepScaleSelfAttention, self).__init__()
        self.config = config
    ...
    def forward(
        ...
        output = DeepScaleSelfAttentionFunction.apply(
            input,
            input_mask,
            head_mask,
            layer_past,
...
class DeepScaleMLPFunction(Function):
    @staticmethod
    def forward(
        ctx,
        input,
        ...
        if config.q_int8:
            ...
            mlp_gemm_func = (inference_cuda_module.mlp_gemm_fp16
                             if config.fp16 else inference_cuda_module.mlp_gemm_fp32)
            vector_matmul_func = (inference_cuda_module.vector_matmul_fp16 if config.fp16
                                  else inference_cuda_module.vector_matmul_fp32)
            (intermediate,
             residual_add) = mlp_gemm_func(
                 input,
                 residual,
                 bias,
                 inter_w,
        ...
        bias_residual_func = (inference_cuda_module.bias_residual_fp16
                              if config.fp16 or config.q_int8 else
...
class DeepScaleMLP(nn.Module):
    def __init__(
        ...
        super(DeepScaleMLP, self).__init__()
    ...
    def forward(self, input, residual, bias):
        return DeepScaleMLPFunction.apply(
            input,
            residual,
            bias,
            self.inter_w,
...
class DeepScaleTransformerInference(nn.Module):
    """Initialize the DeepScale Transformer Layer.

    Arguments:
        layer_id: The layer index starting from 0, e.g. if model has 24 transformer layers,
            layer_id will be 0,1,2...23 when each layer object is instantiated

        config: An object of DeepScaleInferenceConfig
    ...
    def __init__(
        ...
        super(DeepScaleTransformerInference, self).__init__()

        self.config = config
        self.config.layer_id = DeepScaleTransformerInference.layer_id
        DeepScaleTransformerInference.layer_id += 1

        self.attention = DeepScaleSelfAttention(config,
                                                mp_group,
                                                quantize_scales,
                                                quantize_groups,
                                                merge_count,
                                                qkv_merging)
        self.mlp = DeepScaleMLP(
            config,
            mp_group,
            quantize_scales,
            quantize_groups,
        ...
        print("DeepScale Transformer Inference config is ", self.config.__dict__)
    ...
    def forward(
        ...
        if not self.config.pre_layer_norm:
            ds_layernorm = (inference_cuda_module.layer_norm_fp16
                            if self.config.fp16 or self.config.q_int8 else
