...
from .ds_attention import DeepScaleSelfAttention
...
class DeepScaleMoEInferenceConfig(DeepScaleInferenceConfig):
    ...
    def __init__(self,
        ...
        super(DeepScaleMoEInferenceConfig,
...
class DeepScaleMoEMLP(nn.Module):
    ...
    def forward(self, input, async_op=False):
        return DeepScaleMLPFunction.apply(input, self.inter_w, self.inter_b, self.config, self.output_b, self.output_w,
                                          self.q_scales, self.q_groups, self.merge_count, self.mp_group, async_op,
                                          self.gelu_gemm_func, self.vector_matmul_func)


class DeepScaleMoEInference(nn.Module):
    """Initialize the DeepScale MoE Transformer Layer.
        Arguments:
    ...
    def __init__(self,
        ...
        super(DeepScaleMoEInference, self).__init__()

        self.config = config
        self.config.layer_id = DeepScaleMoEInference.layer_id

        assert self.config.dtype != torch.bfloat16, "DeepScale MoE Transformer Inference not yet tested for bfloat support"

        DeepScaleMoEInference.layer_id += 1
        self.attention = DeepScaleSelfAttention(self.config, mp_group, quantize_scales, quantize_groups, merge_count)
        self.attn_nw = nn.Parameter(torch.Tensor(self.config.hidden_size))
        self.attn_nb = nn.Parameter(torch.Tensor(self.config.hidden_size))
        ...
        if config.mlp_type == 'residual':
            self.res_mlp = DeepScaleMoEMLP(config, quantize_scales, quantize_groups, merge_count, mlp_extra_grouping,
                                           mp_group)
        ...
        self.mlp = nn.ModuleList(
            DeepScaleMoEMLP(config, quantize_scales, quantize_groups, merge_count, mlp_extra_grouping, expert_mp_group)
        ...
        print("DeepScale MoE Transformer Inference config is ", self.config.__dict__)
