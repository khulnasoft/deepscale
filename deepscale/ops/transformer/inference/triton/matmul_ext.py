...
import deepscale.ops.transformer.inference.triton.triton_matmul_kernel as triton_matmul_kernel
import pickle
from io import open
import deepscale
...
class TritonCacheDir:
    ...
    def warn_if_nfs(cache_dir):
        if is_nfs_path(cache_dir) and not TritonCacheDir._warning_printed:
            print(
                f"Warning: The cache directory for DeepScale Triton autotune, {cache_dir}, appears to be on an NFS system. While this is generally acceptable, if you experience slowdowns or hanging when DeepScale exits, it is recommended to set the TRITON_CACHE_DIR environment variable to a non-NFS path."
            )
