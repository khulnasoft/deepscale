    ...
    def __init__(self, config: DeepScaleInferenceConfig):
        ...
        try:
            if self.config.dtype == torch.float16:
                if deepscale.HAS_TRITON and config.use_triton:
                    from deepscale.ops.transformer.inference.triton.ops import vector_matmul_func as _triton_vector_matmul_func
