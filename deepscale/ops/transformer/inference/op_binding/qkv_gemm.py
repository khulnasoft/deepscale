...
import deepscale
from deepscale.utils.types import NormType
...
class QKVGemmOp(BaseOp):
        ...
        try:
            if self.config.norm_type == NormType.LayerNorm:
                if self.config.dtype in [torch.float16, torch.int8]:
                    if deepscale.HAS_TRITON and self.config.use_triton and self.config.dtype == torch.float16:
                        from deepscale.ops.transformer.inference.triton.ops import qkv_gemm_func as _triton_qkv_gemm_func
    ...
    def _triton_autotune(min_seqlen, max_seqlen, hidden_size, dtype=torch.float16):
        from deepscale.ops.transformer.inference.triton.matmul_ext import Fp16Matmul, matmul
        seqlen = [(min_seqlen + i)
