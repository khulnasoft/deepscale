...
class DeepScaleTransformerConfig(TransformerConfig):
    """Initialize the DeepScale Transformer Config.

    Arguments:
        batch_size: The maximum batch size used for running the kernel on each GPU

        max_seq_length: The sequence-length of the model being trained with DeepScale
    ...
    def __init__(
        ...
        super(DeepScaleTransformerConfig,
              self).__init__(
                  batch_size,
                  hidden_size,
    ...
    def from_dict(cls, json_object):
        config = DeepScaleTransformerConfig()
...
class DeepScaleTransformerFunction(Function):
    @staticmethod
    def forward(
        ctx,
        input,
        ...
        cuda_module = (stochastic_transformer_cuda_module
                       if config.stochastic_mode else transformer_cuda_module)
        forward_func = (cuda_module.forward_fp16
                        if config.fp16 else cuda_module.forward_fp32)
    ...
    def backward(ctx, grad_output):
        ...
        cuda_module = (stochastic_transformer_cuda_module
                       if ctx.config.stochastic_mode else transformer_cuda_module)
        backward_func = (cuda_module.backward_fp16
                         if ctx.config.fp16 else cuda_module.backward_fp32)
        ...
        (
            ...
            ctx.layer_norm_mean,
            (ctx.inp_norm if
             (ctx.config.pre_layer_norm and ctx.config.normalize_invertible) else input),
            input_mask,
            attn_qkvw,
            attn_qkvb,
            attn_ow,
...
class DeepScaleTransformerLayer(nn.Module):
    ...
    Arguments:
        config: An object of DeepScaleTransformerConfig
    ...
    def __init__(self, config, initial_weights=None, initial_biases=None):
        super(DeepScaleTransformerLayer, self).__init__()

        self.config = config
        self.config.layer_id = DeepScaleTransformerLayer.layer_id
        DeepScaleTransformerLayer.layer_id = DeepScaleTransformerLayer.layer_id + 1

        print("DeepScale Transformer config is ", self.config.__dict__)
        ...
        cuda_module = (stochastic_transformer_cuda_module
                       if self.config.stochastic_mode else transformer_cuda_module)
        create_layer_func = (cuda_module.create_transformer_layer_fp16
                             if self.config.fp16 else
                             cuda_module.create_transformer_layer_fp32)
    ...
    def forward(
        ...
        return DeepScaleTransformerFunction.apply(
            hidden_states,
            attention_mask,
            self,
            grads,
