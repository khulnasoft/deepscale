...
import torch
from deepscale.ops.op_builder import CPUAdagradBuilder
from deepscale.utils.logging import should_log_le


class DeepScaleCPUAdagrad(torch.optim.Optimizer):
    optimizer_id = 0
    ...
    def __init__(self, model_params, lr=1e-2, eps=1e-10, weight_decay=0, amsgrad=False, fp32_optimizer_states=True):
        ...
        super(DeepScaleCPUAdagrad, self).__init__(model_params, default_args)

        self.opt_id = DeepScaleCPUAdagrad.optimizer_id
        DeepScaleCPUAdagrad.optimizer_id = DeepScaleCPUAdagrad.optimizer_id + 1
    ...
    def __setstate__(self, state):
        super(DeepScaleCPUAdagrad, self).__setstate__(state)
