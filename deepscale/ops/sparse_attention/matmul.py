# DeepScale note, code taken & adapted from commit 9aa94789f13ada713af36cfd8cca2fc9a7f6b79a
...
class _sparse_matmul(torch.autograd.Function):
    ...
    def get_locks(size, dev):
        if dev not in _sparse_matmul.locks or size > _sparse_matmul.locks[dev].size(0):
...
class MatMul:
    ...
    def make_lut(self, dtype, device):
        ...
        # DA look-up table
        if self.mode == "sdd":
            da_lut, da_num_locks, da_width, da_packs = _sparse_matmul.make_dxx_lut(
                layout, block, step, True, device
            )
        elif self.mode == "dsd":
            da_lut, da_num_locks, da_width, da_packs = _sparse_matmul.make_sdd_lut(
                layout, block, dtype, device
            )
        elif self.mode == "dds":
            da_lut, da_num_locks, da_width, da_packs = _sparse_matmul.make_dxx_lut(
        ...
        # DB look-up table
        if self.mode == "sdd":
            db_lut, db_num_locks, db_width, db_packs = _sparse_matmul.make_dxx_lut(
            ...
            db_lut, db_num_locks, db_width, db_packs = _sparse_matmul.make_sdd_lut(
                layout, block, dtype, device
            )
        self.lut_cache[key] = (
            c_lut,
