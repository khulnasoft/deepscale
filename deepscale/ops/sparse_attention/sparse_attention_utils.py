...
from deepscale.ops.sparse_attention import BertSparseSelfAttention, SparsityConfig
"""
...
class SparseAttentionUtils:
    ...
    def extend_position_embedding(model, max_position):
        ...
        if hasattr(model, "bert"):
            original_max_position = (
                model.bert.embeddings.position_embeddings.weight.size(0))
            ...
            model.bert.embeddings.position_embeddings.weight.data = (
                model.bert.embeddings.position_embeddings.weight.repeat(
                    extend_multiples,
                    1))
        elif hasattr(model, "roberta"):
            # RoBERTa has positions 0 & 1 reserved, so embedding size is max position + 2
            original_max_position, embed_size = (
            ...
            assert max_position > original_max_position
            max_position += 2
            extended_position_embedding = (
                model.roberta.embeddings.position_embeddings.weight.new_empty(
                    max_position,
                    embed_size))
            k = 2
            for i in range(extend_multiples):
                extended_position_embedding[k:(k + original_max_position)] = (
                    model.roberta.embeddings.position_embeddings.weight[2:])
                k += original_max_position
            model.roberta.embeddings.position_embeddings.weight.data = (
                extended_position_embedding)
        else:
            raise ValueError(
    ...
    def replace_model_self_attention_with_sparse_self_attention(
        ...
        Return:
            model: updated model; in which self attention layer has been repleaced with DeepScale Sparse Self Attention layer.
        """
    ...
    def replace_self_attention_layer_with_sparse_self_attention_layer(
        ...
        Return:
            layers: updated attention layers; in which self attention layers have been repleaced with DeepScale Sparse Self Attention layer.
        """

        for layer in layers:
            deepscale_sparse_self_attn = BertSparseSelfAttention(config, sparsity_config)
            deepscale_sparse_self_attn.query = layer.attention.self.query
            deepscale_sparse_self_attn.key = layer.attention.self.key
            deepscale_sparse_self_attn.value = layer.attention.self.value

            layer.attention.self = deepscale_sparse_self_attn
