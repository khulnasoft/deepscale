...
def op_report(verbose=True):
    ...
    print("DeepScale C++/CUDA extension op report")
    print("-" * (max_dots + max_dots2 + len(h[0]) + len(h[1])))
...
def debug_report():
    ...
    report = [("torch install path", torch.__path__), ("torch version", torch.__version__),
              ("deepscale install path", deepscale.__path__),
              ("deepscale info", f"{deepscale.__version__}, {deepscale.__git_hash__}, {deepscale.__git_branch__}")]
    if get_accelerator().device_name() == 'cuda':
        ...
        report.extend([("torch cuda version", torch.version.cuda), ("torch hip version", hip_version),
                       ...
                       ("deepscale wheel compiled w.", f"torch {torch_info['version']}, " +
                        (f"hip {torch_info['hip_version']}" if hip_version else f"cuda {torch_info['cuda_version']}"))
                       ])
    elif get_accelerator().device_name() == 'npu':
        import torch_npu
        report.extend([("deepscale wheel compiled w.", f"torch {torch_info['version']}"),
    ...
    print("DeepScale general environment info:")
...
def parse_arguments():
    ...
    parser.add_argument('--hide_operator_status',
                        action='store_true',
                        help='Suppress display of installation and compatibility statuses of DeepScale operators. ')
