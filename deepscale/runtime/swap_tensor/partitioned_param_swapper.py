...
class AsyncPartitionedParameterSwapper(object):
    ...
    def _configure_aio(self, ds_config):
        ...
        self.swap_folder = os.path.join(
            self.swap_config[OFFLOAD_PARAM_NVME_PATH],
            "zero_stage_3",
            "fp16params",
        ...
        self.buffers = torch.empty(
            int(self.aligned_elements_per_buffer * self.param_buffer_count),
            dtype=torch.half,
            pin_memory=True,
            requires_grad=False,
        )
        ...
        self.aio_read_handle = self.aio_handle(
            self.aio_config[AIO_BLOCK_SIZE],
    ...
    def _get_swap_buffers(self, params):
        ...
        for param in params:
            param_id = param.ds_id
            assert (
                param_id in self.param_id_to_swap_buffer.keys()
    ...
    def _allocate_and_return_buffers_for_swap_in(self, params):
        ...
        for param in params:
            param_id = param.ds_id
            assert (
                param_id in self.param_id_to_numel.keys()
            ), f" Number of elements in param {param_id} is unknown"
            assert (
                param_id not in self.param_id_to_buffer_id.keys()
            ), f"param {param_id} already assigned swap buffer id {self.param_id_to_buffer_id[param_id]}"
            assert (
                param_id not in self.param_id_to_swap_buffer.keys()
    ...
    def synchronize_reads(self):
        ...
        for param, swap_in_buffer in zip(
            self.inflight_params, self.inflight_swap_in_buffers
        ):
            param_id = param.ds_id
    ...
    def swap_out_and_release(self, params, async_op=False, force_buffer_release=False):
        if async_op:
            ...
            ), "Should not release preallocated buffers without completing the swap out. Set force_buffer_release to True to do it anyways"
    ...
    def swap_in(self, params, async_op=True, swap_in_buffers=None):
        ...
        if swap_in_buffers is None:
            ...
            compute_buffers, swap_in_buffers = (
                self._allocate_and_return_buffers_for_swap_in(params)
            )
    ...
    def swap_into_buffer(self, param, dest_buffer):
        assert (
            param.ds_tensor.status == PartitionedParamStatus.NOT_AVAILABLE
        ), f"param {param.ds_id} is already available or inflight"
        ...
        if require_swap_buffer:
            assert (
                len(self.available_buffer_ids) > 0
            ), f"No buffer available to swap param {param.ds_id}."
            compute_buffers, swap_in_buffers = (
                self._allocate_and_return_buffers_for_swap_in([param])
            )
    ...
    def get_buffer(self, param, numel):
        ...
        assert (
            self.available_swap_in_buffers() > 0
        ), f"No swap buffers to allocate for fp16 param {param_id} of numel = {numel}"
        assert (
            numel < self.elements_per_buffer
        ), f"More elements {numel} than buffer size {self.elements_per_buffer}"
    ...
    def swap_out_partitioned_params(self, dst_fp16_params, src_fp32_params):
        ...
        ), f"partitioned swap buffers for fp16 params not initialized"
        assert (
            self.partitioned_swap_pool is not None
        ), f"partitioned swap pool for fp16 params not initialized"
        assert len(dst_fp16_params) == len(
            src_fp32_params
