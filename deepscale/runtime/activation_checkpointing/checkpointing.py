...
from deepscale.runtime.utils import copy_to_device, move_to_device, see_memory_usage
from deepscale.utils.timer import SynchronizedWallClockTimer as Timers, FORWARD_GLOBAL_TIMER
from deepscale.utils.bwc import bwc_tensor_model_parallel_rank
from deepscale.accelerator import get_accelerator
from deepscale.runtime import compiler

# DeepScale Checkpointing Enabled or Disabled
deepscale_checkpointing_enabled = False
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, all_outputs, *args):
        ...
        def save_args_for_backward(*all_args):
            ...
            ctx.deepscale_saved_tensors = tensor_args
    ...
    def backward(ctx, *grads):
        ...
        cuda_device = get_accelerator().current_device_name()
        transport_stream = get_accelerator().Stream(device=cuda_device)
        # Rebuild deepscale_saved_tensors
        for t in ctx.deepscale_saved_tensors:
        ...
        if PARTITION_ACTIVATIONS:
            ...
            inputs = gather_partitioned_activations(ctx.deepscale_saved_tensors,
                                                    device=cuda_device if CPU_CHECKPOINT else None)
            detached_inputs = detach_variable(inputs)
        elif CPU_CHECKPOINT:
            inputs = move_to_device(ctx.deepscale_saved_tensors, cuda_device, is_activation_to_checkpoint)
            detached_inputs = detach_variable(inputs)
        else:
            inputs = ctx.deepscale_saved_tensors
            detached_inputs = detach_variable(inputs)
        ...
        ctx.deepscale_saved_tensors = None
...
def non_reentrant_checkpoint(function, *args):
    ...
    * can help to solve the issue which is hacked by `deepscale.runtime.pipe.module.PipelineModule._is_checkpointable`

    Main modifications compared to the implementation of torch:
    1. adapt to the signature of `checkpoint` function in this module
    2. solve the non-deterministic by random state management consistent with deepscale `CheckpointFunction`
    ...
    deepscale_saved_tensors = None
    non_tensor_args = None
    tensor_flags = None

    def save_args_for_backward(*all_args):
        """keep this function to reduce the modification from original implementation"""
        nonlocal deepscale_saved_tensors, non_tensor_args, tensor_flags
        tensor_args, non_tensor_args, tensor_flags = extract_tensors(all_objects=all_args)
        deepscale_saved_tensors = tensor_args
    ...
    def checkpoint_unpack(holder_from_backward):
        ...
        if len(storage) == 0:
            ...
            if PARTITION_ACTIVATIONS:
                # with get_accelerator().stream(transport_stream):
                inputs = gather_partitioned_activations(deepscale_saved_tensors,
                                                        device=cuda_device if CPU_CHECKPOINT else None)
                detached_inputs = detach_variable(inputs)
            elif CPU_CHECKPOINT:
                inputs = move_to_device(deepscale_saved_tensors, cuda_device, is_activation_to_checkpoint)
                detached_inputs = detach_variable(inputs)
            else:
                inputs = deepscale_saved_tensors
                detached_inputs = detach_variable(inputs)
...
def _configure_using_config_file(config, mpu=None):
    ...
    config = DeepScaleConfig(config, mpu=mpu).activation_checkpointing_config
    if dist.get_rank() == 0:
...
def _configure_defaults():
    ...
    global mpu, num_layers, deepscale_checkpointing_enabled
    ...
    SYNCHRONIZE = False
    PROFILE_TIME = False
    deepscale_checkpointing_enabled = True
...
def configure(
    ...
    """Configure DeepScale Activation Checkpointing.
    ...
    Arguments:
        ...
        deepscale_config: Optional: DeepScale Config json file when provided will be used to
            configure DeepScale Activation Checkpointing

        partition_activations: Optional: Partitions activation checkpoint across model parallel
            GPUs when enabled. By default False. Will overwrite deepscale_config if provided
        ...
        contiguous_checkpointing: Optional: Copies activation checkpoints to a contiguous memory
            ...
            Must provide num_checkpoints. By default False. Will overwrite deepscale_config if
            provided
        ...
        num_checkpoints: Optional: Number of activation checkpoints stored during the forward
            ...
            Will overwrite deepscale_config if provided

        checkpoint_in_cpu: Optional: Moves the activation checkpoint to CPU. Only works with
            partition_activation. Default is false. Will overwrite deepscale_config if provided

        synchronize: Optional: Performs get_accelerator().synchronize() at the beginning and end of
            each call to deepscale.checkpointing.checkpoint for both forward and backward pass.
            By default false. Will overwrite deepscale_config if provided

        profile: Optional: Logs the forward and backward time for each
            deepscale.checkpointing.checkpoint invocation. Will overwrite deepscale_config
            if provided
    ...
    if deepscale_config is not None:
        _configure_using_config_file(deepscale_config, mpu=mpu)
    ...
    if CONTIGUOUS_CHECKPOINTING:
        assert PARTITION_ACTIVATIONS, "Contiguous Checkpointing is only available with partitioned activations. Set partitioned activations to true in deepscale config"
    if CONTIGUOUS_CHECKPOINTING:
...
def is_configured():
    """True if deepscale activation checkpointing has been configured
        by calling deepscale.checkpointing.configure, else returns false
    ...
    """
    return deepscale_checkpointing_enabled
