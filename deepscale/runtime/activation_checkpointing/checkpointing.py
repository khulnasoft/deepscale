...
from deepscale.runtime.utils import move_to_device, see_memory_usage
from deepscale.utils.timer import SynchronizedWallClockTimer as Timers
...
def get_partition_size(item):
    ...
    assert (
        size % mp_size == 0
    ), "Doesn't handle if partition activation if item is not divisible by mp size"
    partition_size = size / mp_size
            ...
            for i, item in enumerate(args[:-1]):
                ...
                if CONTIGUOUS_CHECKPOINTING:
                    buffer_device = (torch.device("cpu")
                                     if PA_TO_CPU else partition.device)
                    ...
                    contiguous_data_buffers[i][data_offsets[i]].data[range(
                        ...
                        int(mmap.PAGESIZE / contiguous_data_buffers[i][
                            data_offsets[i]].data.element_size()),
                    )] = 0
...
def _configure_using_config_file(config, mpu=None):
    global num_layers, PARTITION_ACTIVATIONS, CONTIGUOUS_CHECKPOINTING, PA_TO_CPU, SYNCHRONIZE, PROFILE_TIME
...
def _configure_defaults():
    ...
    global PARTITION_ACTIVATIONS, CONTIGUOUS_CHECKPOINTING, PA_TO_CPU, SYNCHRONIZE, PROFILE_TIME
...
def configure(
    ...
    if PA_TO_CPU or CONTIGUOUS_CHECKPOINTING:
        ...
        ), "CPU Checkpointing/Contiguous Checkpointing is only availble with partitioned activations. Set partitioned activations to true in deepscale config"
    if CONTIGUOUS_CHECKPOINTING:
        assert (
