...
from deepscale.runtime.checkpoint_engine.checkpoint_engine import \
    CheckpointEngine
from deepscale.utils import logger, log_dist
from deepscale.nebula.constants import *
...
class NebulaCheckpointEngine(CheckpointEngine):
    ...
    def load(self, path: str, map_location=None):
        ...
        if tag in (None, 'latest', 'latest_universal'):
            # In some cases, there is the inconsistent tag between deepscale metadata (latest file)
            # and nebula metadata, will lead to the failure on loading with deepscale tag. Then we
