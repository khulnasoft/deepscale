...
class CurriculumScheduler(object):
    def __init__(self, config):
        ...
        self.state = {}
        assert (
            "curriculum_type" in config
        ), "Curriculum learning requires the config 'curriculum_type'"
        assert (
            "min_difficulty" in config
        ), "Curriculum learning requires the config 'min_difficulty'"
        assert (
            "max_difficulty" in config
        ), "Curriculum learning requires the config 'max_difficulty'"
        assert (
            "schedule_type" in config
        ), "Curriculum learning requires the config 'schedule_type'"
        ...
        if config["schedule_type"] == "fixed_discrete":
            ...
            """
            assert (
                "difficulty" in config["schedule_config"]
            ), "Curriculum learning with fixed_discrete schedule requires the schedule_config 'difficulty'"
            assert (
                "max_step" in config["schedule_config"]
            ), "Curriculum learning with fixed_discrete schedule requires the schedule_config 'max_step'"
            ...
            """
            assert (
                "total_curriculum_step" in config["schedule_config"]
            ), "Curriculum learning with fixed_root schedule requires the schedule_config 'total_curriculum_step'"
            assert (
                "difficulty_step" in config["schedule_config"]
            ), "Curriculum learning with fixed_root schedule requires the schedule_config 'difficulty_step'"
            assert (
                "root_degree" in config["schedule_config"]
            ), "Curriculum learning with fixed_root schedule requires the schedule_config 'root_degree'"
            ...
            assert (
                "total_curriculum_step" in config["schedule_config"]
            ), "Curriculum learning with fixed_linear schedule requires the schedule_config 'total_curriculum_step'"
            assert (
                "difficulty_step" in config["schedule_config"]
            ), "Curriculum learning with fixed_linear schedule requires the schedule_config 'difficulty_step'"
            if config["schedule_config"]["difficulty_step"] % 8 != 0:
                logger.warning(
                    f"The difficulty_step for curriculum learning has to be multiple of 8 (for FP16 data) or 16 (for INT8 data) to enable NVIDIA Tensor Core acceleration. Disregard this warning if this is unrelated to your hardware."
                )
