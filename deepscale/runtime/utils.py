...
from deepscale.utils import groups, logger
from deepscale.utils.bwc import (bwc_tensor_model_parallel_rank, bwc_pipeline_parallel_world_size,
...
from deepscale.module_inject.policy import transpose
...
def get_norm_with_moe_layers_fast(all_groups_norm, group):
    ...
    #print(f"old = {all_groups_norm_old} and new = {all_groups_norm} at rank: {deepscale.comm.get_rank()}")
...
class CheckOverflow(object):
    ...
    def __init__(self, param_groups=None, mpu=None, zero_reduce_scatter=False, deepscale=None):
        self.mpu = mpu
        self.params = [] if param_groups else None
        self.zero_reduce_scatter = zero_reduce_scatter
        self.deepscale = deepscale
    ...
    def has_overflow(self, params, has_moe_params=None):
        ...
        # deepscale.comm.all_reduce(overflow_gpu,
        #                             op=deepscale.comm.ReduceOp.MAX,
        ...
        if self.zero_reduce_scatter:
            ...
            if self.deepscale is not None:
                using_pipeline = hasattr(self.deepscale, 'pipeline_enable_backward_allreduce')
                if (using_pipeline and self.deepscale.pipeline_enable_backward_allreduce is False) or (
                        not using_pipeline and self.deepscale.enable_backward_allreduce is False):
                    dist.all_reduce(overflow_gpu, op=dist.ReduceOp.MAX, group=self.mpu.get_data_parallel_group())
            dist.all_reduce(overflow_gpu, op=dist.ReduceOp.MAX, group=self.mpu.get_model_parallel_group())
        elif self.deepscale is not None and self.deepscale.enable_backward_allreduce is False:
            dist.all_reduce(overflow_gpu, op=dist.ReduceOp.MAX, group=dist.get_world_group())
...
def offload_adam_states(optimizer, device, pin_memory: bool = False, non_blocking: bool = False):
    """Move optimizer states to device. Note that this assumes the state structure of DeepScale Adam."""
