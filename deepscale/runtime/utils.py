...
from deepscale.moe.utils import is_moe_param, split_params_into_shared_and_expert_params
import os
import psutil
import gc
    ...
    def has_overflow(self, params, has_moe_params=None):
        ...
        if self.zero_reduce_scatter:
            ...
            if self.deepscale is not None:
                using_pipeline = hasattr(self.deepscale,
                                         "pipeline_enable_backward_allreduce")
                if (using_pipeline
                        and self.deepscale.pipeline_enable_backward_allreduce is False
                    ) or (not using_pipeline
                          and self.deepscale.enable_backward_allreduce is False):
                    torch.distributed.all_reduce(
                        overflow_gpu,
                        op=torch.distributed.ReduceOp.MAX,
        ...
        elif (self.deepscale is not None
              and self.deepscale.enable_backward_allreduce is False):
            torch.distributed.all_reduce(
                overflow_gpu,
                op=torch.distributed.ReduceOp.MAX,
...
def clip_grad_norm_(
        ...
        norm_type: float = 2.0,
        mpu=None,
        ignore_expert_params: bool = True,
) -> float:
