...
from deepscale.runtime.zero.offload_config import OffloadStateTypeEnum

from deepscale.utils.tensor_fragment import safe_get_local_fp32_param, safe_get_local_optimizer_state
...
def get_state_devices(model, state: OffloadStateTypeEnum) -> Set[torch.device]:
    ...
    if state == OffloadStateTypeEnum.hp_params:
        ...
        if model.optimizer._DeepScaleZeroOptimizer_Stage3__ipg_bucket_flat_buffer == None:
            return {}
        return {model.optimizer._DeepScaleZeroOptimizer_Stage3__ipg_bucket_flat_buffer.device}
