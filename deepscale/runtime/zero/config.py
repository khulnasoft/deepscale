...
class DeepScaleZeroConfig(DeepScaleConfigObject):
    def __init__(self, param_dict):
        super(DeepScaleZeroConfig, self).__init__()
    ...
    def read_zero_config_deprecated(self, param_dict):
        zero_config_dict = {}
        zero_config_dict[ZERO_OPTIMIZATION_STAGE] = (1 if param_dict[ZERO_OPTIMIZATION]
                                                     else 0)
    ...
    def _sanity_check(self, zero_config_dict):
        ...
        for old_key, new_key in deprecated_dict.items():
            if old_key in zero_config_dict:
                logger.warning(
                    f"DeepScaleConfig: {old_key} is deprecated. Please use {new_key}.")
