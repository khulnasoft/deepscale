...
from deepscale.runtime.config_utils import get_scalar_param, pp_int, DeepScaleConfigModel
from deepscale.utils import logger
from .offload_config import DeepScaleZeroOffloadParamConfig, DeepScaleZeroOffloadOptimizerConfig, OffloadDeviceEnum
...
def read_zero_config_deprecated(param_dict):
    ...
    logger.warning(
        "DeepScaleConfig: this format of ZeRO optimization setup is deprecated. Please use the following format: {}".
        format(ZERO_FORMAT))
...
def get_zero_config(param_dict):
    if ZERO_OPTIMIZATION in param_dict:
        ...
        zero_config_dict = {}
    return DeepScaleZeroConfig(**zero_config_dict)
...
class DeepScaleZeroConfig(DeepScaleConfigModel):
    """
    ...
    offload_param: Optional[DeepScaleZeroOffloadParamConfig] = None
    """
    ...
    dictionary containing values for :any:`DeepScaleZeroOffloadParamConfig`.
    """

    offload_optimizer: Optional[DeepScaleZeroOffloadOptimizerConfig] = None
    """
    ...
    for :any:`DeepScaleZeroOffloadOptimizerConfig`.
    """
    ...
    cpu_offload_param: Optional[bool] = Field(
        ...
        json_schema_extra={
            ...
            "new_param_fn": (lambda val: DeepScaleZeroOffloadParamConfig(device=OffloadDeviceEnum.cpu)
                             if val else None)
        },
    )
    ...
    cpu_offload: Optional[bool] = Field(
        ...
        json_schema_extra={
            ...
            "new_param":
            "offload_optimizer",
            "new_param_fn": (lambda val: DeepScaleZeroOffloadOptimizerConfig(device=OffloadDeviceEnum.cpu)
                             if val else None)
        },
    )
