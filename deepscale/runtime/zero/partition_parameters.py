...
from deepscale.runtime.zero.config import DeepScaleZeroConfig
from deepscale.runtime.zero.utils import assert_ints_same_as_other_ranks, is_zero_param
from deepscale.runtime.zero.offload_config import OffloadDeviceEnum
from deepscale.runtime.config_utils import get_config_default
from deepscale.utils import instrument_w_nvtx, logger
from deepscale.comm.comm import init_distributed
from deepscale.utils.debug import (debug_param2name_id_shape, debug_param2name_id_shape_device, debug_module2name,
...
from deepscale.inference.quantization.utils import _quantize_param, WEIGHT_QUANTIZATION_LAYERS, wrap_quantized_functional, wrap_load_from_state_dict
...
def register_external_parameter(module, parameter):
    """Instruct DeepScale to coordinate ``parameter``'s collection and partitioning in
    the forward and backward passes of ``module``.

    This is used when a parameter is accessed outside of its owning module's
    ``forward()``. DeepScale must know to collect it from its partitioned
        ...
        .. code-block:: python
            ...
            class ModuleZ3(torch.nn.Module):
                def __init__(self, *args):
                    ...
                    deepscale.zero.register_external_parameter(self, self.layer1.weight)
...
def shutdown_init_context():
    """
    This function is used to initialize deepscale engine inside the context of Init.
...
def restore_init_context():
    """
    This function is used to restore the wrappers after deepscale engine is initialized.
    """
    if top_level_context:
...
class CUDAQuantizer:
    ...
    def __init__(self) -> None:
        if CUDAQuantizer.quantizer_cuda_module is None:
            CUDAQuantizer.quantizer_cuda_module = deepscale.ops.op_builder.QuantizerBuilder().load()
...
class Init(InsertPostInitMethodToModuleSubClasses):
    param_id = 0
    param_persistence_threshold = get_config_default(DeepScaleZeroConfig, "param_persistence_threshold")
    model_persistence_threshold = get_config_default(DeepScaleZeroConfig, "model_persistence_threshold")
    num_persisted_parameters = 0
    num_persisted_elements = 0
    apply_param_persistence = False
    override_module_apply = get_config_default(DeepScaleZeroConfig, "override_module_apply")
    ...
    def __init__(self,
        ...
        Args:
            ...
            data_parallel_group (``deepscale.comm`` process group, optional):
            ...
            mem_efficient_linear (bool, optional): Replace
                ...
                DeepScale to partition parameters. Defaults to ``True``.
            ...
            param_swapper (``deepscale.runtime.swap_tensor.partitioned_param_swapper.AsyncPartitionedParameterSwapper``, optional): [Experimental] Use existing parameter swapper. Defaults to ``None``.
        ...
        .. note::
            Initializes ``deepscale.comm`` if it has not already been done so.
            See :meth:`deepscale.init_distributed` for more information.
        ...
        if not dist.is_initialized():
            init_distributed()
            assert dist.is_initialized(), "Parameters cannot be scattered without initializing deepscale.comm"
        ...
        self.use_all_reduce_for_fetch_params = get_config_default(DeepScaleZeroConfig,
    ...
    def _zero_init_param(self, param):
        self._convert_to_deepscale_param(param)
            ...
            if remote_device in [None, OffloadDeviceEnum.cpu]:
                if ds_config.zero_config.offload_param is not None:
                    ...
                    assert offload_param_device != OffloadDeviceEnum.nvme, \
                        f"'device' in DeepScale Config cannot be {offload_param_device} if remote device is {remote_device}."

            if remote_device == OffloadDeviceEnum.nvme:
                assert ds_config.zero_config.offload_param is not None, \
                f'"offload_param" must be defined in DeepScale Config if remote device is {OffloadDeviceEnum.nvme}.'

                assert ds_config.zero_config.offload_param.nvme_path is not None, \
                f'"nvme_path" in DeepScale Config cannot be None if remote device is {OffloadDeviceEnum.nvme}'
    ...
    def _convert_to_deepscale_param(self, param):
...
class GatheredParameters:
    ...
    def __init__(self, params, modifier_rank=None, fwd_module=None, enabled=True):
        ...
        :class:`deepscale.zero.Init` context. The parameters are partitioned
        again upon exit.
        ...
        Args:
            ...
            fwd_module (``torch.nn.Module``, optional): If specified, ``params`` will be
                registered as external parameters of ``fwd_module``. See :meth:`deepscale.zero.register_external_parameter`.
        ...
        #. Allocate a partitioned module, initialize its weight on rank 0, and update all
           processes.
            ...
            .. code-block:: python
                ...
                with deepscale.zero.GatheredParameters(linear.weight,
                                                       modifier_rank=0):
           ...
           and made available during the backward pass.
            ...
            .. code-block:: python
                ...
                def forward(self, input):
                    ...
                    # self.layer1.weight is required by self.layer2.forward
                    with deepscale.zero.GatheredParameters(self.layer1.weight,
                                                           fwd_module=self):
                        y = self.layer2(x, self.layer1.weight)
                    return y
