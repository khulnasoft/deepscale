...
from deepscale.utils import log_dist, init_distributed
...
from ..config import DeepScaleConfig
...
class InsertPostInitMethodToModuleSubClasses(object):
        ...
        def partition_after(f):
            ...
            def wrapper(module, *args, **kwargs):
                ...
                if is_child_module:
                    ...
                    print_rank_0(
                        f"Running post_init for {module.__class__.__name__}",
                        force=False,
                    )
    ...
    def _set_dtype(self, ds_config, dtype):
        if ds_config is not None and dtype is None:
            _ds_config = DeepScaleConfig(ds_config)
...
class Init(InsertPostInitMethodToModuleSubClasses):
    ...
    def __init__(
        ...
        self.remote_device = (self.local_device
                              if remote_device is None else remote_device)
        ...
        if self.remote_device == OFFLOAD_NVME_DEVICE:
            _ds_config = DeepScaleConfig(config)
        ...
        if ds_config is not None:
            ...
            if remote_device in [None, OFFLOAD_CPU_DEVICE]:
                if _ds_config.zero_config.offload_param is not None:
                    ...
                    ), f"{OFFLOAD_PARAM_DEVICE} in DeepScale Config cannot be {offload_param_device} if remote device is {remote_device}."

            if remote_device == OFFLOAD_NVME_DEVICE:
                assert (
                    _ds_config.zero_config.offload_param is not None
                ), f"{OFFLOAD_PARAM} must be defined in DeepScale Config if remote device is {OFFLOAD_NVME_DEVICE}."

                assert (
                    _ds_config.zero_config.offload_param[OFFLOAD_PARAM_NVME_PATH]
                    is not None
                ), f"{OFFLOAD_PARAM_NVME_PATH} in DeepScale Config cannot be None if remote device is {OFFLOAD_NVME_DEVICE}"
    ...
    def _ensure_availability_of_partitioned_params(self, params):
        ...
        for param in params:
            if param.ds_tensor.status == PartitionedParamStatus.NOT_AVAILABLE:
                assert (param.ds_tensor.final_location == OFFLOAD_NVME_DEVICE
    ...
    def _partition_param(self, param, buffer=None, has_been_updated=False):
        assert (
            param.ds_status is not ZeroParamStatus.INFLIGHT
        ), f" {param} Cannot parititon a param in flight"
        ...
        if param.ds_status is ZeroParamStatus.AVAILABLE:
            ...
            if param.ds_tensor is None:
                final_location = None
                if (self.remote_device == OFFLOAD_NVME_DEVICE
                        and self.param_swapper.swappable_tensor(numel=partition_size)):
    ...
    def _allgather_param(self, param, async_op=False, hierarchy=0):
        ...
        assert (
            tensor_size == aligned_param_size
        ), f"param id {param.ds_id} aligned size {aligned_param_size} does not match tensor size {tensor_size}"
    ...
    def _reduce_scatter_gradients(self, param_list):
        ...
        for param in param_list:
            assert (
                param.grad.numel() == param.ds_numel
            ), f"{param.grad.numel()} != {param.ds_numel} Cannot reduce scatter gradients whose size is not same as the params"

            handles_and_reduced_partitions.append(self._reduce_scatter_gradient(param))

        for param, (handle, reduced_partition) in zip(
            param_list, handles_and_reduced_partitions
        ):
    ...
    def _partition_gradient(self, param, partition_buffer=None, accumulate=False):
        ...
        if partition_buffer is None:
            ...
            assert (
                partition_buffer.numel() >= partition_size
            ), f"The partition buffer size {partition_buffer.numel()} should match the size of param.ds_tensor {partition_size}"
