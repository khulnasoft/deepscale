import torch
from deepscale.runtime.zero.contiguous_memory_allocator import ContiguousMemoryAllocator
...
def test1():
    ...
    # print(f"a10:{a10}")
    assert (
        a4.norm() + a7.norm() + a8.norm() + a10.norm()
    ).item() == 474.50, "Test failed"
...
def test2():
    ...
    print(f"a12:{a12}")
    assert (
        a7.norm() + a9.norm() + a10.norm() + a11.norm() + a12.norm()
    ) == 460.75, "TestFailed"
