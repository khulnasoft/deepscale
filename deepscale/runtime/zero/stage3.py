...
from deepscale.runtime.fp16.loss_scaler import LossScaler, DynamicLossScaler
from deepscale.runtime.utils import see_memory_usage, is_model_parallel_parameter
from deepscale.runtime.zero.partition_parameters import *
from deepscale.runtime.zero.partition_parameters import _init_external_params
from deepscale.runtime.zero.constants import ZERO_OPTIMIZATION_WEIGHTS
from deepscale.ops.adam import DeepScaleCPUAdam
from deepscale.ops.op_builder import UtilsBuilder
from deepscale.runtime.zero.offload_constants import *
from deepscale.runtime.swap_tensor.partitioned_param_swapper import (
    PartitionedParamStatus,
)
from deepscale.runtime.swap_tensor.partitioned_optimizer_swapper import (
    PartitionedOptimizerSwapper,
)
from deepscale.runtime.swap_tensor.pipelined_optimizer_swapper import (
...
from deepscale.utils.debug import (
...
class PrefetchCoordinator(object):
    ...
    def _distance_in_numel(self, start_step, end_step, trace):
        ...
        for step_id in range(start_step, end_step):
            ...
            for _, param in self.id_to_sub_module_map[module_id].named_parameters(
                recurse=False
            ):
...
class PartitionedParameterCoordinator(object):
    def __init__(
        ...
        self.comm_stream = (comm_stream
                            if comm_stream is not None else torch.cuda.current_stream())
    ...
    def prefetch_next_sub_modules(self, sub_module, numel=5000000, nvme=False):
        ...
        if (not self.in_flight_handles and self.total_available_parameter_numel <
                self.max_available_parameters_in_numel):
    ...
    def _all_gather(self, partitioned_params, async_op=False):
        with torch.cuda.stream(self.comm_stream):
            ...
            ) if partitioned_params else None)
...
class FP16_DeepScaleZeroOptimizer_Stage3(object):
    """
    DeepScaleZeroOptimizer designed to reduce the memory footprint
    required for training large deep learning models.

    For more details please see ZeRO: Memory Optimization Towards Training A Trillion Parameter Models
    https://arxiv.org/abs/1910.02054

    For usage examples, refer to TODO: DeepScale Tutorial
    ...
    def __init__(
        ...
        if offload_optimizer_config is not None:
            ...
            self.swap_optimizer = (offload_optimizer_config[OFFLOAD_OPTIMIZER_DEVICE] ==
                                   OFFLOAD_NVME_DEVICE)
        ...
        self.deepscale_adam_offload = (self.offload_optimizer
                                       and type(init_optimizer) == DeepScaleCPUAdam)
        ...
        if self.reduce_scatter:
            ...
            ), "allreduce_always_fp32 is not yet supported with ZeRO-2 with reduce scatter enabled"
            assert (
            ...
            ), "pre-scale gradients is not yet supported with ZeRO-2 with reduce scatter enabled"
    ...
    def _configure_tensor_swapping(self, offload_optimizer_config, aio_config):
        ...
        swapper_type = (PipelinedOptimizerSwapper
                        if offload_optimizer_config[OFFLOAD_OPTIMIZER_PIPELINE] else
                        PartitionedOptimizerSwapper)
    ...
    def _move_to_flat_buffer(self, param_list, flat_buffer, avoid_copy=False):
        ...
        for param in param_list:
            ...
            if src.status == PartitionedParamStatus.NOT_AVAILABLE:
                ...
                assert (
                    src.status == PartitionedParamStatus.AVAILABLE
                ), "Partitioned Parm must be avialable here"
                if not avoid_copy:
    ...
    def _create_fp16_partitions_with_defragmentation(self):
            ...
            for sub_group in sub_groups:
                ...
                # all partitioned parameters are in CPU during training
                    ...
                    if (not self.params_in_nvme_and_cpu or flat_offset + total_elements
                            <= self.param_groups_fp16_flat_cpu_memory[j].numel()):
                        fp16_partitioned_group_flat = (
                            self.param_groups_fp16_flat_cpu_memory[j].narrow(
                                0,
                                flat_offset,
                                total_elements))
                        print_rank_0(
                    ...
                    # these parameters reside in NVME and
                        ...
                        assert (
                            False
                        ), "Either params are in nvme, or they are in CPU memory. This code path should not be triggered. Please see you max_params_in_cpu and params_in_nvme configs"
        ...
        if create_fp16_flat_reuse_buffer:
            assert (
                len(largest_partition_numel) > 0
            ), f"Unexpected that largest partition is empty"
    ...
    def _swap_in_sub_group_to_flat_buffer(self, flat_buffer, sub_group_id):
        ...
        for param, partitioned_param in zip(
            self.fp16_groups[sub_group_id], self.fp16_partitioned_groups[sub_group_id]
        ):
    ...
    def _create_fp32_partitions(self):
        ...
        for i, tensor in enumerate(self.fp16_partitioned_groups_flat):
            ...
            self.fp32_partitioned_groups_flat[i].requires_grad = (
    ...
    def _partitioned_params_swap_out(self, i):
        ...
        swap_fp32_params = []
        for param, partitioned_param in zip(
            self.fp16_groups[i], self.fp16_partitioned_groups[i]
        ):
    ...
    def reduce_independent_p_g_buckets_and_remove_grads(self, param, i):
        ...
        if (self.elements_in_ipg_bucket > 0 and
    ...
    def average_tensor(self, tensors, params_to_reduce):
        with torch.cuda.stream(self.reduction_stream):
            ...
            # DeepScale param extensions to the pytorch parameter, so its up to
    ...
    def get_flat_partition(self,
        ...
        if current_size < partition_size:
            flat_tensor_list.append(
                torch.zeros(
                    int(partition_size - current_size),
    ...
    def _prepare_fp32_grad_for_sub_group(self, sub_group_id):
        ...
        assert (
            ...
            == self.fp32_partitioned_groups_flat[sub_group_id].numel()
    ...
    def _optimizer_states_and_gradient_swap_in(self, sub_group_id, timer_names=set()):
        ...
        assert self._swappable_optimizer_subgroup(
            sub_group_id
    ...
    def has_overflow(self, partition_gradients=True):
        if partition_gradients:
            ...
            overflow = (self.local_overflow if self.offload_optimizer else
                        self.has_overflow_partitioned_grads_serial())
    ...
    def _restore_from_fp32_weights(self, all_state_dict):
        ...
        for current, saved in zip(
            self.fp32_partitioned_groups_flat, flat_local_partition
        ):
    ...
    def _restore_from_fp16_weights(self):
        for fp16_partitions, fp32_partition in zip(
            self.fp16_partitioned_groups_flat, self.fp32_partitioned_groups_flat
        ):
    ...
    def _rigid_load_state_dict(self, state_dict, load_optimizer_states=True):
        ...
        for curr_param, saved_param in zip(
            self.fp32_partitioned_groups_flat, state_dict["fp32_flat_groups"]
        ):
    ...
    for cpu_offload in [True, False]:
        for cpu_offload_params in [True, False]:
            ...
            for zero_init in [True, False]:
                cpu_mem, gpu_mem, largest_layer_memory = (
