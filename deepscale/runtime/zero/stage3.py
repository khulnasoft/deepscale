...
from deepscale.utils import groups, z3_leaf_parameter

from torch._utils import _flatten_dense_tensors, _unflatten_dense_tensors
from deepscale.runtime.base_optimizer import ZeROOptimizer
from deepscale.utils import logger
from deepscale.runtime.fp16.loss_scaler import CreateLossScaler
from deepscale.runtime.comm.coalesced_collectives import reduce_scatter_coalesced, all_to_all_quant_reduce
from deepscale.runtime.utils import inf, is_model_parallel_parameter, get_only_unique_item
from deepscale.runtime.zero.partition_parameters import *
from deepscale.runtime.zero.config import ZeroStageEnum
from deepscale.runtime.zero.offload_config import OffloadDeviceEnum, OffloadStateTypeEnum
from deepscale.runtime.zero.parameter_offload import DeepScaleZeRoOffload
from deepscale.runtime.zero.utils import apply_to_tensors_only, get_mapping_to_flat_buffer
from deepscale.runtime.zero.offload_states import offload_adam_states, reload_adam_states
from deepscale.ops.adam import DeepScaleCPUAdam
from deepscale.runtime.swap_tensor.partitioned_param_swapper import PartitionedParamStatus
from deepscale.runtime.swap_tensor.optimizer_utils import OptimizerSwapper
from deepscale.runtime.swap_tensor.partitioned_optimizer_swapper import PartitionedOptimizerSwapper
from deepscale.runtime.swap_tensor.pipelined_optimizer_swapper import PipelinedOptimizerSwapper
from deepscale.checkpoint.constants import OPTIMIZER_STATE_DICT, FP32_FLAT_GROUPS, PARTITION_COUNT, ZERO_STAGE, LOSS_SCALER
...
def unwrap_model_for_generation(model):
    ...
    with GatheredParameters(model.parameters()):
        # Removes the optimizer hooks from a DeepScale ZeRO-3 model.
        ...
        # Adds the optimizer hooks from a DeepScale ZeRO-3 model.
        if model.optimizer is not None and hasattr(model.optimizer, "parameter_offload"):
            optimizer_offload = model.optimizer.parameter_offload
        elif model.optimizer is not None:
            optimizer_offload = model.optimizer
...
class DeepScaleZeroOptimizer_Stage3(ZeROOptimizer):
    """
    ...
    def __init__(
        ...
        if self.offload_optimizer and self.partial_offload != 1.0:
            ...
            assert type(init_optimizer) == DeepScaleCPUAdam, 'Hybrid Optimizer Only Supports DeepScaleCPUAdam'
        ...
        self.deepscale_adam_offload = (self.offload_optimizer and type(init_optimizer) == DeepScaleCPUAdam)
    ...
    def initialize_ds_offload(
        ...
        return DeepScaleZeRoOffload(module=module,
                                    timers=timers,
                                    ds_config=ds_config,
                                    overlap_comm=overlap_comm,
                                    prefetch_bucket_size=prefetch_bucket_size,
    ...
    def quantize_nontrainable_params(self):
        """ In ZeRO-3, when the zero_quantized_nontrainable_weights flag is set, we quantize the non-trainable weights and also store them in quantized format. However, this check for trainable/non-trainable is done when deepscale initializes the partitioning. So, if the user changes the trainable/non-trainable status of a parameter after the partitioning is done (e.g. LoRA), the user needs to re-quantize the non-trainable weights by calling this function.
        """
    ...
    def get_grad_norm_direct(self, gradients, params, norm_type=2):
        ...
        if norm_type == inf:
            ...
            # Sum across all model parallel GPUs.
            if len(grad_norms) == 0:
                # FIX https://github.com/khulnasoft/DeepScale/issues/3564
    ...
    def override_loss_scale(self, loss_scale):
        if loss_scale != self.external_loss_scale:
            logger.info(f'[deepscale] setting loss scale from {self.external_loss_scale} -> {loss_scale}')
...
# 3) Using extracted weights to update base optimizer weights directly.
    ...
    def offload_states(self,
        ...
        assert self.optimizer.__class__ == deepscale.ops.adam.fused_adam.FusedAdam, f"Offloading is supported only for DeepScale FusedAdam."
        ...
        if needs_offload(OffloadStateTypeEnum.contiguous_grad_buffer):
            if hasattr(self, "_DeepScaleZeroOptimizer_Stage3__ipg_bucket_flat_buffer"):
