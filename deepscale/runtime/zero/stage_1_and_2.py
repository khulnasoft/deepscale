...
from deepscale.runtime.utils import (empty_cache, see_memory_usage, inf, is_model_parallel_parameter,
...
from deepscale.runtime.constants import PIPE_REPLICATED
from deepscale.accelerator import get_accelerator

from deepscale.checkpoint.constants import (DS_VERSION, GROUP_PADDINGS, PARTITION_COUNT, LOSS_SCALER,
                                            SINGLE_PARTITION_OF_FP32_GROUPS, BASE_OPTIMIZER_STATE,
                                            BASE_OPTIMIZER_STATE_STEP, CLIP_GRAD, ZERO_STAGE, PARAM_SLICE_MAPPINGS)
from deepscale.utils import link_hp_params, lazy_init_hp_params_optimizer_state
from deepscale.checkpoint import enable_universal_checkpoint
...
class DeepScaleZeroOptimizer(ZeROOptimizer):
    """
    ...
    def __init__(self,
        ...
        self.deepscale_adam_offload = self.cpu_offload
        ...
        if self.fp16_master_weights_and_gradients:
            assert self.cpu_offload and type(self.optimizer) in [DeepScaleCPUAdam], \
            ...
            f"Currently only supported using ZeRO-Offload with DeepScaleCPUAdam. But current setting is ZeRO-Offload:{self.cpu_offload} and optimizer type {type(self.optimizer)}." \
            f"Either disable fp16_master_weights_and_gradients or enable {self.zero_stage_string} Offload with DeepScaleCPUAdam."
    ...
    def step(self, closure=None):
        ...
        for i, group in enumerate(self.bit16_groups):
            ...
            if self.cpu_offload:
                ...
                #from deepscale.ops.adam import DeepScaleCPUAdam
                #if not (type(self.optimizer) == DeepScaleCPUAdam and self.dtype == torch.half):
    ...
    def _load_global_state(self, sd):
        ...
        if not self.partition_gradients:
            ...
            error_str = f"ZeRO stage 1 changed in {required_version} and is not backwards compatible " \
                ...
                "please use an older version of DeepScale (<= 0.5.8) and set 'legacy_stage1': true in your zero config json."
