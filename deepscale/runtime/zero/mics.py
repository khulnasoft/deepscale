...
from deepscale.runtime.zero.mics_utils import (MiCS_CommGroups, create_mics_comm_groups, scale_tensors)
from deepscale.runtime.zero.parameter_offload import DeepScaleZeRoOffload
from deepscale.runtime.zero.partition_parameters import Init, AllGatherCoalescedHandle, ZeroParamStatus
from deepscale.runtime.zero.stage3 import DeepScaleZeroOptimizer_Stage3
from deepscale.utils import instrument_w_nvtx, log_dist, logger
...
class MiCS_Init(Init):
    ...
    def __init__(self,
        ...
        deepscale config json file. The context manager also introduces
        hierarchical communication method to reduce the cost of inter-node
        communications, which can be enabled with
        ``mics_hierarchical_params_gather`` field in deepscale config.
        ...
        This context follows the same logic as ``deepscale.zero.Init()``, but
        ...
        #. Allocate a model and partition it among all processes:
            ...
            .. code-block:: python
                ...
                with deepscale.zero.MiCS_Init(config_dict_or_path=ds_config):
                    model = MyLargeModel()
        ...
        #. Allocate a model in pinned CPU memory and partition it among a subgroup of processes:
            ...
            .. code-block:: python
                ...
                with deepscale.zero.MiCS_Init(data_parallel_group=mpu.get_data_parallel_group(),
                                              remote_device="cpu",
                                              pin_memory=True
        ...
        #. Partition an already-allocated model in CPU memory:
            ...
            .. code-block:: python
                ...
                model = deepscale.zero.MiCS_Init(module=model,
                                                 config_dict_or_path=ds_config)
        """

        assert config_dict_or_path is not None, "Must provide configuration for MiCS Initialization"
        _ds_config = deepscale.runtime.config.DeepScaleConfig(config_dict_or_path, mpu)
    ...
    def _convert_to_deepscale_param(self, param):
        super()._convert_to_deepscale_param(param)
...
class MiCS_Offload(DeepScaleZeRoOffload):
...
class MiCS_Optimizer(DeepScaleZeroOptimizer_Stage3):
    """
    MiCS Optimizer
    """
    ...
    def __init__(self,
        ...
        assert hasattr(first_param, "comm"), " ".join([
            ...
            "Might due to the use of deepscale.zero.Init context for initializing the weights.",
            "To use MiCS sharding, please use deepscale.zero.MiCS_Init instead for initializing parameter."
        ])
