...
from deepscale.runtime.zero.config import ZERO_OPTIMIZATION_GRADIENTS
...
from deepscale.git_version_info import version
...
class FP16_DeepScaleZeroOptimizer(object):
    """
    ...
    def __init__(
        ...
        self.deepscale_adam_offload = cpu_offload
        ...
        if self.fp16_master_weights_and_gradients:
            assert self.cpu_offload and type(self.optimizer) in [
                DeepScaleCPUAdam
            ], f"fp16_master_and_gradients requires optimizer to support keeping fp16 master and gradients while keeping the optimizer states in fp32. Currenty only supported using ZeRO-Offload with DeepScaleCPUAdam. But current setting is ZeRO-Offload:{self.cpu_offload} and optimizer type {type(self.optimizer)}. Either disable fp16_master_weights_and_gradients or enable ZeRO-2 Offload with DeepScaleCPUAdam"
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            if (partition_id == dist.get_world_size(group=self.real_dp_process_group[i])
                    - 1):
    ...
    def initialize_optimizer_states(self):
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            self.single_partition_of_fp32_groups[i].grad = (
                single_grad_partition.pin_memory()
    ...
    def independent_gradient_partition_epilogue(self):
        ...
        if self.cpu_offload is False:
            for i, _ in enumerate(self.fp16_groups):
                ...
                if (not i in self.averaged_gradients
            ...
            for i, param, param_id in self.params_in_ipg_bucket:
                ...
                if self.ipg_bucket_has_moe_params:
                    process_group = (self.expert_dp_process_group
                                     if is_moe_param(param) else self.dp_process_group)
    ...
    def set_norm_for_param_grad(self, param):
        param_id = self.get_param_id(param)
        accumulated_grad = (self.accumulated_grads_in_cpu[param_id]
                            if self.gradient_accumulation_steps > 1 else param.grad)
    ...
    def step(self, closure=None):
        ...
        if self.overflow:
            ...
            logger.info(
                "[deepscale] fp16 dynamic loss scale overflow! Rank {} Skipping step. Attempted loss scale: {}, "
        ...
        if self.deepscale_adam_offload:
            from deepscale.ops.adam import DeepScaleCPUAdam

            if type(self.optimizer) == DeepScaleCPUAdam and self.dtype == torch.half:
                fp16_param_groups = [
    ...
    def _average_expert_grad_norms(self, norm_groups):
        for i, norm in enumerate(norm_groups):
            if self.is_moe_param_group[i]:
                scaled_norm = (norm * 1.0 /
                               float(dist.get_world_size(group=self.ep_process_group)))
    ...
    def has_overflow(self, partition_gradients=True):
        if partition_gradients:
            overflow = (self.local_overflow if self.cpu_offload else
