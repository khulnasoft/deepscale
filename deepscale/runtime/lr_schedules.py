...
def override_1cycle_params(args, params):
    ...
    if (hasattr(args,
                CYCLE_SECOND_STEP_SIZE) and args.cycle_second_step_size is not None):
...
class LRRangeTest(object):
    ...
    def __init__(
        ...
        self.interval_fn = (self._staircase_interval
                            if lr_range_test_staircase else self._continous_interval)
...
class OneCycle(object):
    ...
    def _initialize_cycle(
        ...
        cycle_second_step_size = (float(cycle_second_step_size)
                                  if cycle_second_step_size is not None else
                                  cycle_first_step_size)
