...
from deepscale.runtime.base_optimizer import DeepScaleOptimizer
from deepscale.runtime.utils import get_global_norm, get_flattened_grad_norm, CheckOverflow, get_weight_norm, get_norm_with_moe_layers, is_model_parallel_parameter
from deepscale.runtime.fp16.loss_scaler import INITIAL_LOSS_SCALE, SCALE_WINDOW, MIN_LOSS_SCALE
from deepscale.utils import logger, log_dist
from deepscale.utils.torch import required_torch_version
from deepscale.checkpoint.constants import OPTIMIZER_STATE_DICT, CLIP_GRAD
from deepscale.accelerator import get_accelerator
from deepscale.moe.utils import is_moe_param_group
...
class FP16_Optimizer(DeepScaleOptimizer):
    """
   FP16 Optimizer for training fp16 models. Handles loss scaling.

   For usage example please see, TODO:  DeepScale V2 Tutorial
    """
    ...
    def __init__(self,
        ...
        self.using_pipeline = self.deepscale.pipeline_parallelism
        ...
        self.overflow = False
        self.overflow_checker = CheckOverflow(self.fp16_groups, mpu=self.mpu, deepscale=deepscale)
    ...
    def step_fused_adam(self, closure=None):
        ...
        if self.overflow:
            if self.verbose:
                logger.info("[deepscale] fp16 dynamic loss scale overflow! Skipping step. Attempted loss "
