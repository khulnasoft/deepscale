...
from deepscale.runtime.utils import get_grad_norm, CheckOverflow, get_weight_norm
from deepscale.runtime.fp16.loss_scaler import (
    INITIAL_LOSS_SCALE,
    SCALE_WINDOW,
    MIN_LOSS_SCALE,
)
from deepscale.utils import groups, logger, log_dist
...
class FP16_Optimizer(object):
    ...
    def __init__(
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            self.fp32_groups_flat[i].requires_grad = (
    ...
    def step(self, closure=None):
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            grads_groups_flat.append(
                _flatten_dense_tensors([(torch.zeros(p.size(),
                                                     dtype=data_type,
                                                     device=p.device)
                                         if p.grad is None else p.grad.to(data_type))
                                        for p in group]))
        ...
        if self.using_pipeline:
            pg = self.deepscale.mpu.get_data_parallel_group()
        else:
