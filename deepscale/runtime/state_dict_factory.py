...
class SDLoaderBase(ABC):
    ...
    def check_ckpt_list(self):
        ...
        if "mp_world_size" in sd.keys():
            assert (
                len(self.ckpt_list) == sd["mp_world_size"]
            ), f"checkpoint count {len(self.ckpt_list)} is different from saved mp_world_size {sd['mp_world_size']}"
...
class MegatronSDLoader(SDLoaderBase):
                ...
                if quantize and "attention.query_key_value.weight" in key:
                    ...
                    if quantize:
                        ...
                        new_client_sd[key] = self.merge_query_key_value(
                            value_list,
                            ckpt_ver)
            elif ("mlp.dense_h_to_4h.weight" in key or "word_embeddings.weight" in key
    ...
    def sanity_check(self, ckpt_file_name):
        ...
        for key in keys_to_check:
            assert check_key_exist(
                key, self.get_module(sd)
