...
from deepscale.runtime.utils import see_memory_usage, DummyOptim
from .zero.offload_config import OffloadDeviceEnum, OffloadStateTypeEnum
from deepscale.runtime.zero.stage_1_and_2 import DeepScaleZeroOptimizer
from deepscale.runtime.zero.partition_parameters import ZeroParamStatus
from deepscale.runtime.zero.utils import is_zero_supported_optimizer, ZeRORuntimeException
from deepscale.runtime.zero.parameter_offload import DeepScaleZeRoOffload
from deepscale.runtime.zero.config import ZERO_OPTIMIZATION

from deepscale.runtime.fp16.fused_optimizer import FP16_Optimizer
from deepscale.runtime.fp16.unfused_optimizer import FP16_UnfusedOptimizer
from deepscale.runtime.bf16_optimizer import BF16_Optimizer

from deepscale.linear.optimized_linear import LoRAOptimizedLinear

from deepscale.runtime.config import DEEPSCALE_OPTIMIZERS, \
...
from deepscale.runtime.dataloader import DeepScaleDataLoader
from deepscale.runtime.constants import \
    ROUTE_TRAIN, ROUTE_PREDICT, ROUTE_EVAL, \
    PLD_THETA, PLD_GAMMA, BFLOAT16, FP16, AMP, GRADIENT_ACCUMULATION_STEPS, \
    DATA_PARALLEL_GROUP, GLOBAL_RANK
from deepscale.runtime.zero.config import ZeroStageEnum
from deepscale.compression import compression_scheduler
from deepscale.compression.constants import \
...
from deepscale.checkpoint.constants import OPTIMIZER_STATE_DICT, FROZEN_PARAM_FRAGMENTS
from deepscale.runtime.sparse_tensor import SparseTensor

from deepscale.runtime import lr_schedules
from deepscale.utils import groups
from deepscale.utils import logger, log_dist, instrument_w_nvtx
from deepscale.utils.timer import NoopTimer, ThroughputTimer, SynchronizedWallClockTimer, \
    ...
    STEP_GLOBAL_TIMER
from deepscale.utils.debug import debug_extract_module_and_param_names, debug_clear_module_and_param_names
from deepscale.monitor.monitor import MonitorMaster
from deepscale.runtime.progressive_layer_drop import ProgressiveLayerDrop
from deepscale.runtime.utils import clip_grad_norm_
from deepscale.runtime.eigenvalue import Eigenvalue
from deepscale.runtime.data_pipeline.constants import DATA_SAMPLING, \
...
from deepscale.runtime.data_pipeline.curriculum_scheduler import CurriculumScheduler
from deepscale.runtime.data_pipeline.data_routing.scheduler import RandomLTDScheduler
from deepscale.runtime.data_pipeline.data_routing.helper import remove_random_ltd_state_dict
from deepscale.runtime.data_pipeline.data_routing.basic_layer import RandomLayerTokenDrop

from deepscale.runtime.checkpoint_engine.torch_checkpoint_engine import TorchCheckpointEngine
...
from deepscale.profiling.flops_profiler.profiler import FlopsProfiler
from deepscale.utils.logging import print_json_dist, print_configuration

from deepscale.accelerator import get_accelerator

from deepscale.runtime.config import DtypeEnum

MEMORY_OPT_ALLREDUCE_SIZE = 500000000

DeepScaleOptimizerCallable = \
    Callable[[Union[Iterable[Parameter], Dict[str, Iterable]]], Optimizer]
DeepScaleSchedulerCallable = Callable[[Optimizer], _LRScheduler]
...
class EngineTimers(object):
    r"""Wallclock timers for DeepScaleEngine"""
...
class DeepScaleEngine(Module):
    r"""DeepScale engine for training."""
    ...
    def __init__(self,
        ...
        super(DeepScaleEngine, self).__init__()
        ...
        see_memory_usage(f"DeepScale Engine: After args sanity test", force=self.memory_breakdown())
        if mpu is not None:
        ...
        see_memory_usage(
            f"DeepScale Engine: Before configure distributed model",
        ...
        see_memory_usage(f"DeepScale Engine: After configure distributed model")
        ...
        log_dist(f"DeepScale Flops Profiler Enabled: {self.flops_profiler_enabled()}", ranks=[0])

        if self.flops_profiler_enabled():
            self.flops_profiler = FlopsProfiler(self.module, self, self.flops_profiler_recompute_fwd_factor())

        if training_data:
            self.training_dataloader = self.deepscale_io(training_data)
        else:
        ...
        if not isinstance(self.optimizer, DeepScaleZeRoOffload):
        ...
        if self.global_rank == 0:
            self._config.print("DeepScaleEngine configuration")
            if self.dump_state():
                print_configuration(self, "DeepScaleEngine")
    ...
    def _get_model_parameters(self):
        if self.autotuning_profile_model_info():
            ...
            for p in self.module.parameters():
                # since user code might call deepscale.zero.Init() before deepscale.initialize(), need to check the attribute to check if the parameter is partitioned in zero 3 already or not
                n = 0
    ...
    def _configure_lr_scheduler(self):
        if self.client_lr_scheduler:
            if isinstance(self.client_lr_scheduler, Callable):
                log_dist('DeepScale using client callable to create LR scheduler', ranks=[0])
                self.lr_scheduler = self.client_lr_scheduler(self.basic_optimizer)
            else:
                log_dist('DeepScale using client LR scheduler', ranks=[0])
            ...
            log_dist(f"DeepScale using configured LR scheduler = {self.scheduler_name()}", ranks=[0])
            self.lr_scheduler = lr_scheduler

        log_dist(f'DeepScale LR Scheduler = {self.lr_scheduler}', ranks=[0])
    ...
    def _configure_checkpointing(self, dist_init_required):
        ...
        if self._config is not None and self._config.nebula_config.enabled:
            try:
                from deepscale.runtime.checkpoint_engine.nebula_checkpoint_engine import \
        ...
        if scheduler_name is not None:
            if hasattr(lr_schedules, scheduler_name):
                ...
                assert hasattr(torch.optim.lr_scheduler,
                               scheduler_name), f"DeepScale does not recognize LR scheduler {scheduler_name}"
    ...
    def _do_args_sanity_check(self, args):
        assert "LOCAL_RANK" in os.environ or "OMPI_COMM_WORLD_LOCAL_RANK" in os.environ, "DeepScale requires the LOCAL_RANK environment " \
            "variable, it is set by the deepscale launcher, deepscale.init_distributed, or the torch's launcher. If using a " \
            "different launcher please ensure LOCAL_RANK is set prior to initializing deepscale."
    ...
    def _is_supported_optimizer(self, optimizer_name):
        return (optimizer_name in DEEPSCALE_OPTIMIZERS or getattr(torch.optim, optimizer_name, None) is not None)
        ...
        if not self.client_optimizer:
            if self.optimizer_name() is not None:
                assert self._is_supported_optimizer(
                    self.optimizer_name()), "{} is not a supported DeepScale Optimizer".format(self.optimizer_name())

        if (self.optimizer_name() == LAMB_OPTIMIZER or self.optimizer_name() == ONEBIT_LAMB_OPTIMIZER):
            assert (self.dynamic_loss_scale()), "DeepScale {} optimizer requires dynamic loss scaling".format(
                self.optimizer_name())
    ...
    def _configure_distributed_model(self, model):
        ...
        # Set deepscale parallelism spec. for the model including expert parallelism
        for _, module in self.module.named_modules():
            if hasattr(module, 'set_deepscale_parallelism'):
                module.set_deepscale_parallelism(self._config.use_data_before_expert_parallel_)
    ...
    def _configure_optimizer(self, client_optimizer, model_parameters):
        if client_optimizer is None:
            ...
            log_dist(f"Using DeepScale Optimizer param name {self.optimizer_name()} as basic optimizer", ranks=[0])
        else:
            ...
            if self.zero_use_cpu_optimizer() and not isinstance(basic_optimizer, deepscale.ops.adam.DeepScaleCPUAdam):
                if self.zero_force_ds_cpu_optimizer():
                    msg = f'You are using ZeRO-Offload with a client provided optimizer ({type(basic_optimizer)}) which in most cases will yield poor performance. Please either use deepscale.ops.adam.DeepScaleCPUAdam or set an optimizer in your ds-config (https://www.deepscale.ai/docs/config-json/#optimizer-parameters). If you really want to use a custom optimizer w. ZeRO-Offload and understand the performance impacts you can also set <"zero_force_ds_cpu_optimizer": false> in your configuration file.'
        ...
        log_dist("DeepScale Basic Optimizer = {}".format(basic_optimizer.__class__.__name__), ranks=[0])
        ...
        log_dist("DeepScale Final Optimizer = {}".format(self.optimizer.__class__.__name__), ranks=[0])
    ...
    def _configure_basic_optimizer(self, model_parameters):
        ...
        if "max_grad_norm" in optimizer_parameters.keys():
            raise ValueError(
                "'max_grad_norm' is not supported as an optimizer parameter, please switch to using the deepscale parameter 'gradient_clipping' see: https://www.deepscale.ai/docs/config-json/#gradient-clipping for more details"
            )
        ...
        if self.optimizer_name() in [ADAM_OPTIMIZER, ADAMW_OPTIMIZER]:
            ...
            if torch_adam:
                ...
                if self.zero_use_cpu_optimizer():
                    ...
                    optimizer = DeepScaleCPUAdam(model_parameters,
                                                 **optimizer_parameters,
                                                 adamw_mode=effective_adam_w_mode)
                else:
                    from deepscale.ops.adam import FusedAdam
            ...
            if self.zero_use_cpu_optimizer():
                from deepscale.ops.adagrad import DeepScaleCPUAdagrad
                optimizer = DeepScaleCPUAdagrad(model_parameters, **optimizer_parameters)
            else:
                optimizer = torch.optim.Adagrad(model_parameters, **optimizer_parameters)
        elif self.optimizer_name() == LAMB_OPTIMIZER:
            from deepscale.ops.lamb import FusedLamb

            optimizer = FusedLamb(model_parameters, **optimizer_parameters)
        elif self.optimizer_name() == ONEBIT_ADAM_OPTIMIZER:
            assert not self.zero_optimization(), "1bit-Adam is not compatible with ZeRO"
            from deepscale.runtime.fp16.onebit.adam import OnebitAdam
            ...
            from deepscale.runtime.fp16.onebit.zoadam import ZeroOneAdam
            ...
            from deepscale.runtime.fp16.onebit.lamb import OnebitLamb
            ...
            if self.zero_use_cpu_optimizer():
                ...
                optimizer = DeepScaleCPULion(model_parameters, **optimizer_parameters)
            else:
    ...
    def _configure_quantization(self):
        ...
        quantizer = None
        if quantize_enabled and not quantize_weight_in_forward:
            from deepscale.runtime.quantize import Quantizer
    ...
    def _configure_zero_optimizer(self, optimizer):
        ...
        if self.zero_legacy_stage1():
            raise Exception(
                "The deprecated version of ZeRO Stage 1 is not supported in deepscale >= 0.5.9. Please downgrade to a version less than 0.5.9 if you need to use this deprecated version of ZeRO."
            )
        ...
        if zero_stage <= ZeroStageEnum.gradients:
            ...
            if isinstance(self.module, PipelineModule):
                if overlap_comm:
                    ...
                    overlap_comm = False
            optimizer = DeepScaleZeroOptimizer(
                optimizer,
                self.param_names,
                timers=timers,
                static_loss_scale=self.loss_scale(),
            ...
            if isinstance(optimizer, DummyOptim):
                ...
                if self.zero_hpz_partition_size() > 1 and zero_param_parallel_group is None:
                    ...
                    zero_param_parallel_group = groups._get_zero_param_intra_parallel_group()
                optimizer = DeepScaleZeRoOffload(
                    self.module,
                    timers=timers,
                ...
                log_dist(f'Creating {model_dtype} ZeRO stage {zero_stage} optimizer', ranks=[0])
                from deepscale.runtime.zero.stage3 import DeepScaleZeroOptimizer_Stage3
                optimizer = DeepScaleZeroOptimizer_Stage3(
                    self.module,
                    optimizer,
                    timers=timers,
                    ds_config=self.config,
    ...
    def _return_mics_optimizer(self, basic_optimizer, timers):
        from deepscale.runtime.zero.mics import MiCS_Optimizer
        model_dtype, gradient_accumulation_dtype = self.get_data_types()
    ...
    def deepscale_io(self,
        ...
        deepscale_io_timer = None
        if route == ROUTE_TRAIN:
            deepscale_io_timer = self.tput_timer
        ...
        return DeepScaleDataLoader(dataset=dataset,
                                   batch_size=batch_size,
                                   pin_memory=pin_memory,
                                   collate_fn=collate_fn,
                                   local_rank=self.local_rank,
                                   tput_timer=deepscale_io_timer,
                                   num_local_io_workers=num_local_io_workers,
                                   data_sampler=data_sampler,
                                   data_parallel_world_size=data_parallel_world_size,
                                   data_parallel_rank=data_parallel_rank,
                                   dataloader_drop_last=self.dataloader_drop_last(),
                                   deepscale_dataloader_config=deepscale_dataloader_config)
    ...
    def _scale_loss_by_gas(self, prescaled_loss, eval_micro_batches=None):
        ...
        if isinstance(prescaled_loss, torch.Tensor):
            ...
            if self.warn_unscaled_loss:
                logger.warning(f"DeepScale unable to scale loss because of type: {type(prescaled_loss)}")
    ...
    def set_gradient_accumulation_boundary(self, is_boundary):
        """
        Manually overrides the DeepScale engine's gradient accumulation boundary state, this is an optional
        ...
        if overflow:
            ...
            if self.lr_scheduler is not None:
                try:
                    ...
                    # XXX Hack to work with Megatron 2.0 and DeepScale pipelines.
        ...
        if old_moe_load:
            ...
            for local_expert_id in range(num_local_experts):
                ...
                expert_state_dict = checkpoint_engine.load(
                    DeepScaleEngine._get_expert_ckpt_name(
                        checkpoint_path,
                ...
                moe_str_prefix = '.deepscale_moe.experts.deepscale_experts.'
            ...
            for n_module, module in model.named_modules():
                if isinstance(module, MoE):  # and deepscale.comm.get_rank() == 0:
                    ...
                    for local_expert_id in range(num_local_experts):
                        global_expert_id = expp_rank * num_local_experts + local_expert_id
                        expert_state_dict = checkpoint_engine.load(DeepScaleEngine._get_expert_ckpt_name(
    ...
    def load_module_state_dict(self, checkpoint, strict=True, custom_load_fn=None, fetch_z3_params=False):
        ...
        with deepscale.zero.GatheredParameters(params_to_fetch, modifier_rank=0):
    ...
    def _load_checkpoint(self,
        ...
        from deepscale.runtime.state_dict_factory import SDLoaderFactory
        ...
        if self.has_moe_layers:
            ...
            if not isinstance(checkpoint['num_experts'], list):
                old_moe_load = True
            DeepScaleEngine.load_moe_state_dict(load_dir,
                                                tag,
        ...
        if load_lr_scheduler_states:
            deepscale_states.append('lr_scheduler')
        if load_optimizer_states:
            deepscale_states.append('optimizer')

        client_state = {key: value for key, value in checkpoint.items() if not key in deepscale_states}
    ...
    def _save_moe_checkpoint(self, save_dir, tag, client_state={}, exclude_frozen_parameters=False):
        ...
        if groups._get_data_parallel_rank() == 0:
            ...
            # Classes DeepScaleEngine and PipelineEngine have different behavior for method module_state_dict.
            # DeepScaleEngine returns the state dict, where PipelineEngine saves the state dict and returns None.
            # We need to get the state dict, therefore, call to DeepScaleEngine (base class for PipelineEngine)
            model_state_dict = self._get_non_moe_state_dict(
                DeepScaleEngine.module_state_dict(self, exclude_frozen_parameters=exclude_frozen_parameters))
    ...
    def _zero3_consolidated_16bit_state_dict(self, exclude_frozen_parameters=False):
        ...
        def get_layer_state_dict(module, prefix=""):
            ...
            with deepscale.zero.GatheredParameters(list(module.parameters(recurse=False)), modifier_rank=0):
                if dist.get_rank() == 0:
                    # handle params
                    for name, param in module.named_parameters(recurse=False):
    ...
    def compile(self, backend=get_accelerator().get_compile_backend(), compile_kwargs={}) -> None:
        ...
        """
        # Avoid graph breaks
        deepscale.utils.nvtx.enable_nvtx = False
