...
from deepscale.runtime.zero.stage2 import FP16_DeepScaleZeroOptimizer
from deepscale.runtime.zero.stage1 import FP16_DeepScaleZeroOptimizer_Stage1
from deepscale.runtime.zero.partition_parameters import ZeroParamStatus
from deepscale.runtime.zero.utils import (
    is_zero_supported_optimizer,
    _initialize_parameter_parallel_groups,
)
from deepscale.runtime.activation_checkpointing import (
...
from deepscale.runtime.config import (
    DeepScaleConfig,
    DEEPSCALE_OPTIMIZERS,
    ADAM_OPTIMIZER,
    ADAMW_OPTIMIZER,
    LAMB_OPTIMIZER,
...
from deepscale.runtime.zero.constants import (
    ZERO_OPTIMIZATION_OPTIMIZER_STATES,
    ZERO_OPTIMIZATION_GRADIENTS,
    ZERO_OPTIMIZATION_WEIGHTS,
)
from deepscale.runtime.csr_tensor import CSRTensor
import deepscale.runtime.lr_schedules as lr_schedules
import deepscale.utils.groups as groups
from deepscale.utils import logger, log_dist, init_distributed
from deepscale.utils.timer import ThroughputTimer, SynchronizedWallClockTimer
from deepscale.utils.debug import debug_extract_module_and_param_names
...
from ..ops.adam import DeepScaleCPUAdam
...
class DeepScaleEngine(Module):
    ...
    def __init__(
        ...
        if dist_init_required is False:
            ...
            ), "Torch distributed not initialized. Please set dist_init_required to True or initialize before calling deepscale.initialize()"
        else:
            # Initialize torch distributed if needed
            init_distributed(dist_backend=self.dist_backend)

        see_memory_usage(f"DeepScale Engine: Before args sanity test")
        ...
        see_memory_usage(f"DeepScale Engine: Before configure distributed model")
        ...
        if dist.get_rank() == 0:
            logger.info(
                f"DeepScale Flops Profiler Enabled: {self.flops_profiler_enabled()}")
    ...
    def _configure_lr_scheduler(self, client_lr_scheduler):
        ...
        if lr_scheduler:
            if self.global_rank == 0:
                logger.info(
                    f"DeepScale using configured LR scheduler = {self.scheduler_name()}")
            self.lr_scheduler = lr_scheduler
        else:
            if self.global_rank == 0:
                logger.info("DeepScale using client LR scheduler")
    ...
    def _scheduler_from_config(self, optimizer):
        ...
        if scheduler_name is not None:
            if hasattr(lr_schedules, scheduler_name):
                ...
                ), f"DeepScale does not recognize LR scheduler {scheduler_name}"
    ...
    def _configure_with_arguments(self, args, mpu):
        ...
        if self.config is None:
            self.config = (args.deepscale_config
                           if hasattr(args,
                                      "deepscale_config") else None)
        self._config = DeepScaleConfig(self.config, mpu)

    # Validate command line arguments
    def _do_args_sanity_check(self, args):
        if hasattr(args, "deepscale_config") and args.deepscale_config is not None:
            logger.warning(
                "************ --deepscale_config is deprecated, please use --deepscale_config ************"
            )
            if hasattr(args, "deepscale_config"):
                assert (
                    args.deepscale_config is None
                ), "Not sure how to proceed, we were given both a deepscale_config and deepscale_config"
            args.deepscale_config = args.deepscale_config

        assert (
            "LOCAL_RANK" in os.environ or "OMPI_COMM_WORLD_LOCAL_RANK" in os.environ
        ), (
            "DeepScale requires the LOCAL_RANK environment "
            "variable, it is set by the deepscale launcher, deepscale.init_distributed, or the torch.distributed launcher. If using a "
        ...
        if self.config is None:
            assert (
                hasattr(args, "deepscale_config") and args.deepscale_config is not None
            ), "DeepScale requires --deepscale_config to specify configuration file"

            assert os.path.isfile(
                args.deepscale_config
            ), "DeepScale configuration file: {} is not an existing file".format(
                args.deepscale_config
            )

    def _is_supported_optimizer(self, optimizer_name):
        return (optimizer_name in DEEPSCALE_OPTIMIZERS
    ...
    def _do_sanity_check(self):
        if not self.client_optimizer:
            if self.optimizer_name() is not None:
                assert self._is_supported_optimizer(
                    self.optimizer_name()
                ), "{} is not a supported DeepScale Optimizer".format(
                    self.optimizer_name()
                )
        ...
        if (self.optimizer_name() == LAMB_OPTIMIZER
            ...
            ), "DeepScale {} optimizer requires dynamic loss scaling".format(
                self.optimizer_name()
            )
    ...
    def _check_for_duplicates(self, optimizer):
        for name, param in self.module.named_parameters():
            ...
            occurance = sum([(ids_list(group["params"]).count(param_id)
    ...
    def _configure_optimizer(self, client_optimizer, model_parameters):
        ...
        if client_optimizer is not None:
            ...
            if self.global_rank == 0:
                logger.info(
                    "Using DeepScale Optimizer param name {} as basic optimizer".format(
                        self.optimizer_name()))

        self._check_for_duplicates(basic_optimizer)

        if self.global_rank == 0:
            logger.info("DeepScale Basic Optimizer = {}".format(
        ...
        log_dist("DeepScale Final Optimizer = {}".format(self.optimizer_name()),
                 ranks=[0])
    ...
    def _configure_basic_optimizer(self, model_parameters):
        ...
        if "max_grad_norm" in optimizer_parameters.keys():
            raise ValueError(
                "'max_grad_norm' is not supported as an optimizer parameter, please switch to using the deepscale parameter 'gradient_clipping' see: https://www.deepscale.khulnasoft.com/docs/config-json/#gradient-clipping for more details"
            )
        ...
        if self.optimizer_name() in [ADAM_OPTIMIZER, ADAMW_OPTIMIZER]:
            ...
            effective_adam_w_mode = (self.optimizer_name() == ADAMW_OPTIMIZER
                                     or adam_w_mode)
    ...
    def _configure_fp16_optimizer(self, optimizer):
        ...
        if (isinstance(optimizer,
                       fused_opts) or self.optimizer_name() == ONEBIT_ADAM_OPTIMIZER):
    ...
    def _configure_zero_optimizer(self, optimizer):
        ...
        if (self.zero_legacy_stage1()
            ...
            optimizer = FP16_DeepScaleZeroOptimizer_Stage1(
                optimizer,
                static_loss_scale=self.loss_scale(),
                dynamic_loss_scale=self.dynamic_loss_scale(),
                dynamic_loss_args=self.dynamic_loss_scale_args(),
            ...
            optimizer = FP16_DeepScaleZeroOptimizer(
                optimizer,
                timers=timers,
                static_loss_scale=self.loss_scale(),
                dynamic_loss_scale=self.dynamic_loss_scale(),
            ...
            from deepscale.runtime.zero.stage3 import FP16_DeepScaleZeroOptimizer_Stage3

            optimizer = FP16_DeepScaleZeroOptimizer_Stage3(
                self.module,
                optimizer,
                timers=timers,
                static_loss_scale=self.loss_scale(),
    ...
    def _scale_loss_by_gas(self, prescaled_loss):
        if isinstance(prescaled_loss, torch.Tensor):
            ...
            if self.warn_unscaled_loss:
                logger.warning(
                    f"DeepScale unable to scale loss because of type: {type(prescaled_loss)}"
                )
    ...
    def load_moe_state_dict(self, checkpoint_path, tag, state_dict):
        ...
        for local_expert_id in range(num_local_experts):
            ...
            expert_state_dict = torch.load(
                self._get_expert_ckpt_name(checkpoint_path,
                                           global_expert_id,
                                           tag),
    ...
    def _load_checkpoint(
        ...
        # deepscale master branch uses a new system now
        ...
        client_state = {
            key: value
            for key,
            value in model_checkpoint.items() if not key in deepscale_states
        }
    ...
    def _zero3_consolidated_fp16_state_dict(self):
        ...
        def get_layer_state_dict(module, prefix=""):
            ...
            with deepscale.zero.GatheredParameters(list(
    ...
    def set_train_batch_size(self, train_batch_size):
        ...
        if (train_batch_size %
            (self.gradient_accumulation_steps() * self.dp_world_size) != 0):
            raise ValueError(
