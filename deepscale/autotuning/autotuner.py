...
from .config import DeepScaleAutotuningConfig
...
class Autotuner:
    ...
    def __init__(self, args, active_resources):
        ...
        assert tabulate is not None, "Missing required package `tabulate`, please install with `pip install deepscale[autotuning]`."

        logger.debug(f"autotuning args={args}")

        self.user_config = self._get_user_config(args.user_args)
        assert self.user_config is not None, "DeepScale configuration is not provided"

        self.autotuning_config = DeepScaleAutotuningConfig(self.user_config)
    ...
    def _get_user_config(self, user_args):
        """Get DeepScale configuration from the user arguments passed to the launcher.

        Args:
            user_args ([list]): user arguments passed to the DeepScale launcher

        Returns:
            [dict]: DeepScale configuration dictionary
        """
        user_config_file = None
        if "--deepscale_config" in user_args:
            idx = user_args.index("--deepscale_config")
            assert ".json" in user_args[
                idx + 1], "DeepScale --deepscale_config requires a json file to specify the configuration"

            user_config_file = user_args[idx + 1]
        elif "--deepscale" in user_args:
            idx = user_args.index("--deepscale")
            if ".json" in user_args[idx + 1]:
                user_config_file = user_args[idx + 1]

        logger.debug(f"user_config_file = {user_config_file}")
        if user_config_file is not None:
            assert os.path.isfile(user_config_file), "DeepScale configuration file: {} is not an existing file".format(
                user_config_file)
    ...
    def _generate_experiments(self, tuning_space, max_train_batch_size_per_gpu):
        """Generates a list of autotuning experiments given a tuning_space.
            The corresponding parameter values are replaced by user-defined values in the DeepScale configuration file.
        Args:
            tuning_space ([dict]): A DeepScale configuration dictionary where a value can be a list (called a tuning parameter). For example,
                {
                    "zero_optimization": {
                        "stage": 1,
        ...
        Returns:
            [list]: a list of experiments generated by taking combinations of values of the tuning space. The above tuning space generates 3*3 = 9 experiments if the user DeepScale configuration file does not overwrite the two tuning parameters or define more tuning parameters.
        """
        exps = []
        ...
        # replace the corresponding parameter values if the user specifies them in the DeepScale configuration file
    ...
    def tune(self):
        ...
        if self.gpu_mem > required_gpu_mem:
            ...
            logger.info(
                f"The model has {self.get_model_num_params()} parameters and requires at least {memory_to_string(required_gpu_mem, postfix='B')} memory per GPU with DeepScale Zero stage {ZeroStageEnum.weights} optimization. Memory per GPU in system is {memory_to_string(self.gpu_mem)}. No tuning is performed."
            )
            return
        if has_mlflow:
            mlflow.end_run()
    ...
    def model_info_profile_run(self):
        ...
        for exp_id, (exp_json, err) in self.rm.finished_experiments.items():
            self.rm.clear()
            if err:
                logger.error(f"The model is not runnable with DeepScale with error = {err}")
                return None
    ...
    def write_optimal_config(self):
        ...
        if best_exp:
            ...
            logger.info(
                f"Wrote the optimal DeepScale configuration found by autotuning to {ds_config_path}, and the corresponding DeepScale command to {cmd_path}"
            )

    def run_after_tuning(self):
        """ Launches the training with the optimal DeepScale configuration found through the autotuning process.
            "ds_config_optimal.json" describing the optimal DeepScale configuration as well the command used to launch training "cmd_optimal.txt" are saved to self.results_dir.
        """
        if self.optimal_cmd:
            result = subprocess.Popen(self.optimal_cmd)
            result.wait()

            logger.info(f"Done running with the optimal DeepScale configuration using {self.optimal_cmd}")
        else:
            logger.info(f"No optimal DeepScale configuration found by autotuning.")
