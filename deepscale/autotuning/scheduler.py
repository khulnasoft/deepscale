...
class ResourceManager:
    ...
    def parse_results(self, metric):
        """ Parses the metric file of the finished experiments to select the optimal DeepScale configuration.
...
def run_experiment(exp: dict, reservations, user_script, user_args):
    ...
    # remove "--deepscale_config ds_config.json" from user_args
    if user_args:
        if "--deepscale_config" in user_args:
            idx = user_args.index("--deepscale_config")
        # "--deepscale_config" is omitted in HF
        elif "--deepscale" in user_args:
            idx = user_args.index("--deepscale")
        assert idx < len(user_args), "there is no ds_config file specified after --deepscale_config or --deepscale"
    ...
    cmd = ["deepscale"] + exp["launcher_args"] + [user_script] + user_args
