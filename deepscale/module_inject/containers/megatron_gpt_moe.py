...
class DS_MegatronGPTMoEContainer(MegatronContainer, BaseTransformerMoEContainer):
    ...
    def create_module(self, config=None):
        ...
        self.module = DeepScaleMegatronGPTInference(_config, mp_group=self.mp_group)
...
class MegatronMoELayerPolicy(MegatronLayerPolicy):
    ...
    def mlp(self, moe_type='standard', enable_training=False):
        # for now, all of this is tightly coupled to megatron-deepscale moe implementation
        # todo: think and refactor this to be more general

        #from deepscale.moe.utils import has_moe_layers
        #moe, _ = has_moe_layers(self.client_module)

        moe_experts = self.client_module.mlp.deepscale_moe.experts.deepscale_experts if moe_type == 'standard' else \
                        self.client_module.mlp.moe.deepscale_moe.experts.deepscale_experts
