...
def quantize_transformer_layer(orig_layer_impl, model, megatron=False, preln=False):
    """ Quantize bert-style transformer layers with DeepScale's transformer layer
    Arguments:
        ...
        Note: For Bert kind of models, we inject based on the DeepScale-Example models, if not setting huggingface flag.
    ...
    def quantize_fn(child):
        if megatron:
            ...
            # Quantize either DeepScale or HuggingFace trained model
