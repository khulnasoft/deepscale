...
import tqdm
import deepscale
import deepscale.ops.transformer as transformer_inference
from deepscale.ops.transformer.inference.diffusers_attention import DeepScaleDiffusersAttention
from deepscale.ops.transformer.inference.diffusers_transformer_block import DeepScaleDiffusersTransformerBlock
from deepscale.ops.transformer.inference.diffusers_2d_transformer import Diffusers2DTransformerConfig
...
from deepscale.module_inject.tp_shard import set_num_kv_heads, set_n_embd, set_num_attention_heads
...
def generic_injection(module, dtype=None, enable_cuda_graph=True):
    ...
    def replace_attn(child, policy):
        ...
        config = transformer_inference.DeepScaleInferenceConfig(
        ...
        attn_module = DeepScaleDiffusersAttention(config)
    ...
    def replace_attn_block(child, policy):
        ...
        return DeepScaleDiffusersTransformerBlock(child, config)
...
def replace_transformer_layer(orig_layer_impl, model, checkpoint_dict, config, model_config):
    """ Replace bert-style transformer layers with DeepScale's transformer layer
    Arguments:
...
def revert_transformer_layer(orig_layer_impl, model, config, preln=False):
    """ Revert DeepScale's transformer layer back to original bert-style transformer layer
    Arguments:
    ...
    return replace_module(model=model,
                          orig_class=deepscale.DeepScaleTransformerLayer,
                          replace_fn=replace_fn,
...
def replace_module(model, orig_class, replace_fn, _replace_policy, checkpoint=None):
    ...
    assert len(policy.items()) > 0,\
        ...
        "You can find some samples here: https://github.com/khulnasoft/DeepScale/blob/master/deepscale/module_inject/replace_policy.py"
...
def _replace_module(model, policies, prefix='', layer_id=0, level_id=0, state_dict=None):
    ...
    model.reset_cache = transformer_inference.DeepScaleTransformerInference.reset_cache
