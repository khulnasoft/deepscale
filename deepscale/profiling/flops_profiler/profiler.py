...
class FlopsProfiler(object):
    ...
    The DeepScale flops profiler can be used with the DeepScale runtime or as a standalone package.
    When using DeepScale for model training, the flops profiler can be configured in the deepscale_config file and no user code change is required.
    ...
    def get_total_params(self, as_string=False):
        ...
        """
        return (params_to_string(self.model.__params__)
    ...
    def print_model_profile(
        ...
        print("{:<60}  {:<8}".format(
            "fwd flops per GPU = 2 * fwd MACs per GPU: ",
...
def _conv_flops_compute(input,
    ...
    conv_per_position_flops = (int(_prod(kernel_dims)) * in_channels *
                               filters_per_channel)
...
def _rnn_flops(flops, rnn_module, w_ih, w_hh, input_size):
    ...
    if isinstance(rnn_module, (nn.RNN, nn.RNNCell)):
        ...
        flops += (rnn_module.hidden_size + rnn_module.hidden_size +
