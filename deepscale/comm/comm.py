...
# DeepScale Team
"""
    DeepScale Communication Package: deepscale.comm
    deepscale.comm
        -- import and use deepscale.ops.comm
        ...
        Note: the old 1-bit compressed allreduce variants that resided in deepscale.runtime.comm will be moved here as well.
    deepscale.comm API
    ...
    Future:
        -- deepscale groups API should be brought into ds.comm
"""
...
from deepscale.comm.ccl import CCLBackend

from deepscale.utils.comms_logging import CommsLogger
from deepscale.utils import timer, get_caller_func
from deepscale.comm.torch import TorchBackend

from deepscale import utils
from datetime import timedelta

# Current deepscale.comm backend (cdb) global object for simple access by client code
cdb = None
...
from deepscale.comm.utils import *
...
def configure(
    ...
    if deepscale_config is not None:
        _configure_using_config_file(deepscale_config.comms_config)
...
# DeepScale NCCL/MPI backend may not need all these params as we will have our own implementation.
...
def init_deepscale_backend(ds_backend, timeout, init_method):
    global cdb
    global nccl_backend
    global mpi_backend
    global ccl_backend
    ...
    if ds_backend == NCCL_BACKEND:
        utils.logger.debug("NCCL backend in DeepScale not yet implemented")
    elif ds_backend == MPI_BACKEND:
        utils.logger.debug("MPI backend in DeepScale not yet implemented")
    elif ds_backend == GLOO_BACKEND:
        utils.logger.debug("Gloo backend in DeepScale not yet implemented")
        ...
        utils.logger.debug("HCCL backend in DeepScale not yet implemented")
    else:
        utils.logger.debug(f"DeepScale does not support {ds_backend} backend")


def is_initialized():
    #assert cdb is not None, 'DeepScale backend not set, please initialize it using init_process_group()'
    if cdb is None:
        return False
    else:
...
def new_group(ranks):
    ...
    ), 'DeepScale backend not set, please initialize it using init_process_group()'
    return cdb.new_group(ranks)


def is_available() -> bool:

    # Returns ``True`` if the deepscale comm package is available.

    # TODO: load other ops. Clients including deepscale itself should use deepscale.comm to import
    # any communication related primitives from this package.
    # use hasattr(deepscale.csrc.ops, "_comm") or something
    return True
...
# Main DeepScale Comms. public API.
def init_distributed(dist_backend=None,
    ...
    Arguments:
        ...
        timeout: Optional (timedelta). Timeout for operations executed against the process group. The default value of 30 minutes can be overridden by the environment variable `DEEPSCALE_TIMEOUT`.
        init_method: Optional (string). Torch distributed, URL specifying how to initialize the process group. Default is “env://” if no init_method or store is specified.
        config: Optional (dict). DeepScale configuration for setting up comms options (e.g. Comms profiling)
    ...
    configure(deepscale_config=config)

    if dist_init_required is None:
        dist_init_required = cdb is None or not cdb.is_initialized()

    if cdb is None:
        init_deepscale_backend(get_accelerator().communication_backend_name(), timeout, init_method)
        set_backend()
    ...
    if dist_init_required is False:
        ...
        ), "Distributed backend is not initialized. Please set dist_init_required to True or initialize before calling deepscale.initialize()"
    else:
        # Initialize torch distributed if needed
        required_env = ["RANK", "WORLD_SIZE", "MASTER_ADDR", "MASTER_PORT", "LOCAL_RANK"]
        if auto_mpi_discovery and not all(map(lambda v: v in os.environ, required_env)):
            if verbose:
                utils.logger.info("Not using the DeepScale or dist launchers, attempting to detect MPI environment...")
        ...
        if cdb is not None and cdb.is_initialized():
            ...
            if int(os.getenv('RANK', '0')) == 0:
                utils.logger.info('Initializing TorchBackend in DeepScale with backend {}'.format(dist_backend))
