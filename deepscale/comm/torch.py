...
def build_shm_op():
    ...
    if builder is None or not deepscale.ops.__compatible_ops__[builder.NAME]:
        return None
    shm_cpp_module = builder.load()
    print(f'DeepScale {builder.absolute_name()} built successfully')
...
class TorchBackend(Backend):
    ...
    def inference_all_reduce(self, tensor, op, group=None):
        if not hasattr(torch.ops, 'deepscale') or not hasattr(torch.ops.deepscale, 'inference_all_reduce_'):
            op = self._reduce_op(op)
            return torch.distributed.all_reduce(tensor=tensor, op=op, group=group, async_op=False)
        else:
            return torch.ops.deepscale.inference_all_reduce_(tensor)
