...
from deepscale.ops.transformer.inference.op_binding.layer_norm import LayerNormOp
from deepscale.utils.logging import log_dist

from deepscale.ops.transformer.inference.ds_mlp import DeepScaleMLP
from deepscale.ops.transformer.inference.ds_attention import DeepScaleSelfAttention, BloomSelfAttention
from deepscale.ops.transformer.inference.op_binding.workspace import WorkspaceOp
from deepscale.accelerator import get_accelerator
import deepscale
if deepscale.HAS_TRITON:
    from deepscale.ops.transformer.inference.triton.mlp import TritonMLP
    from deepscale.ops.transformer.inference.triton.attention import TritonSelfAttention
...
class DeepScaleTransformerInference(nn.Module):
    ...
    def __init__(self,
        ...
        if DeepScaleTransformerInference.layer_id == 1:
            log_dist(f"DeepScale-Inference config: {self.config.__dict__}", [0])
            if deepscale.HAS_TRITON and self.config.use_triton:
        ...
        if self.config.bigscience_bloom:
            ...
            if deepscale.HAS_TRITON and self.config.use_triton:
                ...
                self.attention = DeepScaleSelfAttention(self.config, mp_group, quantize_scales, quantize_groups,
                                                        merge_count)

        if deepscale.HAS_TRITON and self.config.use_triton:
            self.mlp = TritonMLP(self.config)
        else:
            self.mlp = DeepScaleMLP(self.config, mp_group, quantize_scales, quantize_groups, merge_count,
                                    mlp_extra_grouping)
        ...
        if DeepScaleTransformerInference.workspace is None:
            DeepScaleTransformerInference.workspace = WorkspaceOp(self.config)
    ...
    def allocate_workspace(self, size):
        ...
        if self.config.layer_id == 0 and self._should_allocate_workspace:
            DeepScaleTransformerInference.workspace.allocate_workspace(
                self.config.hidden_size, self.config.heads, size[1], size[0], DeepScaleTransformerInference.layer_id,
