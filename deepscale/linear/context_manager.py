...
class Init(object):
    ...
    Example usage:
        lora_config = deepscale.linear.LoRAConfig(..)
        quant_config = deepscale.linear.QuantizationConfig(..)
    ...
    def __exit__(self, *args, **kwargs):
        ...
        if not self._post_init_complete:
            print('WARNING: For some reason LoRA modules are not initialized, this is usually done automatically '
                  ...
                  'You must call `init_lora` on each module in order to use DeepScale LoRA, otherwise '
