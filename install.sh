...
err_report() {
    ...
    echo "Fail to install deepscale"
}
...
By default will install deepscale and all third party dependencies across all machines listed in
...
[optional]
    ...
    -e, --examples          Checkout deepscale example submodule (no install)
...
if [ "$allow_sudo" == "0" ]; then
    if [ "$user" == "root" ]; then
        echo "WARNING: running as root, if you want to install DeepScale with sudo please use -s/--pip_sudo instead"
        usage
        exit 1
    fi
fi
...
if [ "$no_clean" == "0" ]; then
    # remove deepscale build files
    rm_if_exist deepscale/git_version_info_installed.py
    rm_if_exist dist
    rm_if_exist build
    rm_if_exist deepscale.egg-info
fi
...
if [ "$local_only" == "1" ]; then
    ...
    tmp_wheel_path="/tmp/deepscale_wheels"
    ...
    echo "Installing deepscale"
    pdsh -w $hosts "$PIP_SUDO pip uninstall -y deepscale"
    pdcp -w $hosts dist/deepscale*.whl $tmp_wheel_path/
    pdsh -w $hosts "$PIP_SUDO $PIP_INSTALL $tmp_wheel_path/deepscale*.whl"
