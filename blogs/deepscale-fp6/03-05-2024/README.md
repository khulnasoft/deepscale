...
# DeepScale-FP6: The Power of FP6-Centric Serving for Large Language Models

</div>

<div align="center">

<img src="./assets/hero-figure.png" width="1000px" alt="DeepScale-VisualChat!"/>

</div>


To cite DeepScale-FP6, please cite the following two arxiv reports - ZeroQuant(4+2) and FP6-LLM:
...
<p align="center">
  ...
  *Figure 1*:  End-to-end serving performances in DeepScale-MII with 32 clients and total of 128 requests, for LLaMA-2-70B model on 2xA100-80g with two-way tensor parallelism. We experimented with different number of requests between 128, 256 and 512 and found that the speedup is simillar.
...
# 4. How to begin with DeepScale-FP6  <a name="how-to-start"></a>

The quantization-and-inference experience of DeepScale-FP6 is straightforward and convenient. Here we give an example based on LLaMa-2-70B model:

```python
import mii
pipe = mii.pipeline("NousResearch/Llama-2-70b-hf", quantization_mode='wf6af16')
response = pipe(["DeepScale is", "Seattle is"], max_new_tokens=128)
print(response)
```
...
To benchmark with our DeepScale-FP6, please visit the following script:
```bash
https://github.com/khulnasoft/DeepScaleExamples/blob/master/benchmarks/inference/mii/run_fp6.sh
```
...
Currently, DeepScale-FP6 supports only dense models with MoE models support upcoming. We will continue to improve DeepScale-FP6 with your feedback and support. DeepScale-FP6 is a component of the larger DeepScale ecosystem, which includes a range of Deep Learning systems and modeling technologies. To learn more,
...
* "Star" our [DeepScale GitHub](https://github.com/khulnasoft/DeepScale/) and [DeepScale-MII GitHub](https://github.com/khulnasoft/DeepScale-MII/) and [DeepScaleExamples GitHub](https://github.com/khulnasoft/DeepScaleExamples/)  repositories if you like our work!
