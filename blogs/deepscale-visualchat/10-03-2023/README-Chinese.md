...
# DeepScale-VisualChat：多轮图像+文字，为你展现不一样的AI聊天魅力
...
要引用 DeepScale-VisualChat，请引用我们的 [arxiv 报告](https://arxiv.org/abs/2309.14327)：
...
为了解决这些问题，我们推出了 DeepScale-VisualChat（请参阅 [arxiv 报告](https://arxiv.org/abs/2309.14327) 以获取更多详细信息），带有以下新特性：

* ***全开源多轮多图框架与前所未有的可扩展性***：DeepScale-VisualChat，作为开创性的全开源框架之一，支持多轮和多图对话，容纳交错的文本和图像输入。我们利用 DeepScale 提高我们的训练效果，使用一个 2B 的视觉编码器和一个 70B 的 LLaMA-2 解码器模型，展示了我们框架的显著可扩展性。
...
* ***交错输入的数据混合*** 为了促进交错模态的对话，DeepScale-VisualChat 在现有数据集上采用了各种数据混合技术，克服了大多数现有开源数据集中交错文本和图像输入的短缺。

# 2. 模型架构概述
<div align="center">
  <img src="../assets/images/model.png" alt="模型结构" width="400"/>

  *图 1：DeepScale-VisualChat 的模型架构示意图。*
</div>

如 *图 1* 所示，DeepScale-VisualChat 的模型架构由三个部分组成：一个视觉编码器，如 CLIP；一个语言解码器，如 LLaMa-7B；和一个特征对齐线性投影层。模型的大部分都是冻结的，只有语言模型的嵌入和线性投影层是可训练的。因此，可训练参数的总数大约在 O(10M) (LLaMa-2-13B) 到 O(100M) (LLaMa-2-70B) 之间。

# 3. DeepScale 多模态因果注意力
...
<b>演示结果。</b>我们首先通过几个例子展示在不同的注意机制下 DeepScale-VisualChat 的单图像视觉语言对话功能。在这些实验中，我们使用 LLaMA2-7B 语言模型和 QWen-VL 视觉编码器作为我们的视觉编码器。这两个模型通过一个简单的线性投影层连接在一起。这个模型在两个 LLaVa 数据集上进行了训练。正如 *图 3* 和 *图 4* 所示，当与 MMCA 配合使用时，DeepScale-VisualChat 有效地识别了图像中的视觉细节，对用户的问题提供了准确通顺的回答。
...
<div align="center">
  ...
  *图 3：示例视觉和语言输入，显示了（1）标准因果注意力 (CA) （2）与交叉注意力组合的标准因果注意力 (CA+ CrA) 和（3）DeepScale-VisualChat 中的特殊多模态因果注意力 (MMCA) 之间的输出比较。*
</div>

<div align="center">
  <img src="../assets/images/lake-chat.png" alt="美丽的湖泊" width="600"/>

  *图 4：DeepScale-VisualChat 准确地识别了场景是一个美丽的湖泊，并提供了一组合理的建议。相比之下，其他的注意力机制误解了图像认为其包含“带船坡的码头”。*
</div>
...
# 5. 演示
我们在几个开源数据集上训练了我们的 DeepScale-VisualChat-13B 模型，该模型使用一个 2B 的视觉编码器和 13B 的 LLaMA 模型。DeepScale-VisualChat-13B 展示了图像字幕功能（*图 6--8*），计数和文本阅读（*图 6*），名人识别（*图 7*），讲故事（*图 8*）等。

<div align="center">
  <img src="../assets/images/friends.png" alt="朋友们" width="600"/>

  *图 6：DeepScale-VisualChat 可以计算图像中的人数，并读取第一张图像中的文本。它还展示了跨图像的理解。*
</div>

<div align="center">
  <img src="../assets/images/ceos.png" alt="CEO" width="600"/>

  *图 7：DeepScale-VisualChat 可以识别名人并将他们与其成就联系起来。*
</div>

<div align="center">
  <img src="../assets/images/zootopia.png" alt="疯狂动物城" width="600"/>

  *图 8：DeepScale-VisualChat 可以讲故事并识别电影。*
</div>

# 6. 如何开始使用 DeepScale-VisualChat
DeepScale-VisualChat 是一个易于使用的训练框架，具有很好的可扩展性，到目前为止已经在 LLaMa-2-70B 模型上进行了测试。我们为所有实验采用了统一的指令调优格式，模板如下所示。
```
...
使用 DeepScale-VisualChat 训练模型是简单和方便的。这里我们给出了基于 CLIP 视觉编码器和 LLaMa-7B 模型的一个例子：
...
为了支持更大的模型推理，我们已经将 Hugging Face 大模型推理集成到我们的 DeepScale-VisualChat API 中。因此，用户可以根据 GPU 内存容量和模型大小选择不同数量的 GPU。

请参考我们的 [GitHub 主页](https://github.com/khulnasoft/DeepScaleExamples/tree/master/applications/DeepScale-VisualChat) 了解更多细节。

# 7. 发布：今天尝试 DeepScale-VisualChat！

我们非常兴奋地分享 DeepScale-VisualChat 现已开源并供 AI 社区使用。

* 要开始使用，请访问我们的 DeepScale-VisualChat GitHub 页面：[GitHub 主页](https://github.com/khulnasoft/DeepScaleExamples/tree/master/applications/DeepScale-VisualChat)

* 我们将继续在您的反馈和支持下改进 DeepScale-VisualChat。我们的 [路线图](https://github.com/khulnasoft/DeepScaleExamples/tree/master/applications/DeepScale-VisualChat/README.md#-deepscale-visualchats-roadmap-) 显示了目前支持的功能以及未来计划支持的功能。

DeepScale-VisualChat 是更大的 DeepScale 生态系统的一部分，其中包括一系列深度学习系统和建模技术。要了解更多信息，

* 请访问我们的 [网站](https://www.deepscale.ai/) 了解详细的博客文章、教程和文档。
* 在我们的 [英文 X(Twitter)](https://twitter.com/MSFTDeepScale)、[日语 X(Twitter)](https://twitter.com/MSFTDeepScaleJP) 和 [中文知乎](https://www.zhihu.com/people/deepscale) 上关注我们，以获取 DeepScale 的最新消息。

我们欢迎您为 DeepScale 做出贡献！我们鼓励您报告问题、贡献 PRs、并在 [DeepScale GitHub](https://github.com/khulnasoft/DeepScale/) 页面上参加讨论。有关更多详细信息，请查看我们的 [贡献指南](https://github.com/khulnasoft/DeepScale/blob/master/CONTRIBUTING.md)。我们对与大学、研究实验室、公司等进行合作持开放态度，例如共同进行深度学习研究、应用 DeepScale 为现实世界的 AI 模型和应用提供支持等等。对于此类请求（以及其他不适合 GitHub 的请求），请直接发送电子邮件至 deepscale-info@microsoft.com。

* 如果你喜欢我们的工作，请在 [DeepScale GitHub](https://github.com/khulnasoft/DeepScale/) 和 [DeepScaleExamples GitHub](https://github.com/khulnasoft/DeepScaleExamples/) 上为我们的仓库点“星”。
