...
deepscale_aio_config_t::deepscale_aio_config_t()
...
deepscale_aio_config_t::deepscale_aio_config_t(const int block_size,
...
void deepscale_aio_latency_t::dump(const std::string tag)
{
    std::cout << tag << _min_usec << " " << _max_usec << " " << _avg_usec << " " << std::endl;
}

void deepscale_aio_latency_t::accumulate(const struct deepscale_aio_latency_t& other)
{
...
void deepscale_aio_latency_t::scale(const float scaler)
{
    _min_usec *= scaler;
    _max_usec *= scaler;
    _avg_usec *= scaler;
