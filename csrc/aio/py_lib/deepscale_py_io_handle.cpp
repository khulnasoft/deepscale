...
static void _start_aio_thread(std::shared_ptr<struct deepscale_aio_thread_t> ctxt) { ctxt->run(); }

deepscale_io_handle_t::deepscale_io_handle_t(const int block_size,
    ...
    : _aio_ctxt(new aio_context(block_size, queue_depth)),
      ...
      _num_pending_ops(0),
      _pinned_tensor_mgr(new deepscale_pin_tensor_t())
{
    for (auto i = 0; i < intra_op_parallelism; ++i) {
        _thread_contexts.push_back(std::make_shared<deepscale_aio_thread_t>(i, _aio_config));
    }
...
deepscale_io_handle_t::~deepscale_io_handle_t()
{
    _stop_threads();
    for (auto& thr : _threads) { thr.join(); }
}

const int deepscale_io_handle_t::get_block_size() const
{
    return _aio_ctxt ? _aio_ctxt->_block_size : -1;
}

const int deepscale_io_handle_t::get_queue_depth() const
{
    return _aio_ctxt ? _aio_ctxt->_queue_depth : -1;
}

const bool deepscale_io_handle_t::get_single_submit() const { return _single_submit; }

const bool deepscale_io_handle_t::get_overlap_events() const { return _overlap_events; }

const int deepscale_io_handle_t::get_intra_op_parallelism() const { return _intra_op_parallelism; }

int deepscale_io_handle_t::read(torch::Tensor& buffer, const char* filename, const bool validate)
{
...
int deepscale_io_handle_t::write(const torch::Tensor& buffer,
                                 const char* filename,
                                 const bool validate)
{
    assert(_aio_ctxt);
...
void deepscale_io_handle_t::_schedule_aio_work(std::shared_ptr<struct io_op_desc_t> scheduled_op)
{
    for (auto& ctxt : _thread_contexts) {
        {
...
std::shared_ptr<struct io_op_desc_t> deepscale_io_handle_t::_wait_for_aio_work()
{
...
void deepscale_io_handle_t::_stop_threads()
{
...
int deepscale_io_handle_t::wait()
{
...
bool deepscale_io_handle_t::_is_valid_parallel_aio_op(const bool read_op, const int64_t num_bytes)
{
    const auto op_string = read_op ? "Read" : "Write";
    if (num_bytes % get_intra_op_parallelism()) {
        std::cout << "deepscale_aio failure: parallel " << op_string << " num_bytes = " << num_bytes
...
std::shared_ptr<struct io_op_desc_t> deepscale_io_handle_t::_create_io_op_desc(
    const bool read_op,
...
int deepscale_io_handle_t::pread(const torch::Tensor& buffer,
                                 const char* filename,
                                 const bool validate,
                                 const bool async)
{
...
int deepscale_io_handle_t::pwrite(const torch::Tensor& buffer,
                                  const char* filename,
                                  const bool validate,
                                  const bool async)
{
...
int deepscale_io_handle_t::sync_pread(torch::Tensor& buffer, const char* filename)
{
    return pread(buffer, filename, false, false);
}

int deepscale_io_handle_t::sync_pwrite(const torch::Tensor& buffer, const char* filename)
{
    return pwrite(buffer, filename, false, false);
}

int deepscale_io_handle_t::async_pread(torch::Tensor& buffer, const char* filename)
{
    return pread(buffer, filename, false, true);
}

int deepscale_io_handle_t::async_pwrite(const torch::Tensor& buffer, const char* filename)
{
    return pwrite(buffer, filename, false, true);
}

at::Tensor deepscale_io_handle_t::new_cpu_locked_tensor(const int64_t num_elem,
                                                        const torch::Tensor& example_tensor)
{
    return _pinned_tensor_mgr->alloc(num_elem, example_tensor.scalar_type());
}

bool deepscale_io_handle_t::free_cpu_locked_tensor(torch::Tensor& locked_tensor)
{
