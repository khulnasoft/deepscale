...
deepscale_pin_tensor_t::~deepscale_pin_tensor_t()
{
...
torch::Tensor deepscale_pin_tensor_t::alloc(const int64_t num_elem,
...
torch::Tensor deepscale_pin_tensor_t::alloc(const int64_t num_elem, const at::ScalarType& elem_type)
{
    auto options = torch::TensorOptions().dtype(elem_type).device(torch::kCPU).requires_grad(false);
    return alloc(num_elem, options);
}

bool deepscale_pin_tensor_t::free(torch::Tensor& locked_tensor)
{
...
bool deepscale_pin_tensor_t::is_managed(const torch::Tensor& buffer)
{
