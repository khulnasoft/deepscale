...
def run_job(job):
    ...
    assert proc.returncode == 0, f"This command failed: {job.cmd()}"
...
def create_read_file(sweep_config):
    ...
    block_size, block_count = get_block_size_and_count(
        refine_integer_value(sweep_config.io_size)
    )
    dd_job = Job(cmd_line=[
...
def main():
    print("Running performance sweep of deepscale nvme library")

    if not async_io_setup():
        error_msg = """
            Failing because environment is not properly configured for deepscale async i/o module.
