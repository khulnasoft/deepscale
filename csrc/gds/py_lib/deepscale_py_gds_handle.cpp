...
#include <cstdlib>
#include "deepscale_gds_op.h"

using namespace std;

int deepscale_gds_handle_t::s_cuFile_init = 0;

deepscale_gds_handle_t::deepscale_gds_handle_t(const int block_size,
                                               const int queue_depth,
                                               const bool single_submit,
                                               const bool overlap_events,
                                               const int intra_op_parallelism)
    : deepscale_io_handle_t(block_size, queue_depth, single_submit, overlap_events, 1),
      _intra_gds_op_parallelism(intra_op_parallelism)
{
    _init_cuFile(block_size, queue_depth);
}

deepscale_gds_handle_t::~deepscale_gds_handle_t() { _close_cuFile(); }

const int deepscale_gds_handle_t::get_intra_op_parallelism() const
{
    return _intra_gds_op_parallelism;
}

void deepscale_gds_handle_t::_init_cuFile(const int block_size, const int queue_depth)
{
    if (deepscale_gds_handle_t::s_cuFile_init == 0) {
    ...
    deepscale_gds_handle_t::s_cuFile_init++;
}

void deepscale_gds_handle_t::_close_cuFile()
{
    deepscale_gds_handle_t::s_cuFile_init--;
    if (deepscale_gds_handle_t::s_cuFile_init == 0) { cuFileDriverClose(); }
}

torch::Tensor deepscale_gds_handle_t::new_pinned_device_tensor(const size_t num_elem,
...
bool deepscale_gds_handle_t::free_pinned_device_tensor(torch::Tensor& buffer)
{
    unpin_device_tensor(buffer);
    return true;
}

bool deepscale_gds_handle_t::pin_device_tensor(const torch::Tensor& buffer)
{
    gds_op_desc_t::add_buffer_to_registry(buffer);
    return true;
}

bool deepscale_gds_handle_t::unpin_device_tensor(const torch::Tensor& buffer)
{
    gds_op_desc_t::remove_buffer_from_registry(buffer);
    return true;
}

std::shared_ptr<struct io_op_desc_t> deepscale_gds_handle_t::_create_io_op_desc(
    const bool read_op,
    ...
    const bool validate)
{
    ...
    return deepscale_io_handle_t::_create_io_op_desc(
